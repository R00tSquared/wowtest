...
{
	return poly && (poly->flags & m_includeFlags) != 0 && (poly->flags & m_excludeFlags) == 0;
}
	...
	*pathCount = 0;
	...
	while (!m_openList->empty())
	{
		...
		for (unsigned int i = bestPoly->firstLink; i != DT_NULL_LINK; i = bestTile->links[i].next)
		{
			...
			if (!filter->passFilter(neighbourRef, neighbourTile, neighbourPoly)) // @!path_crash needed?
				continue;
										 ...
										 float* left, float* right) const
{
	...
	if (toPoly->getType() == DT_POLYTYPE_OFFMESH_CONNECTION)
	{
		...
		{
			volatile unsigned int debug_i = i;

			if (toTile->links[i].ref == from) // @!path_crash needed?
			{
	...
	// @!path_crash debug
	volatile int debug_sizeof = sizeof(fromPoly->verts);
	volatile int debug_edge = link->edge;
	volatile unsigned char debug_char_edge = link->edge;
	volatile unsigned char debug_char_vertCount = (int)fromPoly->vertCount;
	volatile int debug_vertCount = (int)fromPoly->vertCount;

	// @!path_crash fix 20.02.24
	if ((int)fromPoly->vertCount == 0)
		return DT_FAILURE;
