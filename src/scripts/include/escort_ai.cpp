...
void npc_escortAI::DamageTaken(Unit* pDone_by, uint32& uiDamage)
{
	me->SetLastDamagedTime(time(NULL));
	AddEscortState(STATE_ESCORT_INCOMBAT);
}
...
{
    // update combat 
	if (HasEscortState(STATE_ESCORT_ESCORTING) && !HasEscortState(STATE_ESCORT_PAUSED) && HasEscortState(STATE_ESCORT_INCOMBAT) && !UpdateVictim() && time(NULL) - me->GetLastDamagedTime() > 10)
		OnCombatStop();
	
	//Waypoint Updating
	// && !me->IsInCombat()
    if (HasEscortState(STATE_ESCORT_ESCORTING) && WPWaitTimer && !HasEscortState(STATE_ESCORT_INCOMBAT))
    {
        if (WPWaitTimer <= uiDiff)
        {
            ...
            if (!HasEscortState(STATE_ESCORT_PAUSED))
            {
				...
				if (me->IsFormationLeader())
					me->GetFormation()->LeaderMoveTo(CurrentWP->x, CurrentWP->y, CurrentWP->y, false);
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING) && PlayerGUID && !HasEscortState(STATE_ESCORT_INCOMBAT))
    {
...
void npc_escortAI::OnCombatStop()
{
...
void npc_escortAI::Start(bool bIsActiveAttacker, bool bRun, uint64 uiPlayerGUID, const Quest* pQuest, bool bInstantRespawn, bool bCanLoopPath)
{
    ...
    debug_log("TSCR: EscortAI started with %llu waypoints. ActiveAttacker = %d, Run = %d, PlayerGUID = %llu", WaypointList.size(), IsActiveAttacker, IsRunning, PlayerGUID);
