...
struct npc_ameAI : public npc_escortAI
{
    ...
    Timer_UnCheked DEMORALIZINGSHOUT_Timer;
...
struct npc_ringoAI : public FollowerAI
{
    ...
    void Reset()
    {
        m_uiFaintTimer.Reset(urand(30000, 45000));
        ...
        if (!UpdateVictim())
        {
            ...
            else if (HasFollowState(STATE_FOLLOW_INPROGRESS))
            {
                if (!HasFollowState(STATE_FOLLOW_PAUSED))
                {
                    ...
                    {
                        SetFaint();
                        m_uiFaintTimer = urand(35000, 50000);
                    }
                }
            }
...
{
    npc_simone_the_inconspicuousAI(Creature *c) : ScriptedAI(c) {}

    Timer FoolsPlightTimer;
    Timer TemptressKissTimer;
    Timer LightingBoltTimer;
    Timer CheckTimer;
    bool CleanersSummoned;
    uint8 phase;

    void Reset()
    {
        FoolsPlightTimer.Reset(3000);
        TemptressKissTimer.Reset(1000);
        LightingBoltTimer.Reset(3000);
        CheckTimer = 0;
        CleanersSummoned = false;
        phase = 0;
    ...
    {
        Creature* Precious = (Creature*)FindCreature(14538, 50, me);
        if (Precious)
        {
            if (!Precious->isAlive())
                Precious->Respawn();
            if (Unit* target = SelectUnit(SELECT_TARGET_RANDOM, 0))
                Precious->AI()->AttackStart(target);
            Precious->SetIsDistanceToHomeEvadable(false);
        }
    ...
    void SpellHit(Unit* pCaster, const SpellEntry* pSpell)
    {
        for (int j = 0; j < 3; j++)
        {
            if (pSpell->EffectApplyAuraName[j] == 64)
            {
                me->CastSpell(me, 23207, true);
                break;
            }
        }
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        if (CheckTimer.Expired(diff))
        {
            ...
            {
                case 0:
                    me->MonsterSay(-1200090);
                    phase++;
                break;

                case 1:
                    me->UpdateEntry(14533);

                    if (Creature* Precious = (Creature*)FindCreature(14528, 50, me))
                    {
                        if (!Precious->isAlive())
                            Precious->Respawn();

                        Precious->UpdateEntry(14538);
                        Precious->SetSpeed(MOVE_WALK, 1.0f, true);
                        Precious->SetSpeed(MOVE_RUN, 1.1f, true);
                    }
                ...
                default:
                    if (m_creature->getThreatManager().getThreatList().size() > 1 && !CleanersSummoned)
                    {
                        me->Say(-1200091, LANG_UNIVERSAL, 0);
                        me->SummonCreature(14503, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 3000);
                        CleanersSummoned = true;
        ...
        if (me->GetEntry() == 14527)
        {
            if (FoolsPlightTimer.Expired(diff))
            {
            ...
            if (HealthBelowPct(30) && !CleanersSummoned)
            {
                me->Say(-1200092, LANG_UNIVERSAL, 0);
                me->SummonCreature(14503, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 3000);
                CleanersSummoned = true;
                me->ForcedDespawn(1000);
            }
        ...
        {
            if (TemptressKissTimer.Expired(diff))
            {
                AddSpellToCast(me->GetVictim(), 23205);
                TemptressKissTimer = 45000;
            }
        
            if (LightingBoltTimer.Expired(diff))
            {
                AddSpellToCast(me->GetVictim(), 20802);
                LightingBoltTimer = 3000;
            }
...
CreatureAI* GetAI_npc_simone_the_inconspicuous(Creature *_Creature)
{
    return new npc_simone_the_inconspicuousAI (_Creature);
}

bool GossipHello_npc_simone_the_inconspicuous(Player *player, Creature *_Creature)
{
    if (player->GetClass() == CLASS_HUNTER && player->GetQuestStatus(7636) == QUEST_STATUS_INCOMPLETE)
        player->ADD_GOSSIP_ITEM( 0, player->GetSession()->GetHellgroundString(16018), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);

    player->SEND_GOSSIP_MENU(7041, _Creature->GetGUID());

    return true;
}

bool GossipSelect_npc_simone_the_inconspicuous(Player *player, Creature *_Creature, uint32 sender, uint32 action)
{
    if (action == GOSSIP_ACTION_INFO_DEF + 1)
    {
        CAST_AI(npc_simone_the_inconspicuousAI, _Creature->AI())->CheckTimer.Reset(2000);
...
void AddSC_ungoro_crater()
{
    ...
    newscript->pQuestAcceptNPC = &QuestAccept_npc_ame;
