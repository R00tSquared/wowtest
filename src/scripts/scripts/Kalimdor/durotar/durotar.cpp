...
enum LazyPeon
{
    ...
    // Wowwiki: It takes a minute or two for them to cycle through their activities
    MIN_TIME_TO_GO_ASLEEP     = 60000,         //1 minute
    MAX_TIME_TO_GO_ASLEEP     = 120000,        //2 minutes
...
struct npc_lazy_peonAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_uiRebuffTimer <= uiDiff)
        {
            ...
            m_uiRebuffTimer = urand(MIN_TIME_TO_GO_ASLEEP, MAX_TIME_TO_GO_ASLEEP);        //Rebuff agian in 1-2 minutes
        }
        else
