...
struct boss_sarturaAI : public ScriptedAI
{
    ...
    Timer_UnCheked WhirlWindRandom_Timer;
    Timer_UnCheked WhirlWindEnd_Timer;
    Timer_UnCheked AggroReset_Timer;
    Timer_UnCheked AggroResetEnd_Timer;
    Timer_UnCheked EnrageHard_Timer;
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (!WhirlWind)
        {
            ...
            if (!Enraged)
            {
                if (me->GetHealthPercent() <= 20 && !m_creature->IsNonMeleeSpellCast(false))
                {
