...
struct eye_of_cthunAI : public Scripted_NoMovementAI
{
    ...
    //Global variables
    Timer_UnCheked PhaseTimer;

    //Eye beam phase
    Timer_UnCheked BeamTimer;
    Timer_UnCheked EyeTentacleTimer;
    Timer_UnCheked ClawTentacleTimer;

    //Dark Glare phase
    uint32 DarkGlareTick;
    Timer_UnCheked DarkGlareTickTimer;
    ...
    void Reset()
    {
        ...
        m_creature->SetNonAttackableFlag(UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NON_ATTACKABLE, false);
    ...
    void JustDied(Unit * killer)
    {
        if (pInstance)
        {
            ...
            pInstance->SetData(DATA_CTHUN_PHASE, DONE);
        }
    }

    void SpawnEyeTentacle(float x, float y)
    {
        if (Creature* Spawned = (Creature*)m_creature->SummonCreature(MOB_EYE_TENTACLE, m_creature->GetPositionX() + x, m_creature->GetPositionY() + y, m_creature->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 500))
        {
        ...
        switch (pInstance->GetData(DATA_CTHUN_PHASE))
        {
            case 0:
            {
                ...
                if (PhaseTimer.Expired(diff))
                {
                    ...
                    if (target)
                    {
                        ...
                        //Face our target
                        DarkGlareAngle = m_creature->GetOrientationTo(target);
                    ...
                    PhaseTimer = 35000;
                    me->SetIgnoreVictimSelection(true); // cannot turn by itself, will be able by script
                }
            }
            break;
            case 1:
            {
                    ...
                    if (DarkGlareTickTimer.Expired(diff))
                {
                    ...
                    if (ClockWise)
                    {
                        m_creature->SetOrientation(DarkGlareAngle + ((float)DarkGlareTick*PI/35)); // changes real orientation on server, but doesn't change visually for client until next creature update(which will never happen cause we cant move/turn).
                        m_creature->SetFacingTo(DarkGlareAngle + ((float)DarkGlareTick*PI/35)); // doesnt change real orientation when in combat - cant use in combat. But gives client visual information about orientation
                    }
                    else 
                    {
    ...
    void DamageTaken(Unit *done_by, uint32 &damage)
    {
        ...
        switch (pInstance->GetData(DATA_CTHUN_PHASE))
        {
            ...
            case 1:
            {
                ...
                m_creature->HandleEmoteCommand(0);
                m_creature->SetNonAttackableFlag(UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NON_ATTACKABLE);
                ...
                m_creature->RemoveAllAurasNotCreatureAddon();
...
struct cthunAI : public Scripted_NoMovementAI
{
    ...
    Timer_UnCheked GiantClawTentacleTimer;
    Timer_UnCheked GiantEyeTentacleTimer;
    Timer_UnCheked StomachAcidTimer;
    Timer_UnCheked StomachEnterTimer;
    Timer_UnCheked StomachEnterVisTimer;
    uint64 StomachEnterTarget;

    //Stomach map, bool = true then in stomach
    std::set<uint64> Stomach_Set;
    ...
    void Reset()
    {
        ...
        // FIXME - Those values for other creatures, and this. and shit and fuck this i'm going home(
        m_creature->SetFloatValue(UNIT_FIELD_BOUNDINGRADIUS, 50);   //custom, should be verified
        m_creature->SetFloatValue(UNIT_FIELD_COMBATREACH, 20);
    ...
    void SpawnEyeTentacle(float x, float y)
    {
        if (Creature* Spawned = m_creature->SummonCreature(MOB_EYE_TENTACLE, m_creature->GetPositionX() + x, m_creature->GetPositionY() + y, m_creature->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 500))
        {
    ...
    Unit* SelectRandomNotStomach()
    {
        ...
        {
            if (Stomach_Set.find((*i)->getUnitGuid()) == Stomach_Set.end())
            {
        ...
        std::list<Unit*>::iterator j = temp.begin();
        ...
        switch (pInstance->GetData(DATA_CTHUN_PHASE))
        {
            ...
            case 2:
            {
                ...
                if (PhaseTimer.Expired(diff))
                {
                    ...
                    if (Spawned = (Creature*)m_creature->SummonCreature(MOB_FLESH_TENTACLE, TENTACLE_POS1_X, TENTACLE_POS1_Y, TENTACLE_POS1_Z, TENTACLE_POS1_O, TEMPSUMMON_CORPSE_DESPAWN, 0))
                        ((flesh_tentacleAI*)(Spawned->AI()))->SpawnedByCthun(m_creature->GetGUID());
                    else
                        ++FleshTentaclesKilled;

                    //Spawn flesh tentacle
                    if (Spawned = (Creature*)m_creature->SummonCreature(MOB_FLESH_TENTACLE, TENTACLE_POS2_X, TENTACLE_POS2_Y, TENTACLE_POS2_Z, TENTACLE_POS2_O, TEMPSUMMON_CORPSE_DESPAWN, 0))
                        ((flesh_tentacleAI*)(Spawned->AI()))->SpawnedByCthun(m_creature->GetGUID());
                    else
                        ++FleshTentaclesKilled;
            ...
            case 3:
            {
                ...
                if (FleshTentaclesKilled > 1)
                {
                    ...
                    std::set<uint64>::iterator next;

                    for (std::set<uint64>::iterator i = Stomach_Set.begin(); i != Stomach_Set.end(); i = next)
                    {
                        next = i;
                        ++next;

                        //Check for valid player
                        Unit* pUnit = Unit::GetUnit(*m_creature, (*i));
                ...
                if (StomachEnterTimer.Expired(diff))
                {
                    ...
                    if (target)
                    {
                        Stomach_Set.insert(target->GetGUID());
                    ...
                    if (pUnit)
                    {
                        DoCast(pUnit, SPELL_DIGESTIVE_ACID, true); // Player is in stomach! Do not cast/teleport to him in 200ms window
    ...
    void JustDied(Unit * killer)
    {
        if (pInstance)
        {
            if(pInstance->GetData(DATA_C_THUN) != DONE)
                pInstance->SetData(DATA_C_THUN, DONE);
            pInstance->SetData(DATA_CTHUN_PHASE, 5);
			if (Unit* eye = FindCreature(15589, 100, me))
				eye->setDeathState(JUST_DIED);
        }
    }
...
struct eye_tentacleAI : public Scripted_NoMovementAI
{
    ...
    Timer_UnCheked MindflayTimer;
    Timer_UnCheked KillSelfTimer;
    uint64 Portal;
    uint64 targetGUID;
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
            Unit* targ = Unit::GetUnit(*m_creature, targetGUID);
            if (targ && targ->HasAura(SPELL_DIGESTIVE_ACID, 0))
            {
...
struct claw_tentacleAI : public Scripted_NoMovementAI
{
    ...
    Timer_UnCheked GroundRuptureTimer;
    ...
    void UpdateAI(const uint32 diff)
    {
            ...
            if (EvadeTimer.Expired(diff))
        {
            ...
            if (!target->HasAura(SPELL_DIGESTIVE_ACID, 0))
            {
                ...
                HamstringTimer = 2000;
                EvadeTimer = 5000;
                DoResetThreat(); // so new target will be attacked
                AttackStart(target);
            }
...
void flesh_tentacleAI::JustDied(Unit* killer)
{
    if (!Parent)
    {
        DoYell(-1200443, LANG_UNIVERSAL, NULL);
        return;
    }
    ...
    else DoYell(-1200444, LANG_UNIVERSAL, NULL);
}
