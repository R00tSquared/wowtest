...
struct aqsentinelAI : public ScriptedAI
{
    ...
    void AddBuddyToList(uint64 guid)
    {
        if (guid == m_creature->GetGUID())
            return;

        for (int i=0; i<3; i++)
        {
            if (nearbyGUID[i] == guid)
                return;
    ...
    void SendMyListToBuddies()
    {
        for (int i = 0; i < 3; i++)
        {
            if (nearbyGUID[i])
            {
                if (Creature* buddy = me->GetCreature(nearbyGUID[i]))
                    GiveBuddyMyList(buddy);
            }
        }
    }
    ...
    void CallBuddiesToAttack(Unit *who)
    {
        for (int i=0; i<3; i++)
        {
            if (Creature* c = me->GetCreature(nearbyGUID[i]))
            {
                if (!c->IsInCombat())
                {
    ...
    void GetOtherSentinels(Unit *who)
    {
        ...
        for (uint32 i = 0; i < 3; i++)
        {
            ...
            Creature* c = me->GetCreature(nearbyGUID[i]);
            if (!c)
                continue;

            AddSentinelsNear(c);
            ((aqsentinelAI *)c->AI())->gatherOthersWhenAggro = false;
            ((aqsentinelAI *)c->AI())->selectAbility(pickAbilityRandom(chosenAbilities));
        }
        /*if (bli < 3)
    ...
    {
        const SpellEntry *spell = GetSpellStore()->LookupEntry<SpellEntry>(id);
        for (int i=0; i<3; i++)
        {
            if (!spell->Effect[i])
                continue;
    ...
    void JustDied(Unit *pKiller)
    {
        for (int ni=0; ni<3; ni++)
        {
            ...
            Creature* sent = me->GetCreature(nearbyGUID[ni]);
            if (!sent)
                continue;
        ...
        pKiller->SummonGameObject(181068, m_creature->GetPositionX(), m_creature->GetPositionY(),  m_creature->GetPositionZ(), 0, 0, 0, 0, 0, 0);
    }
