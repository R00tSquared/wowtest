...
bool GossipHello_npc_skorn_whitecloud(Player *player, Creature *_Creature)
{
    ...
    if (!player->GetQuestRewardStatus(770))
        player->ADD_GOSSIP_ITEM( 0, player->GetSession()->GetHellgroundString(GOSSIP_SW), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF );
    ...
    void MovementInform(uint32 type, uint32 id)
    {
        if(type == POINT_MOTION_TYPE)
        {
            ...
            case 1:
                ...
                if(plr)
                    m_creature->SetFacingTo(m_creature->GetOrientationTo(plr));
