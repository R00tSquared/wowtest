...
enum OnyxiaLair
{
    ...
    CREATURE_ONYXIA   = 10184
};

struct instance_onyxia_lair : public ScriptedInstance
{
    instance_onyxia_lair(Map *map) : ScriptedInstance(map), m_gbk(map) {Initialize();};
    ...
    void OnCreatureCreate(Creature *c, uint32 creature_entry)
    {
        switch (creature_entry)
        {
            case CREATURE_WHELP:
                ...
                break;
            case CREATURE_ONYXIA:
                OnyxiaGUID = c->GetGUID();
                break;
        }
    }
    ...
    void DespawnWhelps()
    {
        error_log("TSCR: Despawning Whelps in Onyxia's Lair: whelps count %llu", m_whelps.size());
    ...
    void SetData(uint32 index, uint32 data)
    {
        ...
        GBK_Encounters gbkEnc = EncounterForGBK(index);
        if (gbkEnc != GBK_NONE)
        {
            if (data == DONE)
    ...
    GBK_Encounters EncounterForGBK(uint32 enc)
    {
        switch (enc)
        {
        case DATA_ONYXIA: return GBK_ONYXIA;
        }
        return GBK_NONE;
    }
    ...
    void ActivateLavaTraps()
    {
        if (Unit *target = (instance->GetPlayers().begin() != instance->GetPlayers().end()) ? instance->GetPlayers().begin()->getSource() : NULL)
        {
            ...
            for (std::set<uint32>::const_iterator itr = randomized.begin(); itr != randomized.end(); ++itr)
            {
                ...
                {
                    obj->CastSpell(target, SPELL_ERUPT, OnyxiaGUID);
