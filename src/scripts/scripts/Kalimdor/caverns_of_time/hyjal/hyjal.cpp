...
#define GOSSIP_ITEM_BEGIN_ALLY      16286
#define GOSSIP_ITEM_ANETHERON       16287

#define GOSSIP_ITEM_BEGIN_HORDE     16288
#define GOSSIP_ITEM_AZGALOR         16289

#define GOSSIP_ITEM_RETREAT         16290

#define GOSSIP_ITEM_TYRANDE         16291
...
CreatureAI* GetAI_npc_jaina_proudmoore(Creature *_Creature)
{
    ...
    ai->Spell[0].CooldownStart = 8000;
    ai->Spell[0].CooldownMin = 15000;
    ai->Spell[0].CooldownMax = 45000;
    ai->Spell[0].TargetType = TARGETTYPE_SELF;

    ai->Spell[1].SpellId = SPELL_BLIZZARD;
    ai->Spell[1].CooldownStart = 6000;
...
bool GossipHello_npc_jaina_proudmoore(Player *player, Creature *_Creature)
{
    ...
    if(RageEncounter == NOT_STARTED)
        player->ADD_GOSSIP_ITEM( 0, player->GetSession()->GetHellgroundString(GOSSIP_ITEM_BEGIN_ALLY), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
    else if(RageEncounter == DONE && AnetheronEncounter == NOT_STARTED)
        player->ADD_GOSSIP_ITEM( 0, player->GetSession()->GetHellgroundString(GOSSIP_ITEM_ANETHERON), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
    else if(RageEncounter == DONE && AnetheronEncounter == DONE)
        player->ADD_GOSSIP_ITEM( 0, player->GetSession()->GetHellgroundString(GOSSIP_ITEM_RETREAT), GOSSIP_SENDER_MAIN,    GOSSIP_ACTION_INFO_DEF + 3);

    if(player->isGameMaster())
        player->ADD_GOSSIP_ITEM(2, player->GetSession()->GetHellgroundString(16292), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
...
bool GossipHello_npc_thrall(Player *player, Creature *_Creature)
{
    ...
    if (AnetheronEvent == DONE && ai->GetInstanceData(DATA_ALLIANCE_RETREAT))
    {
        ...
        if(KazrogalEvent == NOT_STARTED)
            player->ADD_GOSSIP_ITEM( 0, player->GetSession()->GetHellgroundString(GOSSIP_ITEM_BEGIN_HORDE), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
        else if(KazrogalEvent == DONE && AzgalorEvent != DONE && AzgalorEvent != IN_PROGRESS)
            player->ADD_GOSSIP_ITEM( 0, player->GetSession()->GetHellgroundString(GOSSIP_ITEM_AZGALOR), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
        else if(AzgalorEvent == DONE)
            player->ADD_GOSSIP_ITEM( 0, player->GetSession()->GetHellgroundString(GOSSIP_ITEM_RETREAT), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3);
    }

    if(player->isGameMaster())
        player->ADD_GOSSIP_ITEM(2, player->GetSession()->GetHellgroundString(16293), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
...
bool GossipHello_npc_tyrande_whisperwind(Player* player, Creature* _Creature)
{
    ...
    // ai->GetInstanceData(DATA_OPEN_PATH) == DONE
    if ((ai->GetInstanceData(DATA_AZGALOREVENT) == DONE) && !player->HasItemCount(ITEM_TEAR_OF_GODDESS, 1))
        player->ADD_GOSSIP_ITEM(0, player->GetSession()->GetHellgroundString(GOSSIP_ITEM_TYRANDE), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
    player->SEND_GOSSIP_MENU(907, _Creature->GetGUID());
    return true;
}
