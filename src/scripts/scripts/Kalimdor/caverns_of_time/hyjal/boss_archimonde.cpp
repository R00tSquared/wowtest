...
struct mob_doomfire_targettingAI : public NullCreatureAI
{
    ...
    Timer_UnCheked ChangeTargetTimer;
    Timer_UnCheked SummonTimer;     // This timer will serve to check on Archionde
    ...
    uint32 ChangeWaypointAndTimer()
    {
        if(pInstance)
        {
            ...
            else
            {
                ...
                {
                    float angle = Archi->GetOrientationTo(me);
    ...
    */
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (ChangeTargetTimer.Expired(diff))
        {
            ...
            if (pArchimonde && pArchimonde->isAlive())
            {
            float angle = me->GetOrientationTo(pArchimonde);
            Position dest;
            me->GetPosition(dest);
...
struct boss_archimondeAI : public hyjal_trashAI
{
    ...
    Timer_UnCheked DrainNordrassilTimer;
    Timer_UnCheked FearTimer;
    Timer_UnCheked AirBurstTimer;
    Timer_UnCheked GripOfTheLegionTimer;
    Timer_UnCheked DoomfireTimer;
    Timer_UnCheked SoulChargeTimer;
    uint32 SoulChargeCount;
    Timer_UnCheked SoulChargeUnleashTimer;
    Timer_UnCheked MeleeRangeCheckTimer;
    Timer_UnCheked HandOfDeathTimer;
    Timer_UnCheked SummonWispTimer;
    int32 WispCount;
    Timer_UnCheked EnrageTimer;
    Timer_UnCheked CheckDistanceTimer;
    Timer_UnCheked CheckTimer;
    ...
    void UpdateAI(const uint32 diff)
    {
        if (pInstance)
        {
            // I don't want to fuck my mind with these if else
            //if (pInstance->GetData(DATA_OPEN_PATH) == DONE)
            //{
            //    if (me->GetVisibility() != VISIBILITY_ON || me->getFaction() == 35)
            //    {
            //        me->setFaction(1720);
