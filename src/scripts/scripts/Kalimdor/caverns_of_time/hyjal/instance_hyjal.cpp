...
struct instance_mount_hyjal : public ScriptedInstance
{
    ...
    void OnObjectCreate(GameObject *go)
    {
        switch(go->GetEntry())
        {
            ...
            case 182061:
                ElfGate = go->GetGUID();
                if (hordeRetreat || Encounters[5] == DONE)
                    go->SetGoState(GO_STATE_ACTIVE);
                else
                    go->SetGoState(GO_STATE_READY);
                break;
    ...
    uint32 GetData(uint32 type)
    {
        switch(type)
        {
            ...
            //case DATA_OPEN_PATH:           return Encounters[5];
        }
        return 0;
    }
    ...
    void Load(const char* in)
    {
        ...
        loadStream.str(in);
        loadStream >> Encounters[0] >> Encounters[1] >> Encounters[2] >> Encounters[3] >> Encounters[4] >> Encounters[5] >> allianceRetreat >> hordeRetreat >> RaidDamage;
        for(uint8 i = 0; i < ENCOUNTERS; ++i)
