...
void hyjalAI::Retreat()
{
    if(pInstance)
    {
        if(Faction == 0)
        {
            pInstance->SetData(DATA_ALLIANCE_RETREAT, DONE);
        ...
        if(Faction == 1)
        {
            pInstance->SetData(DATA_HORDE_RETREAT, DONE);
        ...
        if(!creatures.empty())
        {
            for(std::list<Creature*>::iterator itr = creatures.begin(); itr != creatures.end(); ++itr)
            {
                if((*itr) && (*itr)->isAlive() && (*itr) != m_creature && (*itr)->GetEntry() != JAINA)
                {
                    ...
                    m_creature->GetNearPoint(x, y, z, range, 0, m_creature->GetOrientationTo((*itr)));
            ...
            if(!creatures.empty())
            {
                for(std::list<Creature*>::iterator itr = creatures.begin(); itr != creatures.end(); ++itr)
                {
                    if((*itr) && (*itr)->isAlive())
                    {
                        ...
                        (*itr)->setFaction(35);//make them friendly so mobs won't attack them
                        (*itr)->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE);
                    }
                }
