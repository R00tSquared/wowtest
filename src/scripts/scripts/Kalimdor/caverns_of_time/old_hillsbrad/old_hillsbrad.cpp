...
{
    player->ADD_GOSSIP_ITEM(0, player->GetSession()->GetHellgroundString(16294), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
...
bool GossipHello_npc_erozion(Player *player, Creature *creature)
{
    ...
    if ( pInstance && pInstance->GetData(TYPE_BARREL_DIVERSION) != DONE && !player->HasItemCount(ITEM_ENTRY_BOMBS, 1))
        player->ADD_GOSSIP_ITEM( 0, player->GetSession()->GetHellgroundString(16295), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
...
struct npc_thrall_old_hillsbradAI : public npc_escortAI
{
    ...
    Timer_UnCheked StrikeTimer;
    Timer_UnCheked ShieldBlockTimer;
    Timer_UnCheked EmoteTimer;
        ...
        if (!players.isEmpty() && pMap->IsDungeon())
        {
            for(Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
            {
                if (Player* player = itr->getSource())
                    player->KilledMonster(NPC_THRALL_QUEST_TRIGGER,me->GetGUID());
            }
        }
    }
...
bool GossipHello_npc_thrall_old_hillsbrad(Player *player, Creature *creature)
{
    ...
    if (pInstance)
    {
        ...
        {
            player->ADD_GOSSIP_ITEM(0, player->GetSession()->GetHellgroundString(GOSSIP_ITEM_SKARLOC1), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);
        ...
        {
            player->ADD_GOSSIP_ITEM(0, player->GetSession()->GetHellgroundString(GOSSIP_ITEM_TARREN), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+3);
...
bool GossipSelect_npc_thrall_old_hillsbrad(Player *player, Creature *creature, uint32 sender, uint32 action)
{
    ...
    switch (action)
    {
        ...
        case GOSSIP_ACTION_INFO_DEF+2:
            player->ADD_GOSSIP_ITEM(0, player->GetSession()->GetHellgroundString(GOSSIP_ITEM_SKARLOC2), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+20);
            player->SEND_GOSSIP_MENU(GOSSIP_ID_SKARLOC2, creature->GetGUID());
            break;

        case GOSSIP_ACTION_INFO_DEF+20:
            player->ADD_GOSSIP_ITEM(0, player->GetSession()->GetHellgroundString(GOSSIP_ITEM_SKARLOC3), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+22);
...
bool GossipHello_npc_taretha(Player *player, Creature *creature)
{
    ...
    {
        player->ADD_GOSSIP_ITEM(0, player->GetSession()->GetHellgroundString(GOSSIP_ITEM_EPOCH1), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
...
bool GossipSelect_npc_taretha(Player *player, Creature *creature, uint32 sender, uint32 action)
{
    ...
    if (action == GOSSIP_ACTION_INFO_DEF+1)
    {
        player->ADD_GOSSIP_ITEM(0, player->GetSession()->GetHellgroundString(GOSSIP_ITEM_EPOCH2), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);
