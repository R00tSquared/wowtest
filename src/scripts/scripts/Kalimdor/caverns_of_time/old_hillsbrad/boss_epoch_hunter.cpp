...
struct boss_epoch_hunterAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        if(pInstance)
            pInstance->SetData(DATA_EPOCH_HUNTER, NOT_STARTED);
    }
    ...
    void EnterCombat(Unit *who)
    {
        ...
        if(pInstance)
            pInstance->SetData(DATA_EPOCH_HUNTER, IN_PROGRESS);
    }
    ...
    void JustDied(Unit *victim)
    {
        ...
        if (pInstance->GetData(TYPE_THRALL_EVENT) == IN_PROGRESS)
        {
            ...
            if(pInstance)
                pInstance->SetData(DATA_EPOCH_HUNTER, DONE);
        }
        ...
        if (!attackers.empty())
        {
            for (std::list<uint64>::iterator itr = attackers.begin(); itr != attackers.end(); ++itr)
            {
                if (Creature* attacker = tmpMap->GetCreature((*itr)))
                {
                    ...
                    attacker->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE, false);
