            ...
            switch (EventStage)
            {
                ...
                case 4:
                    if (Creature* Xiraxis = (Unit::GetCreature(*me, XiraxisGUID)))
                    {
                        ...
                        Xiraxis->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE, false);
...
#define GOSSIP_ETCP_HELLO   183877

bool GOUse_go_transportercp(Player* pPlayer, GameObject* pGO)
{
    ...
    if (pInstance->GetData(DATA_SHAHEENEVENT) != DONE)
        pPlayer->ADD_GOSSIP_ITEM(0, pPlayer->GetSession()->GetHellgroundString(16405), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);

    pPlayer->SEND_GOSSIP_MENU(GOSSIP_ETCP_HELLO, pGO->GetGUID());
    
    return false;
}

bool GOGossipSelect_go_transportercp(Player* pPlayer, GameObject* pGO, uint32 Sender, uint32 action)
{
    switch (action)
    {
        case GOSSIP_ACTION_INFO_DEF + 1:
            ScriptedInstance* pInstance = (ScriptedInstance*)pGO->GetInstanceData();
            if (pInstance && pInstance->GetData(DATA_SHAHEENEVENT) != DONE)
            {
                pInstance->SetData(DATA_SHAHEENEVENT, DONE);
                if (Creature* shaheen = pGO->SummonCreature(NPC_SHAHEEN, -351.21f, -69.19f, -0.962f, 3.414f, TEMPSUMMON_CORPSE_DESPAWN, 10000))
                {
                    DoScriptText(EMOTE_CHECK_BODY, shaheen);
                    DoScriptText(SAY_INTRO, shaheen, pPlayer);
                    if (npc_shaheenAI* escortAI = dynamic_cast<npc_shaheenAI*>(shaheen->AI()))
                        escortAI->StartEscortPartOne(pPlayer);
                }
            }
...
void AddSC_mana_tombs()
{
    ...
    newscript->pGossipSelectGO = &GOGossipSelect_go_transportercp;
    newscript->RegisterSelf();
}