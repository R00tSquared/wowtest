...
struct advisorbase_ai : public ScriptedAI
{
    ...
    void Revive()
    {
        ...
        {
            me->AttackStop(); // gotta call stop first, so they start needed movement
            AttackStart(target);
        }
        CanDie = true;
    ...
    void UpdateMaxHealth(bool twice)
    {
        if(m_creature->GetGUID() == pInstance->GetData64(DATA_LORDSANGUINAR))
            m_creature->SetMaxHealth((twice ? HP_SANGUINAR*2 : HP_SANGUINAR) * me->GetCreatureHealthMod());

        if(m_creature->GetGUID() == pInstance->GetData64(DATA_GRANDASTROMANCERCAPERNIAN))
            m_creature->SetMaxHealth((twice ? HP_CAPERNIAN*2 : HP_CAPERNIAN) * me->GetCreatureHealthMod());

        if(m_creature->GetGUID() == pInstance->GetData64(DATA_MASTERENGINEERTELONICUS))
            m_creature->SetMaxHealth((twice ? HP_TELONICUS*2 : HP_TELONICUS) * me->GetCreatureHealthMod());

        if (m_creature->GetGUID() == pInstance->GetData64(DATA_THALADREDTHEDARKENER))
            m_creature->SetMaxHealth((twice ? HP_THALADRED*2 : HP_THALADRED) * me->GetCreatureHealthMod());
    }
...
struct boss_kaelthasAI : public ScriptedAI
{
    ...
    void DeleteLegs()
    {
        ...
        for(Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
        {
            ...
            i_pl->DestroyItemCount(30312, 1, true, false);
            i_pl->DestroyItemCount(30311, 1, true, false);
            i_pl->DestroyItemCount(30317, 1, true, false);
            i_pl->DestroyItemCount(30316, 1, true, false);
            i_pl->DestroyItemCount(30313, 1, true, false);
            i_pl->DestroyItemCount(30314, 1, true, false);
            i_pl->DestroyItemCount(30318, 1, true, false);
            i_pl->DestroyItemCount(30319, 1, true, false);
            i_pl->DestroyItemCount(30320, 1, true, false);
        }
    }
    ...
    void Reset()
    {
        ...
        m_creature->RemoveAllAurasNotCreatureAddon(); //if Reset called while animation
    ...
    bool StartEvent()
    {
        ...
        PhaseSubphase = 0;
        Phase_Timer = sWorld.isEasyRealm() && !sGameEventMgr.IsActiveEvent(1053) ? 10000 : 23000;
        Phase = 1;
        DoZoneInCombat();
        ...
        switch (Phase)
        {
                ...
                switch(PhaseSubphase)
                {
                    ...
                    case 1:
                        if(Phase_Timer <= diff)
                        {
                            ...
                            if(Advisor)
                            {
                                Advisor->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE, false);
            ...
            case 2:
            {
                ...
                if (PhaseSubphase == 1)
                {
                    ...
                    PhaseSubphase = 2;
                    Phase_Timer = sWorld.isEasyRealm() && !sGameEventMgr.IsActiveEvent(1053) ? 60000 : TIME_PHASE_2_3;
                }
            ...
            case 3:
            {
                if (PhaseSubphase == 0)
                {
                    ...
                    PhaseSubphase = 1;
                    Phase_Timer = sWorld.isEasyRealm() && !sGameEventMgr.IsActiveEvent(1053) ? 90000 : TIME_PHASE_3_4;
                }
                ...
                if(Phase_Timer <= diff)
                {
                    ...
                    Phase_Timer = sWorld.isEasyRealm() && !sGameEventMgr.IsActiveEvent(1053) ? 15000 : 30000;
                }
                else
                    Phase_Timer -= diff;
            }
...
struct boss_thaladred_the_darkenerAI : public advisorbase_ai
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
            if(t->IsImmunedToDamage(SPELL_SCHOOL_MASK_NORMAL))
            {
                if(Unit* target = SelectUnit(SELECT_TARGET_TOPAGGRO, 1))
                {
                    m_creature->AddThreat(target, 10001.0f);
                    AttackStart(target);
                }
            }
        }
        ...
        if(Check_Timer <= diff)
        {
           if(m_creature->getThreatManager().getThreat(m_creature->GetVictim(),false) < 10000.0f)
               m_creature->AddThreat(m_creature->GetVictim(), 10001.0f);
...
struct mob_nether_vaporAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        if(Move_Timer <= diff)
        {
            ...
            m_creature->MonsterMoveWithSpeed(newX, newY, newZ, 5000,true);
            Move_Timer = 3000;
        }
        else
            Move_Timer -= diff;
