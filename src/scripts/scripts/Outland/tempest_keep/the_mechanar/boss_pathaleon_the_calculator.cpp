...
struct boss_pathaleon_the_calculatorAI : public ScriptedAI
{
    ...
    SummonList summons;
    Timer_UnCheked ManaTap_Timer;
    Timer_UnCheked ArcaneTorrent_Timer;
    Timer_UnCheked Domination_Timer;
    Timer_UnCheked ArcaneExplosion_Timer;
    ...
    void Reset()
    {
        ...
        if(pInstance)
            pInstance->SetData(DATA_PATHAELON, NOT_STARTED);
    }
    void EnterCombat(Unit *who)
    {
        DoScriptText(SAY_AGGRO, me);
        if(pInstance)
            pInstance->SetData(DATA_PATHAELON, IN_PROGRESS);
    }
    ...
    void JustDied(Unit* Killer)
    {
        ...
        summons.DespawnAll();
        if(pInstance)
            pInstance->SetData(DATA_PATHAELON, DONE);
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (!Enraged && me->GetHealthPercent() < 21)
        {
...
struct mob_nether_wraithAI : public ScriptedAI
{
    ...
    Timer_UnCheked ArcaneMissiles_Timer;
    Timer_UnCheked Detonation_Timer;
    Timer_UnCheked Die_Timer;
    bool Detonation;
