...
{
    instance_mechanar(Map *map) : ScriptedInstance(map), m_gbk(map) {Initialize();};
    ...
    bool CleanupCharges;
    Timer_UnCheked BridgeEventTimer;
    ...
    uint32 GetData(uint32 type)
    {
        ...
        case DATA_PATHAELON:                return Encounters[6];
        }
    ...
    GBK_Encounters EncounterForGBK(uint32 enc)
    {
        switch (enc)
        {
            case DATA_NETHERMANCER_EVENT:   return GBK_MECH_NETHERMANCER;
            case DATA_IRONHAND_EVENT:       return GBK_MECH_IRONHAND;
            case DATA_GYROKILL_EVENT:       return GBK_MECH_GYROKILL;
            case DATA_MECHANO_LORD_EVENT:   return GBK_MECH_MECHANOLORD;
            case DATA_PATHAELON:            return GBK_MECH_PATHAELON;
        }
        return GBK_NONE;
    }
    ...
    void Load(const char* in)
    {
        ...
        stream >> Encounters[0] >> Encounters[1] >>  Encounters[2] >>  Encounters[3] >>  Encounters[4] >> Encounters[5] >> Encounters[6];
