...
struct npc_millhouse_manastormAI : public ScriptedAI
{
    ...
    Timer_UnCheked EventProgress_Timer;
    uint32 Phase;
    bool Init;
    bool LowHp;

    Timer_UnCheked Pyroblast_Timer;
    Timer_UnCheked Fireball_Timer;

    void Reset()
    {
        EventProgress_Timer.Reset(5000);
        LowHp = false;
        Init = false;
        Phase = 1;
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if(!LowHp && ((me->GetHealthPercent()) < 20))
        {
...
struct npc_warden_mellicharAI : public ScriptedAI
{
    ...
    bool CanProgress()
    {
        if(pInstance)
        {
            ...
            {
                EventProgress_Timer.Reset(10000);
                return true;
            }
            return false;
        }
            ...
            if(CanProgress())
            {
                switch(Phase)
                {
                    ...
                    case 2:
                        ...
                        DoPrepareForPhase();
                        EventProgress_Timer.Reset(7000);
                        break;
                    case 3:
                    ...
                    case 4:
                        DoPrepareForPhase();
                        EventProgress_Timer.Reset(15000);
                        break;
                    case 5:
...
struct npc_arcatraz_sentinelAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
            if(Unit* pKiller = me->GetVictim())
                pKiller->Kill(me, false);
            else
                me->Kill(me, false);
        }
...
struct npc_negaton_screamerAI : public ScriptedAI
{
    ...
    Timer_UnCheked fearTimer;
    Timer_UnCheked volleyTimer;
    SpellSchools school;
    ...
    {
        if (school != SPELL_SCHOOL_NORMAL || spell->Id == SPELL_PSYCHIC_SCREAM)
        {
            me->RemoveSpellsCausingAura(SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN);
            school = SPELL_SCHOOL_NORMAL;
...
enum ArcatrazWarder
{
    ...
    SPELL_ARCANE_SHOT               = 38807,
    SPELL_CHARGED_ARCANE_SHOT       = 38808,
    SPELL_SHOOT_N                   = 15620,
    SPELL_SHOOT_H                   = 22907
    
};

struct npc_arcatraz_warderAI : public ScriptedAI
{
    npc_arcatraz_warderAI(Creature *c) : ScriptedAI(c)
    {
        pInstance = c->GetInstanceData();
        HeroicMode = me->GetMap()->IsHeroic();
    }

    Timer Timer_WingClip;
    Timer Timer_ShootArrow;
    Timer Timer_RangeCheck;
    ...
    void Reset()
    {
        Timer_WingClip.Reset(1000);
        Timer_ShootArrow.Reset(1000);
        Timer_RangeCheck.Reset(1000);
        me->SetRooted(true);
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
            if(Timer_WingClip.Expired(diff))
            {
                AddSpellToCast(SPELL_WING_CLIP, CAST_TANK);
                Timer_WingClip = 15000;
            }
            me->SetRooted(false);
        }

        if(Timer_RangeCheck.Expired(diff))
        {
            ...
            Timer_RangeCheck = 2000;
        }

        if(Timer_ShootArrow.Expired(diff))
        {
            AddSpellToCast(HeroicMode ? SPELL_CHARGED_ARCANE_SHOT : SPELL_ARCANE_SHOT, CAST_TANK);
            Timer_ShootArrow = 2000;
        }
...
CreatureAI* GetAI_npc_arcatraz_warderAI(Creature *_Creature)
{
    return new npc_arcatraz_warderAI(_Creature);
}

enum ArcatrazDefender
{
    SPELL_IMMOLATE_H        = 38806,
    SPELL_IMMOLATE_N        = 36638,
    SPELL_INFECTED_BLOOD    = 38812
};

struct npc_arcatraz_defenderAI : public ScriptedAI
{
    npc_arcatraz_defenderAI(Creature *c) : ScriptedAI(c)
    {
        pInstance = c->GetInstanceData();
        HeroicMode = me->GetMap()->IsHeroic();
    }

    Timer Timer_Immolate;
    Timer Timer_InfectedBlood;
    ...
    void Reset()
    {
        Timer_Immolate.Reset(1000);
        Timer_InfectedBlood.Reset(3000);
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if(Timer_Immolate.Expired(diff))
        {
            AddSpellToCast(HeroicMode ? SPELL_IMMOLATE_H : SPELL_IMMOLATE_N, CAST_RANDOM);
            Timer_Immolate = urand(15000, 20000);
        }
        
        if(Timer_InfectedBlood.Expired(diff))
        {
            AddSpellToCast(SPELL_INFECTED_BLOOD, CAST_RANDOM);
            Timer_InfectedBlood = urand(12000, 14000);
        }
...
CreatureAI* GetAI_npc_arcatraz_defenderAI(Creature *_Creature)
{
    return new npc_arcatraz_defenderAI(_Creature);
}
...
void AddSC_arcatraz()
{
    ...
    newscript = new Script;
    newscript->Name = "npc_arcatraz_warder";
    newscript->GetAI = &GetAI_npc_arcatraz_warderAI;
    newscript->RegisterSelf();

    newscript = new Script;
    newscript->Name = "npc_arcatraz_defender";
    newscript->GetAI = &GetAI_npc_arcatraz_defenderAI;
    newscript->RegisterSelf();
}
