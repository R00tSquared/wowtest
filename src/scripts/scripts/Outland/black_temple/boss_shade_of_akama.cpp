...
struct mob_ashtongue_defenderAI : public ScriptedAI
{
    ...
    Timer_UnCheked m_debilStrikeTimer;
    Timer_UnCheked m_shieldBashTimer;
...
struct mob_ashtongue_spiritbinderAI : public ScriptedAI
{
    ...
    Timer_UnCheked m_chainHealTimer;
    Timer_UnCheked m_spiritHealTimer;
    Timer_UnCheked m_spiritMendTimer;
    Timer_UnCheked m_checkTimer;
...
struct mob_ashtongue_elementalistAI : public ScriptedAI
{
    ...
    Timer_UnCheked m_rainofFireTimer;
    Timer_UnCheked m_lightningBoltTimer;
    Timer_UnCheked m_checkTimer;
...
struct mob_ashtongue_rogueAI : public ScriptedAI
{
    ...
    Timer_UnCheked m_debilPoisonTimer;
    Timer_UnCheked m_eviscerateTimer;
    Timer_UnCheked m_checkTimer;
...
struct boss_shade_of_akamaAI : public ScriptedAI
{
    ...
    Timer_UnCheked m_damageTimer;
    
    Timer_UnCheked m_waveTimer;
    Timer_UnCheked m_guardTimer;
    Timer_UnCheked m_sorcTimer;
    Timer_UnCheked m_checkTimer;
        ...
        if (m_channelers.empty())
        {
            ...
            for (int i = 0; i < CHANNELERS_COUNT; ++i)
            {
                ...
                if (channeler)
                {
                    ...
                    channeler->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE);
                }
            }
        }
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        else
        {
            if (event_phase == START_EVENT)
            {
                ...
                {
                    if (Creature *channeler = me->GetCreature(*me, *itr))
                        channeler->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE, false);
                }
...
struct npc_akamaAI : public Scripted_NoMovementAI
{
    ...
    Timer_UnCheked m_destructiveTimer;
    Timer_UnCheked m_lightningBoltTimer;
    
    uint32 m_talk;
    Timer_UnCheked m_talkTimer;
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (!m_yell && (me->GetHealthPercent()) < 15)
        {
...
bool GossipHello_npc_akama(Player *player, Creature *_Creature)
{
    if (ScriptedInstance *instance = (ScriptedInstance *)_Creature->GetInstanceData())
    {
        if (instance->GetData(EVENT_SUPREMUS) != DONE)
            return true;
