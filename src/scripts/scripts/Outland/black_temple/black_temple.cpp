...
#define GOSSIP_ASHTONGUE        16407
#define GOSSIP_ILLIDARI         16408
...
bool GossipHello_npc_spirit_of_olum(Player* player, Creature* _Creature)
{
    ...
    if(pInstance)
    {
        ...
        {
            player->ADD_GOSSIP_ITEM(0, player->GetSession()->GetHellgroundString(GOSSIP_ASHTONGUE), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);

            if (pInstance->GetData(EVENT_ILLIDARICOUNCIL) >= DONE)
                player->ADD_GOSSIP_ITEM(0, player->GetSession()->GetHellgroundString(GOSSIP_ILLIDARI), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
