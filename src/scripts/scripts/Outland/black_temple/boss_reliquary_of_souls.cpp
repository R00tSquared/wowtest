...
struct boss_reliquary_of_soulsAI : public Scripted_NoMovementAI
{
    ...
    {
        if (pInstance->GetData(EVENT_GURTOGGBLOODBOIL) != DONE) // on heroic do not start until gurtogg is defeated
            return;
        ...
        if (_Timer.Expired(diff))
        {
            switch (Counter)
            {
                ...
                case 4:
                {
                    ...
                    _Timer = 1000;
                    if (Essence->IsWithinDistInMap(me, 11))
                        me->RemoveAurasDueToSpell(SPELL_SUBMERGE);
                    else
                    {
                        Essence->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_ROTATE);
                        Essence->GetMotionMaster()->MovePoint(100, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), true, true, UNIT_ACTION_CONTROLLED);
                        return;
                    }
                    break;
                }
...
struct npc_ros_triggerAI : public ScriptedAI
{
    ...
    void MoveInLineOfSight(Unit * who)
    {
        ...
        if (pInstance->GetData(EVENT_RELIQUARYOFSOULS) == NOT_STARTED && who->GetTypeId() == TYPEID_PLAYER)
        {
...
struct boss_essence_of_sufferingAI : public ScriptedAI
{
    ...
    Timer AggroYellTimer;
    Timer FixateTimer;
    Timer EnrageTimer;
    Timer SoulDrainTimer;
    Timer AuraTimer;
    Timer checkTimer;
    ...
    void KilledUnit(Unit *victim)
    {
        ...
        DoScriptText(RAND(SUFF_SAY_SLAY1, SUFF_SAY_SLAY2), me);
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (EnrageTimer.Expired(diff))
        {
            ...
            DoScriptText(SUFF_SAY_SLAY3, me);
            EnrageTimer = 44000 + rand() % 3000;
        }
        
        if (SoulDrainTimer.Expired(diff))
        {
...
struct boss_essence_of_desireAI : public ScriptedAI
{
    ...
    Timer RuneShieldTimer;
    Timer DeadenTimer;
    Timer SoulShockTimer;
    Timer checkTimer;
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (RuneShieldTimer.Expired(diff))
        {
        ...
        if (SoulShockTimer.Expired(diff))
        {
...
struct boss_essence_of_angerAI : public ScriptedAI
{
    ...
    Timer CheckTankTimer;
    Timer SoulScreamTimer;
    Timer SpiteTimer;
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (CheckTankTimer.Expired(diff))
        {
        ...
        if (SoulScreamTimer.Expired(diff))
        {
