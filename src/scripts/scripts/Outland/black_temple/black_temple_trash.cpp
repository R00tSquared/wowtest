...
{
    mob_aqueous_lordAI(Creature *c) : ScriptedAI(c), summons(c) { me->SetAggroRange(AGGRO_RANGE); }
...
struct mob_coilskar_harpoonerAI : public ScriptedAI
{
    ...
    Timer_UnCheked SpearThrow;
    Timer_UnCheked HookedNet;
    Timer_UnCheked HarpoonersMark;
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
            if (Player* target = me->GetPlayerInWorld(MarkTargetGUID))
            {
                if (!target->HasAura(40084, 0))
...
struct mob_coilskar_seacallerAI : public ScriptedAI
{
    ...
    Timer_UnCheked ForkedLightning;
    Timer_UnCheked Hurricane;
    Timer_UnCheked SummonGeyser;
...
struct mob_coilskar_soothsayerAI : public ScriptedAI
{
    ...
    Timer_UnCheked HolyNova;
    Timer_UnCheked Restoration;
...
struct mob_coilskar_wranglerAI : public ScriptedAI
{
    ...
    Timer_UnCheked ElectricSpur;
    Timer_UnCheked LightningProd;
...
struct mob_dragon_turtleAI : public ScriptedAI
{
    ...
    bool CanBeShielded;
    Timer_UnCheked ShellShield;
    Timer_UnCheked WaterSpit;
...
struct mob_leviathanAI : public ScriptedAI
{
    ...
    Timer_UnCheked DebilitatingSpray;
    Timer_UnCheked PoisonSpit;
    Timer_UnCheked TailSweep;
...
struct mob_bonechewer_taskmasterAI : public ScriptedAI
{
    ...
    Timer_UnCheked disgruntledTimer;
...
struct mob_dragonmaw_wyrmcallerAI : public ScriptedAI
{
    ...
    Timer_UnCheked fixateTimer;
    Timer_UnCheked jabTimer;
...
struct mob_illidari_fearbringerAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (stompTimer.Expired(diff))
        {
            ...
            stompTimer = 20000 + urand(0, 10000);
        }
...
struct mob_ashtongue_battlelordAI : public ScriptedAI
{
    ...
    Timer_UnCheked Cleave;
    Timer_UnCheked ConcussionBlow;
    Timer_UnCheked ConcussionThrow;
    Timer_UnCheked Frenzy;
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (ConcussionThrow.Expired(diff))
        {
            ...
            {
                AddSpellToCast(target, SPELL_CONCUSSION_THROW, true);
            }
...
struct mob_ashtongue_feral_spiritAI : public ScriptedAI
{
    ...
    Timer_UnCheked ChargeRage;
    Timer_UnCheked SpiritBond;
    ...
    void Reset()
    {
        ...
        switch (me->GetEntry())
        {
            case NPC_CYCLONE_TOTEM:
                me->SetMaxHealth((urand(4100,4200)) * me->GetCreatureHealthMod());
                SpellTimer.Reset(urand(2000, 11000));
                break;
            case NPC_ASHTONGUE_SEARING_TOTEM:
                me->SetMaxHealth((urand(4800,4900)) * me->GetCreatureHealthMod());
                SpellTimer.Reset(1000);
                break;
            case NPC_SUMMONED_WINDFURY_TOTEM:
                me->SetMaxHealth((urand(1800,1900)) * me->GetCreatureHealthMod());
...
struct mob_ashtongue_mysticAI : public ScriptedAI
{
    ...
    Timer_UnCheked FrostShock;
    Timer_UnCheked FlameShock;
    Timer_UnCheked ChainHeal;
    Timer_UnCheked SearingTotem;
    Timer_UnCheked WindfuryTotem;
    Timer_UnCheked CycloneTotem;
...
struct mob_ashtongue_primalistAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
            Unit* target = SelectUnit(SELECT_TARGET_RANDOM, 0, 100.0f, true, WyvernStingTarget ? WyvernStingTarget : 0);
        ...
        {
            Unit* target = SelectUnit(SELECT_TARGET_RANDOM, 0, 30.0f, true, WyvernStingTarget ? WyvernStingTarget : 0);
            if (target && !me->IsWithinDist(target, 5.0f))
        ...
        if (WyvernSting.Expired(diff))
        {
            ...
            if (target && !me->IsWithinDist(target, 5.0f))
            {
                ...
                WyvernSting = 15000;
                WyvernStingTarget = target->GetGUID();
            }
            else
                WyvernSting = 2000;
        }
...
struct mob_ashtongue_stalkerAI : public ScriptedAI
{
    ...
    Timer_UnCheked Blind;
    Timer_UnCheked InstantPoison;
    Timer_UnCheked MindNumbingPoison;
...
struct mob_ashtongue_stormcallerAI : public ScriptedAI
{
    ...
    Timer_UnCheked ChainLightning;
    Timer_UnCheked LightningBolt;
    Timer_UnCheked LightningShield;
...
struct mob_illidari_boneslicerAI : public ScriptedAI
{
    ...
    Timer_UnCheked CloakOfShadows;
    Timer_UnCheked Gouge;
    Timer_UnCheked Shadowstep;
    Timer_UnCheked WoundPoison;
...
struct mob_illidari_defilerAI : public ScriptedAI
{
    ...
    Timer_UnCheked FelImmolate;
    Timer_UnCheked CurseOfAgony;
    Timer_UnCheked Banish;
    Timer_UnCheked RainOfChaos;
...
struct mob_illidari_heartseekerAI : public ScriptedAI
{
    ...
    Timer_UnCheked RapidShot;
    Timer_UnCheked Shoot;
    Timer_UnCheked SkeletonShot;
    Timer_UnCheked Curse;
...
struct mob_illidari_nightlordAI : public ScriptedAI
{
    ...
    Timer_UnCheked CurseOfMending;
    Timer_UnCheked Fear;
    Timer_UnCheked ShadowInferno;
    Timer_UnCheked Shadowfiends;
...
struct mob_shadowmoon_blood_mageAI : public ScriptedAI
{
    ...
    Timer_UnCheked BloodSiphon;
    Timer_UnCheked Bloodbolt;
...
struct mob_whirling_bladeAI : public Scripted_NoMovementAI
{
    ...
    Timer_UnCheked Whirl;
    Timer_UnCheked DieTimer;
...
struct mob_shadowmoon_deathshaperAI : public ScriptedAI
{
    ...
    Timer_UnCheked Shadowbolt;
    Timer_UnCheked DeathCoil;
    Timer_UnCheked DemonArmor;
    Timer_UnCheked RaiseDeadCheck;
...
struct mob_shadowmoon_reaverAI : public ScriptedAI
{
    ...
    Timer_UnCheked SpellAbsorption;
    Timer_UnCheked ShadowResonance;
...
struct mob_shadowmoon_riding_houndAI : public ScriptedAI
{
    ...
    void DamageMade(Unit* target, uint32 &damage, bool direct_damage, uint8 school)
    {
        if (damage)
        {
...
#define BATTLE_STANCE_YELL     -1200454
#define DEFENSIVE_STANCE_YELL  -1200455
...
struct mob_shadowmoon_weapon_masterAI : public ScriptedAI
{
    ...
    Timer_UnCheked m_nextMove;
    int32  m_nextId;
    uint8  Stance;

    Timer_UnCheked SpecialTimer;
    Timer_UnCheked KnockAway;
    ...
    void SetWeaponModelAndDamage(uint8 Stance)
    {
        ...
        switch (Stance)
        {
            ...
            case BATTLE:
                ...
                me->SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, cinfo->maxdmg * me->GetCreatureDamageMod());
                me->UpdateDamagePhysical(BASE_ATTACK);
                me->SetBaseWeaponDamage(OFF_ATTACK, MINDAMAGE, cinfo->mindmg * me->GetCreatureDamageMod());
                me->SetBaseWeaponDamage(OFF_ATTACK, MAXDAMAGE, cinfo->maxdmg * me->GetCreatureDamageMod());
                me->SetFloatValue(UNIT_FIELD_MINOFFHANDDAMAGE, cinfo->mindmg * me->GetCreatureDamageMod());
                me->SetFloatValue(UNIT_FIELD_MAXOFFHANDDAMAGE, cinfo->maxdmg * me->GetCreatureDamageMod());
    ...
    void DamageTaken(Unit* who, uint32& damage)
    {
        if (damage)
        {
            if (me->GetHealthPercent() < 80 && Stance == DEFENSIVE)
            {
            ...
            if (me->GetHealthPercent() < 35 && Stance == BERSERKER)
            {
...
struct mob_bonechewer_shield_discipleAI : public ScriptedAI
{
    ...
    Timer_UnCheked Intervene;
    Timer_UnCheked ShieldBash;
    Timer_UnCheked ThrowShield;
    ...
    void DamageTaken(Unit* who, uint32& damage)
    {
        if (damage)
        {
            if (me->GetHealthPercent() < 30 && !me->HasAura(SPELL_BSD_SHIELD_WALL, 0))
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (me->HasAura(SPELL_WHIRLWIND_2, 0))
        {
            if (MoveTimer.Expired(diff))
            {
                ...
                Position pos;
                me->Relocate(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation());
                me->GetValidPointInAngle(pos, 5.0, 3.14*RAND(0, 1 / 6, 2 / 6, 3 / 6, 4 / 6, 5 / 6, 1), true);
                me->GetMotionMaster()->Clear(false);
...
struct mob_bonechewer_blood_prophetAI : public ScriptedAI
{
    ...
    Timer_UnCheked BloodDrain;
    Timer_UnCheked Bloodbolt;
    Timer_UnCheked ProphecyOfBlood;
...
struct mob_pristess_of_dementiaAI : public ScriptedAI
{
    ...
    Timer_UnCheked Confusion;
    Timer_UnCheked Dementia;
    Timer_UnCheked ImageSummon;
...
struct mob_image_of_dementiaAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        DoZoneInCombat(80.0f);
        for (uint8 i = 0; i < 8; ++i)
            me->ApplySpellImmune(i, IMMUNITY_MECHANIC, CCMechanics[i], true);
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        if (MoveTimer.Expired(diff))
        {
            ...
            {
                Position pos;
                target->GetValidPointInAngle(pos, 10.0, 3.14*RAND(0, 1 / 6, 2 / 6, 3 / 6, 4 / 6, 5 / 6, 1), true);
                me->GetMotionMaster()->Clear(false);
...
struct mob_pristess_of_delightAI : public ScriptedAI
{
    ...
    Timer_UnCheked CurseOfVitality;
    Timer_UnCheked Polymorph;
...
#define YELL_SISTER_OF_PAIN -1200456
...
struct mob_sister_of_painAI : public ScriptedAI
{
    ...
    uint64 SisterGUID;
    Timer_UnCheked LashOfPain;
    Timer_UnCheked ShadowWordPain;
    Timer_UnCheked ShellOfPain;
    ...
    void JustDied(Unit* Killer)
    {
        if (SisterGUID)
        {
            if (Unit* sister = Unit::GetUnit((*me), SisterGUID))
                if (sister->isAlive())
                    me->Kill(sister, true);
        }
    }
    ...
    void DamageTaken(Unit* who, uint32 &damage)
    {
        uint32 HPPercent = me->GetHealthPercent();
        uint8 stack = 0;
...
#define YELL_SISTER_OF_PLEASURE -1200457
...
struct mob_sister_of_pleasureAI : public ScriptedAI
{
    ...
    Timer_UnCheked GreaterHeal;
    Timer_UnCheked HolyNova;
    Timer_UnCheked ShellOfLife;
    bool cooldown;
    uint64 SisterGUID;
    uint64 SisterToCast;
    ...
    {
        DoZoneInCombat(80.0f);
        DoYell(-1200457, 0, who);

        std::list<Creature*> crList = FindAllCreaturesWithEntry(NPC_SISTER_OF_PAIN, 20);
        if (!crList.empty())
        {
          for (std::list<Creature*>::iterator itr = crList.begin(); itr != crList.end(); ++itr)
          {
            if ((*itr)->isAlive())
            {
              if((*itr)->GetDBTableGUIDLow() == SisterToCast)
              {
                if (!me->HasAura(SPELL_SHARED_BONDS, 0) && !(*itr)->HasAura(SPELL_SHARED_BONDS, 0))
                {
                  (*itr)->CastSpell(me, SPELL_SHARED_BONDS, false);
                  me->CastSpell((*itr), SPELL_SHARED_BONDS, false);
                  SisterGUID = (*itr)->GetGUID();
                  ((mob_sister_of_painAI*)((*itr)->AI()))->SisterGUID = me->GetGUID();
                }
              }
            }
          }
...
#define YELL_CHARMING_COURTESAN -1200458
...
struct mob_charming_courtesanAI : public ScriptedAI
{
    ...
    Timer_UnCheked Infatuation;
    Timer_UnCheked PoisonousThrow;
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (Infatuation.Expired(diff))
        {
            Unit * target = SelectUnit(SELECT_TARGET_RANDOM, 0,
                [this](Unit* u) {return u->GetTypeId() == TYPEID_PLAYER && !u->HasAura(SPELL_POLYMORPH, 0) &&
...
struct mob_enslaved_servantAI : public ScriptedAI
{
    ...
    Timer_UnCheked Uppercut;
    Timer_UnCheked KidneyShot;
...
#define YELL_TEMPLE_CONCUBINE -1200459
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (Polymorph.Expired(diff))
        {
            Unit * target = SelectUnit(SELECT_TARGET_RANDOM, 1,
                [this](Unit* u) {return u->GetTypeId() == TYPEID_PLAYER && !u->HasAura(SPELL_INFATUATION, 0) &&
                me->IsWithinDist(u, 100, false); });
...
struct mob_illidari_archonAI : public ScriptedAI
{
    ...
    uint32 type;
    Timer_UnCheked shieldCooldownTimer;
    Timer_UnCheked Heal;
    bool shieldCooldown;

    Timer_UnCheked MindFlay;
    Timer_UnCheked MindBlast;
    Timer_UnCheked wordDeathTimer;
...
struct mob_illidari_assassinAI : public ScriptedAI
{
    ...
    Timer_UnCheked VanishEvent;
    Timer_UnCheked ParalyzingPoison;
    ...
    void OnAuraRemove(Aura* Aur, bool stack)
    {
        if(Aur->GetId() == SPELL_VANISH)
        {
            ...
            me->ApplySpellImmune(0, IMMUNITY_ID, SPELL_IA_FLARE, false);
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (VanishEvent.Expired(diff))
        {
            ...
            me->ApplySpellImmune(0, IMMUNITY_ID, SPELL_IA_FLARE, true);
        ...
        {
            if (Player* PlayerTarget = me->GetPlayerInWorld(AmbushTagetGUID))
            {
...
struct mob_illidari_battle_mageAI : public ScriptedAI
{
    ...
    Timer_UnCheked Blizzard;
    Timer_UnCheked Flamestrike;
    Timer_UnCheked DirectTimer;
...
#define YELL_ILLIDARI_BLOOD_LORD    -1200460
...
#define YELL_PROMENADE_SENTINEL    -1200461
...
struct mob_promenade_sentinelAI : public ScriptedAI
{
    ...
    Timer_UnCheked L5arcane;
    Timer_UnCheked L4arcane;
