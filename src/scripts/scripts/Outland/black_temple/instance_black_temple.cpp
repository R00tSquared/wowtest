...
struct instance_black_temple : public ScriptedInstance
{
    ...
    void OnCreatureCreate(Creature *pCreature, uint32 cEntry)
    {
        ...
        switch (cEntry)
        {
            ...
            case 22948:
            {
                m_gurtoggGUID = pCreature->GetGUID();
                break;
            }
            // Supremus
            case 22898:
                if (GetData(EVENT_HIGHWARLORDNAJENTUS) != DONE)
                    pCreature->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE);
                m_supremusGUID = pCreature->GetGUID();
                break;
            // Mother Shahraz
            case 22947:
				//if (sWorld.isEasyRealm() && !instance->IsHeroic())
				//{
				//	if (GetData(EVENT_SUPREMUS) != DONE)
				//		pCreature->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE);
				//}
				//else

                if (GetData(EVENT_RELIQUARYOFSOULS) != DONE || GetData(EVENT_TERONGOREFIEND) != DONE)
					pCreature->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE);

                m_motherShahrazGUID = pCreature->GetGUID();
                break;
            // Illidan Stormrage
            case 22917:
            ...
            // Illidari Council
            case 22949:
                    if (GetData(EVENT_MOTHERSHAHRAZ) != DONE)
                        pCreature->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE);
    ...
    uint64 GetData64(uint32 identifier)
    {
        switch(identifier)
        {
            ...
            case DATA_MOTHERSHAHRAZ:               return m_motherShahrazGUID;
            case DATA_GURTOGG:                     return m_gurtoggGUID;
    ...
    void SetData(uint32 type, uint32 data)
    {
        switch (type)
        {
            case EVENT_HIGHWARLORDNAJENTUS:
                if (data == DONE)
                {
                    HandleGameObject(NajentusGate, true);
                        if (Creature *pSupremus = GetCreature(GetData64(DATA_SUPREMUS)))
                            pSupremus->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE, false);
                }
                ...
                if (data == DONE && !AshtongueBrokenList.empty())
                {
                    ...
                    for (std::list<uint64>::iterator itr = AshtongueBrokenList.begin(); itr != AshtongueBrokenList.end(); ++itr)
                    {
                        if (Creature *pBroken = instance->GetCreature(*itr))
                        {
                            ...
                            if(pBroken->IsInCombat())
                                pBroken->AI()->EnterEvadeMode();
                        }
                    }
                }
            ...
            case EVENT_TERONGOREFIEND:
                ...
                if (data == DONE)
                {
                    if (Encounters[2] == DONE && Encounters[4] == DONE && Encounters[5] == DONE)
                    {
                        HandleGameObject(TempleDoor, true);
                        
                        if (Creature* pMotherShahraz = GetCreature(GetData64(DATA_MOTHERSHAHRAZ)))
                            pMotherShahraz->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE, false);
                    } 
                }
            ...
            case EVENT_MOTHERSHAHRAZ:
                if (data == DONE)
                {
                    HandleGameObject(MotherDoor, true);
                        ...
                        if (Creature *pGathios = GetCreature(GetData64(DATA_GATHIOSTHESHATTERER)))
                            pGathios->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE, false);
                        if (Creature *pZerevor = GetCreature(GetData64(DATA_HIGHNETHERMANCERZEREVOR)))
                            pZerevor->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE, false);
                        if (Creature *pMalande = GetCreature(GetData64(DATA_LADYMALANDE)))
                            pMalande->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE, false);
                        if (Creature *pVeras = GetCreature(GetData64(DATA_VERASDARKSHADOW)))
                            pVeras->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE, false);
                }
    ...
    uint32 GetData(uint32 type)
    {
        switch(type)
        {
            ...
            //case DATA_OPEN_PATH:            return Encounters[10];
