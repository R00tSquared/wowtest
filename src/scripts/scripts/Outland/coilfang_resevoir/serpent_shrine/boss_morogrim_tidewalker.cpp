...
struct boss_morogrim_tidewalkerAI : public ScriptedAI
{
    ...
    Timer_UnCheked TidalWave_Timer;
    Timer_UnCheked WateryGrave_Timer;
    Timer_UnCheked Earthquake_Timer;
    Timer_UnCheked WateryGlobules_Timer;
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
            AddSpellToCast(SPELL_TIDAL_WAVE, CAST_SELF);
...
struct mob_water_globuleAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        if (isSpawned)
        {
            ...
            if (Unit *tmpUnit = SelectUnit(SELECT_TARGET_RANDOM, 0, 200, true))
            {
                m_creature->AddThreat(tmpUnit, 20000.0f);
                me->AI()->AttackStart(tmpUnit);
            }
            isSpawned = false;
        }
