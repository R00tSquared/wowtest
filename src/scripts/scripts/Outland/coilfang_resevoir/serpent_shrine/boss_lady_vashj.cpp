...
#define SPELL_FORKED_LIGHTNING      38145
...
struct boss_lady_vashjAI : public ScriptedAI
{
    ...
    Timer_UnCheked AggroTimer;
    Timer_UnCheked ShockBlast_Timer;
    Timer_UnCheked Entangle_Timer;
    Timer_UnCheked StaticCharge_Timer;
    Timer_UnCheked ForkedLightning_Timer;
    Timer_UnCheked Check_Timer;
    Timer_UnCheked ParalyzeCheck_Timer;
    Timer_UnCheked EnchantedElemental_Timer;
    Timer_UnCheked TaintedElemental_Timer;
    Timer_UnCheked CoilfangElite_Timer;
    Timer_UnCheked CoilfangStrider_Timer;
    Timer_UnCheked SummonSporebat_Timer;
    Timer_UnCheked SummonSporebat_StaticTimer;
    ...
    void Reset()
    {
        ...
        ShockBlast_Timer.Reset(urand(15000, 20000));
        ...
        me->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE); //set it only once on creature create (no need do intro if wiped)
    ...
    void StartEvent()
    {
        DoScriptText(RAND(SAY_AGGRO1, SAY_AGGRO2, SAY_AGGRO3, SAY_AGGRO4, SAY_PHASE1), me);
    ...
    void EnterCombat(Unit *who)
    {
        if (instance)
        {
            ...
            for(Map::PlayerList::const_iterator i = PlayerList.begin();i != PlayerList.end(); ++i)
            {
                ...
                {
                    i_pl->DestroyItemCount(ITEM_TAINTED_CORE, 1, true, false);
                }
            }
        }
    ...
    {
		//if (HeroicMode) // || !sWorld.RaidNerfed(MAP_SSK)
		if (instance->GetData(DATA_HYDROSSTHEUNSTABLEEVENT) != DONE ||
            instance->GetData(DATA_LEOTHERASTHEBLINDEVENT) != DONE ||
            instance->GetData(DATA_THELURKERBELOWEVENT) != DONE ||
            instance->GetData(DATA_KARATHRESSEVENT) != DONE ||
            instance->GetData(DATA_MOROGRIMTIDEWALKEREVENT) != DONE)
			return;
    ...
    {	
        ...
        if(Phase == 1 || Phase == 3)
        {
            ...
            if (ShockBlast_Timer.Expired(diff))
            {
                ...
                ShockBlast_Timer = 13000+rand()%12000;       //random cooldown
            }
            ...
            else
            {
				...
				if (!sWorld.RaidNerfed(MAP_TK))
					if (Possession_Timer.Expired(diff))
					{
						...
						SelectUnitList(targets, 2, SELECT_TARGET_RANDOM, 0, true, me->getVictimGUID());
        ...
        else
        {
            ...
            if (ForkedLightning_Timer.Expired(diff))
            {
                ...
                ForkedLightning_Timer = 5000+rand()%5000;   //blizzlike
            }
...
struct mob_enchanted_elementalAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        move = 1000;
        phase = 1;
        VashjGUID = instance ? instance->GetData64(DATA_LADYVASHJ) : 0;
    ...
    void UpdateAI(const uint32 diff)
    {
        if (move.Expired(diff))
        {
            ...
            Creature* Vashj = me->GetCreature(VashjGUID);
            if (!Vashj ||
                ((boss_lady_vashjAI*)((Creature*)Vashj)->AI())->InCombat == false || ((boss_lady_vashjAI*)((Creature*)Vashj)->AI())->Phase != 2 || Vashj->isDead())
            {
                //call Unsummon()
                me->DealDamage(me, me->GetMaxHealth(), DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL, false);
                return;
            ...
            if (phase == 3)
            {
                ...
                {
                    SpellEntry *spell = (SpellEntry *)GetSpellStore()->LookupEntry<SpellEntry>(SPELL_SURGE);
                    if( spell )
                    {
...
struct mob_tainted_elementalAI : public Scripted_NoMovementAI
{
    ...
    Timer_UnCheked PoisonBolt_Timer;
    Timer_UnCheked Despawn_Timer;

    void Reset()
    {
        PoisonBolt_Timer.Reset(5000 + rand() % 5000);
        Despawn_Timer.Reset(15000);
    }
...
struct mob_toxic_sporebatAI : public ScriptedAI
{
    ...
    bool moving;
    Timer_UnCheked ToxicSpore_Timer;
    Timer_UnCheked bolt_timer;
    Timer_UnCheked Check_Timer;
...
struct mob_coilfang_eliteAI : public ScriptedAI
{
    ...
    Timer_UnCheked Cleave_Timer, Check_Timer;
...
struct mob_coilfang_striderAI : public ScriptedAI
{
    ...
    uint8 MoveWP, path_nr;
    Timer_UnCheked MindBlast_Timer, Check_Timer;
...
bool ItemUse_item_tainted_core(Player *player, Item* _Item, SpellCastTargets const& targets)
{
    ...
    {
            player->DestroyItemCount(ITEM_TAINTED_CORE, 1, true, false);
