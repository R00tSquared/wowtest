...
enum // Ahune
{
    ...
    SPELL_AHUNE_CHEST_N             = 45939,
    SPELL_AHUNE_CHEST_H             = 46622,
...
struct boss_lord_ahuneAI : public Scripted_NoMovementAI
{
    ...
    void Reset()
    {
        if (pInstance)
        {
            ...
            if (GameObject* pGo = FindGameObject(187882, 100, me))
            {
...
bool GOHello_go_ice_stone_ahune(Player* pPlayer, GameObject* pGo)
{
	...
	if (pGo->GetMap()->IsHeroic())
	{
		//if (pInstance->GetData(DATA_MENNU) == DONE && pInstance->GetData(DATA_ROKMAR) == DONE && pInstance->GetData(DATA_QUAGMIRRAN) == DONE)
			canSummon = true;
	}
	//else
	//{
	//	// auto spawn boss after click - need to fix, but later
	//	if (pPlayer->GetQuestStatus(11691) == QUEST_STATUS_COMPLETE)
	//		canSummon = true;
	//}

	if (canSummon)
	{
		pGo->SummonCreature(25740, -90.151627, -243.48, -1.102, 1.621, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 60000);
		pGo->SetFlag(GAMEOBJECT_FLAGS, GO_FLAG_INTERACT_COND);
		pGo->SetLootState(GO_JUST_DEACTIVATED);
		pGo->Delete();
	}
...
void AddSC_boss_lord_ahune()
{
    ...
    newscript->pGOUse = &GOHello_go_ice_stone_ahune;
    newscript->RegisterSelf();
}
