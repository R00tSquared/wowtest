        ...
        switch(type)
        {
            case DATA_MAGTHERIDON_EVENT:
                switch (data)
                {
                    ...
                    case NOT_STARTED:
                        RespawnTimer = 10000;
                        CageTimer = 0; // Disable timer after wipe
    ...
    void Update(uint32 diff)
    {
        if (CageTimer.Expired(diff))
        {
            ...
            if (Magtheridon && Magtheridon->isAlive())
            {
                ...
                if (Unit* victim = Magtheridon->AI()->SelectUnit(SELECT_TARGET_NEAREST, 0, 100.0, true))                    
                    Magtheridon->AI()->AttackStart(victim);
            }
        ...
        if (RespawnTimer.Expired(diff))
        {
            for (std::set<uint64>::iterator i = ChannelerGUID.begin(); i != ChannelerGUID.end(); ++i)
            {
                if (Creature *Channeler = instance->GetCreature(*i))
                {
                    ...
                    {
                        Channeler->Respawn();
                        Channeler->CastSpell((Unit*)NULL, SPELL_SHADOW_GRASP_C, false);
                    }
                }
            }
