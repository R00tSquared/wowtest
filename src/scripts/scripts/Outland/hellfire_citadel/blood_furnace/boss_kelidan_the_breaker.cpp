...
struct boss_kelidan_the_breakerAI : public ScriptedAI
{
        ...
        for (uint8 i = 0; i < 5; i++)
        {
            if (Unit* caster = me->GetCreature(Channelers[i]))
                if (Unit* receiver = me->GetCreature(Channelers[GetChanneler(i)]))
                    caster->CastSpell(receiver, SPELL_CHANNELING, false);
        }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
            if(!m_creature->IsWithinDistInMap(&wLoc, 86))
                EnterEvadeMode();
            else
                DoZoneInCombat();
        ...
        if (BurningNova_Timer.Expired(diff))
        {
            ...
            if (SpellEntry *nova = (SpellEntry*)GetSpellStore()->LookupEntry<SpellEntry>(SPELL_BURNING_NOVA))
            {
...
struct mob_shadowmoon_channelerAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        if (m_creature->IsNonMeleeSpellCast(true))
            m_creature->InterruptNonMeleeSpells(true);
        if (Creature* Kelidan = GetClosestCreatureWithEntry(me, ENTRY_KELIDAN, 1000.0f))
    ...
    void EnterCombat(Unit* who)
    {
        if(Creature *Kelidan = (Creature *)FindCreature(ENTRY_KELIDAN, 1000, m_creature))
