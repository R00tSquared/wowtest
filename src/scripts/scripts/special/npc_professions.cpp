...
bool GossipHello_npc_prof_alchemy(Player *player, Creature *_Creature)
{
    ...
    if(_Creature->isTrainer())
        player->ADD_GOSSIP_ITEM(2, player->GetSession()->GetNpcOptionLocaleString(GOSSIP_TEXT_TRAIN), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_TRAIN);
...
{
    npc_prof_tailorAI(Creature *c) : ScriptedAI(c) {}
...
CreatureAI* GetAI_npc_prof_tailor(Creature *_Creature)
{
    return new npc_prof_tailorAI(_Creature);
}
    ...
    if (player->HasSkill(SKILL_ENGINERING) && player->GetBaseSkillValue(SKILL_ENGINERING) >= 200 && player->GetLevel() > 39)
    {
        if (player->GetQuestRewardStatus(3643) || player->GetQuestRewardStatus(3641) || player->GetQuestRewardStatus(3639))
        {
            ...
            if (player->HasSpell(S_GOBLIN))
                player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, GOSSIP_UNLEARN_GOBLIN, GOSSIP_SENDER_CHECK, GOSSIP_ACTION_INFO_DEF+7, BOX_UNLEARN_GOBLIN_SPEC, DoHighUnlearnCost(player), false);
        }
    }
    player->SEND_GOSSIP_MENU(30000, go->GetGUID());
    return true;
...
bool GossipSelect_go_soothsaying_for_dummies(Player *player, GameObject* go, uint32 sender, uint32 action)
{
    ...
    switch(sender)
    {
        case GOSSIP_SENDER_CHECK:
            SendActionMenu_go_soothsaying_for_dummies(player, go, action);
            break;
    }
    return true;
}
...
void AddSC_npc_professions()
{
    ...
    newscript->GetAI = &GetAI_npc_prof_tailor;
    newscript->RegisterSelf();
