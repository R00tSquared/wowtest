...
{
    player->ADD_GOSSIP_ITEM_EXTENDED(0, player->GetSession()->GetHellgroundString(16089), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1, "", 0, true);
    player->ADD_GOSSIP_ITEM(0, player->GetSession()->GetHellgroundString(16090), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);
...
bool GossipSelect_custom_gossip_codebox(Player *player, Creature *_Creature, uint32 sender, uint32 action )
{
    ...
    {
        _Creature->Say(-1200194, LANG_UNIVERSAL, 0);
...
bool GossipSelectWithCode_custom_gossip_codebox( Player *player, Creature *_Creature, uint32 sender, uint32 action, const char* sCode )
{
    if(sender == GOSSIP_SENDER_MAIN)
    {
        if(action == GOSSIP_ACTION_INFO_DEF+1)
        {
            ...
            {
                _Creature->Say(-1200195, LANG_UNIVERSAL, 0);
                _Creature->CastSpell(player, 12826, true);
            }
            else
            {
                _Creature->Say(-1200196, LANG_UNIVERSAL, 0);
