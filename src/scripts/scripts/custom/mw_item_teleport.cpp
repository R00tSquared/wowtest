...
bool DeathSide_Item_Teleporter(Player* Plr, Item* source, SpellCastTargets const& targets)
{
...
void SendDefaultMenu_TeleItem(Player* Plr, Item* source, uint32 action, SpellCastTargets const& targets)
{
    if (!sWorld.isEasyRealm())
    {
        Plr->CLOSE_GOSSIP_MENU();
    ...
    case 5025: //Outland 1
        ...
        Plr->ADD_GOSSIP_ITEM(7, Plr->GetSession()->GetHellgroundString(LANG_SCRIPT_NEXT), GOSSIP_SENDER_MAIN, 5030);
        Plr->ADD_GOSSIP_ITEM(7, Plr->GetSession()->GetHellgroundString(LANG_SCRIPT_BACK), GOSSIP_SENDER_MAIN, 5000);
        Plr->SEND_GOSSIP_MENU(990122, source->GetGUID());
        break;
    case 5030: //Outland 2
    ...
    case 5005: //Back To Main Menu
        DeathSide_Item_Teleporter(Plr, source, targets);
        break;
 ...
 bool DeathSide_Item_Teleporter_Gossip(Player* pPlayer, Item* pItem, uint32 uiSender, uint32 uiAction, SpellCastTargets const& targets)
 {
     if (sWorld.isEasyRealm() &&  !pPlayer->GetSession()->isPremium())
     {
         pPlayer->CLOSE_GOSSIP_MENU();
         ChatHandler(pPlayer->GetSession()).SendSysMessage(LANG_PREMIUM_NOT_ACTIVE);
         return true;
     }
     
    // Main menu
    if (uiSender == GOSSIP_SENDER_MAIN)
    SendDefaultMenu_TeleItem(pPlayer, pItem, uiAction, targets);
 
    return true;
 }

 void AddSC_DeathSide_Item_Teleporter()
 {
     Script *newscript;

     newscript = new Script;
     newscript->Name = "DeathSide_Item_Teleporter";
     newscript->pItemUse           = &DeathSide_Item_Teleporter;
     newscript->pGossipSelectItem  = &DeathSide_Item_Teleporter_Gossip;
