...
{
    instance_zulaman(Map *map) : ScriptedInstance(map), m_gbk(map) {Initialize();};
    ...
    std::list<uint64> flaggedCreatureList;

    void Initialize()
    {
        flaggedCreatureList.clear();
        StrangeGongGUID = 0;
    ...
    void OnCreatureCreate(Creature *creature, uint32 creature_entry)
    {
        switch(creature_entry)
        {
            ...
            case 24065:
            {
                if(GetData(TYPE_EVENT_RUN) == NOT_STARTED)
                {
                    if(!creature->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE))
                    {
                        flaggedCreatureList.push_back(creature->GetGUID());
                        creature->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE, true);
                    }
                }
                break;
    ...
    void CheckInstanceStatus()
    {
        if (BossKilled >= 4)
        {
            ...
            if (Creature* creature = GetCreature(HexLordGUID))
        ...
        if (BossKilled >= 5)
        {
            ...
            if (Creature* creature = GetCreature(ZuljinGUID))
                creature->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE, false);
    ...
    GBK_Encounters EncounterForGBK(uint32 enc)
    {
        switch (enc)
        {
            case DATA_NALORAKKEVENT:    return GBK_ZA_NALORAK;
            case DATA_AKILZONEVENT:     return GBK_ZA_AKILZON;
            case DATA_JANALAIEVENT:     return GBK_ZA_JANALAI;
            case DATA_HALAZZIEVENT:     return GBK_ZA_HALAZZI;
            case DATA_HEXLORDEVENT:     return GBK_ZA_HEXLORD;
            case DATA_ZULJINEVENT:      return GBK_ZA_ZULJIN;
        }
        return GBK_NONE;
    }
    ...
    void Load(const char* in)
    {
        ...
        loadStream >> Encounters[0] >> Encounters[1] >> Encounters[2] >> Encounters[3] >> Encounters[4] >>
            Encounters[5] >> Encounters[6] >> BossKilled >> ChestLooted >> QuestMinute >> Hostages[0] >> Hostages[1] >> Hostages[2] >> Hostages[3];
    ...
    void SetData(uint32 type, uint32 data)
    {
        ...
        case TYPE_EVENT_RUN:
            if (data == SPECIAL)
            {
                if(!flaggedCreatureList.empty())
                {
                    for(std::list<uint64>::iterator itr = flaggedCreatureList.begin(); itr != flaggedCreatureList.end(); ++itr)
                    {
                        if(Creature* flaggedNpc = GetCreature(*itr))
                            flaggedNpc->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE, false);
                    }
                }
