...
struct boss_akilzonAI : public ScriptedAI
{
    ...
    Timer_UnCheked StaticDisruption_Timer;
    Timer_UnCheked GustOfWind_Timer;
    Timer_UnCheked CallLighting_Timer;
    Timer_UnCheked ElectricalStorm_Timer;
    Timer_UnCheked SummonEagles_Timer;
    ...
    void DoSummonEagles()
    {
        std::list<Creature*> BirdsList = FindAllCreaturesWithEntry(NPC_SOARING_EAGLE, 200.0);
        uint32 count;
        if(BirdsList.empty())
            count = urand(6, 12);
        else
...
struct mob_soaring_eagleAI : public ScriptedAI
{
    ...
    Timer_UnCheked EagleSwoop_Timer;
    Timer_UnCheked Return_Timer;
    bool canMoveRandom;
    bool canCast;
