...
struct boss_nalorakkAI : public ScriptedAI
{
    ...
    Timer_UnCheked BrutalSwipe_Timer;
    Timer_UnCheked Mangle_Timer;
    Timer_UnCheked Surge_Timer;
    
    Timer_UnCheked LaceratingSlash_Timer;
    Timer_UnCheked RendFlesh_Timer;
    Timer_UnCheked DeafeningRoar_Timer;
    
    Timer_UnCheked ShapeShift_Timer;
    ...
    void Reset()
    {
        ...
        else
        {
            ...
            // causing bugs with EnterEvadeMode()
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
            if (!m_creature->IsWithinDistInMap(&wLoc, 100) && !MoveEvent)
                EnterEvadeMode();
            else
                DoZoneInCombat();
            m_creature->SetSpeed(MOVE_RUN,2);
