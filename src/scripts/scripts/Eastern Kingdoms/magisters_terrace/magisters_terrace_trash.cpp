...
struct mob_sunwell_mage_guardAI : public ScriptedAI
{
    ...
    Timer_UnCheked Glaive_Timer;
    Timer_UnCheked Magic_Field_Timer;
...
struct mob_sunblade_magisterAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (Frostbolt_Timer.Expired(diff))
        {
            ...
            Frostbolt_Timer = urand(5000, 6000);
        }
...
struct mob_sunblade_warlockAI : public ScriptedAI
{
    ...
    Timer_UnCheked SummonImp_Timer;
    Timer_UnCheked FelArmor_Timer;
    Timer_UnCheked Immolate_Timer;
    Timer_UnCheked OOCTimer;
    uint64 SummonGUID;
...
struct mob_sunblade_physicianAI : public ScriptedAI
{
    ...
    Timer_UnCheked Poison_Timer;
    Timer_UnCheked Prayer_of_Mending_Timer;
    Timer_UnCheked OOCTimer;
...
struct mob_sunblade_blood_knightAI : public ScriptedAI
{
    ...
    Timer_UnCheked Judgement_Timer;
    Timer_UnCheked Holy_Light_Timer;
    Timer_UnCheked Seal_Timer;
    Timer_UnCheked OOCTimer;
...
int32 SAY_OOC1 = -1200258;
int32 SAY_OOC2 = -1200259;
int32 SAY_AGGRO1 = -1200260;
int32 SAY_AGGRO2 = -1200261;
int32 SAY_AGGRO3 = -1200262;
...
struct mob_wretched_skulkerAI : public ScriptedAI
{
    ...
    Timer_UnCheked Drink_Timer;
    Timer_UnCheked Wretched_Stab_Timer;
    Timer_UnCheked OOCTimer;
...
struct mob_wretched_bruiserAI : public ScriptedAI
{
    ...
    Timer_UnCheked OOCTimer;
    Timer_UnCheked Drink_Timer;
    Timer_UnCheked Wretched_Strike_Timer;
...
struct mob_wretched_huskAI : public ScriptedAI
{
    ...
    Timer_UnCheked OOCTimer;
    Timer_UnCheked Drink_Timer;
    Timer_UnCheked Wretched_Cast_Timer;
...
struct mob_brightscale_wyrmAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        if (Unit* npcNetherEnergy = FindCreature(NPC_NETHER_ENERGY, 100, m_creature))
        {
            float fX, fY, fZ;
            m_creature->GetRandomPoint(npcNetherEnergy->GetPositionX(), npcNetherEnergy->GetPositionY(), npcNetherEnergy->GetPositionZ(), 10.0f, fX, fY, fZ);
    ...
    void UpdateAI(const uint32 diff)
    {
        if (!UpdateVictim())
        {
            ...
            {
                Unit* npcNetherEnergy = FindCreature(NPC_NETHER_ENERGY, 100, m_creature);
...
struct mob_sister_of_tormentAI : public ScriptedAI
{
    ...
    Timer_UnCheked LashOfPain_Timer;
    Timer_UnCheked DeadlyEmbrace_Timer;
    Timer_UnCheked OOCTimer;
...
struct mob_coilskar_witchAI : public ScriptedAI
{
    ...
    Timer_UnCheked FrostArrow_Timer;
    Timer_UnCheked ForkedLightning_Timer;
    Timer_UnCheked OOCTimer;
    bool canShield;
...
struct mob_ethereum_smugglerAI : public ScriptedAI
{
    ...
    ScriptedInstance* pInstance;
     ...
     Timer_UnCheked ExplosionCombo_Timer;
     Timer_UnCheked Check_Timer;
     Timer_UnCheked OOCTimer;
...
#define KALEC_WELCOME1      16183
#define KALEC_WELCOME2      16184
#define KALEC_CONTINUE1     16185
#define KALEC_CONTINUE2A    16186
#define KALEC_CONTINUE2B    16187
...
bool GossipHello_npc_kalec(Player *player, Creature *_Creature)
{
    ...
    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, player->GetSession()->GetHellgroundString(KALEC_WELCOME1), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, player->GetSession()->GetHellgroundString(KALEC_WELCOME2), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3);
...
bool GossipSelect_npc_kalec(Player *player, Creature *_Creature, uint32 sender, uint32 action)
{
    switch (action)
    {
        case GOSSIP_ACTION_INFO_DEF + 1:
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, player->GetSession()->GetHellgroundString(KALEC_CONTINUE1), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
        ...
        case GOSSIP_ACTION_INFO_DEF + 3:
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, player->GetSession()->GetHellgroundString(KALEC_CONTINUE2A), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 4);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, player->GetSession()->GetHellgroundString(KALEC_CONTINUE2B), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 5);
