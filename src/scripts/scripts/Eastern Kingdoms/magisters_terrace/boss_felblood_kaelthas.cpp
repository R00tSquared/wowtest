...
struct boss_felblood_kaelthasAI : public ScriptedAI
{
    ...
    SummonList summons;
    Timer_UnCheked FireballTimer;
    Timer_UnCheked PhoenixTimer;
    Timer_UnCheked FlameStrikeTimer;
    Timer_UnCheked CheckTimer;
    Timer_UnCheked IntroTimer;
    Timer_UnCheked OutroTimer;
    bool Intro;
    bool Outro;
    std::list<uint64> trashList;

    //Heroic only
    Timer_UnCheked PyroblastTimer;

    Timer_UnCheked GravityLapseTimer;
    ...
    void Reset()
    {
        ...
        IntroTimer.Reset(sWorld.isEasyRealm() ? 3000 : 36000);
    ...
    void DamageTaken(Unit* done_by, uint32 &damage)
    {
        if(damage > m_creature->GetHealth() && done_by->GetGUID() != me->GetGUID())
        {
            ...
            me->SetNonAttackableFlag(UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NON_ATTACKABLE);
            RemoveGravityLapse();
            me->DeleteThreatList();
            me->RemoveAllAuras();
            me->CombatStop(true);
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if(Outro)
        {
            ...
            {
                me->SetNonAttackableFlag(UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NON_ATTACKABLE, false);
