...
#define C_CRIMSON_INITIATE    10420
#define C_CRIMSON_GALLANT     10424
#define C_CRIMSON_GUARDSMAN   10418
#define C_CRIMSON_CONJURER    10419
#define NPC_TIMMY_THE_CRUEL     10808
...
struct instance_stratholme : public ScriptedInstance
{
    ...
    Timer _ChangedNameBaronRun_Timer;
    Timer _ChangedNameSlaugtherSquare_Timer;
    ...
    void Initialize()
    {
        ...
        _ChangedNameBaronRun_Timer.Reset(0);
        _ChangedNameSlaugtherSquare_Timer.Reset(0);
    ...
    void OnCreatureCreate(Creature *creature, uint32 creature_entry)
    {
        switch(creature->GetEntry())
        {
            ...
            case C_CRIMSON_INITIATE:
            case C_CRIMSON_GALLANT:
            case C_CRIMSON_GUARDSMAN:
            case C_CRIMSON_CONJURER:
                // Only store those in the yard
                if (creature->IsWithinDist2d(3668.603, -3183.314, 40.0f))
                    m_suiCrimsonLowGuids.insert(creature->GetGUID());
                break;
        }
    }
    ...
    void OnCreatureDeath(Creature* creature)
    {
        switch (creature->GetEntry())
        {
            ...
            case C_CRIMSON_GUARDSMAN:
            case C_CRIMSON_CONJURER:
                if (m_suiCrimsonLowGuids.find(creature->GetGUID()) != m_suiCrimsonLowGuids.end())
                {
                    m_suiCrimsonLowGuids.erase(creature->GetGUID());
    
                    // If all courtyard mobs are dead then summon Timmy
                    if (m_suiCrimsonLowGuids.empty())
                        creature->SummonCreature(NPC_TIMMY_THE_CRUEL, 3696.851, -3152.736, 127.661, 4.024, TEMPSUMMON_DEAD_DESPAWN, 0);
                }
                break;
        }
    }
        ...
        case TYPE_BARON_RUN:
            ...
            case IN_PROGRESS:
                if (Encounter[0] == IN_PROGRESS || Encounter[0] == FAIL)
                    break;
                _ChangedNameBaronRun_Timer.Reset(2700000);
    ...
    {
        if (_ChangedNameBaronRun_Timer.GetInterval())
        {
            if (_ChangedNameBaronRun_Timer.Expired(diff))
            {
        ...
        if (_ChangedNameSlaugtherSquare_Timer.GetInterval())
        {
            if (_ChangedNameSlaugtherSquare_Timer.Expired(diff))
            {
