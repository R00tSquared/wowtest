...
instance_karazhan::instance_karazhan(Map* map) : ScriptedInstance(map), m_gbk(map) {Initialize();}
...
GBK_Encounters instance_karazhan::EncounterForGBK(uint32 enc)
{
    switch (enc)
    {
        case DATA_ATTUMEN_EVENT:                return GBK_KARA_ATTUNEMENT;
        case DATA_MOROES_EVENT:                 return GBK_KARA_MOROES;
        case DATA_MAIDENOFVIRTUE_EVENT:         return GBK_KARA_MAIDEN;
        case DATA_CURATOR_EVENT:                return GBK_KARA_CURATOR;
        case DATA_SHADEOFARAN_EVENT:            return GBK_KARA_ARAN;
        case DATA_TERESTIAN_EVENT:              return GBK_KARA_TERESTIAN;
        case DATA_NETHERSPITE_EVENT:            return GBK_KARA_NETHERSPITE;
        case DATA_MALCHEZZAR_EVENT:             return GBK_KARA_MALCHEZAR;
        case DATA_NIGHTBANE_EVENT:              return GBK_KARA_NIGHTBANE;
        case DATA_DUST_COVERED_CHEST:           return GBK_KARA_CHESS;
        case DATA_OPERA_EVENT:                  return GBK_KARA_OPERA;
    }
    return GBK_NONE;
}
...
void instance_karazhan::SetData(uint32 type, uint32 data)
{
    switch (type)
    {
        ...
        case DATA_CHESS_EVENT:
            if (GetData(DATA_DUST_COVERED_CHEST) != DONE)
                SetData(DATA_DUST_COVERED_CHEST, data);
...
void instance_karazhan::OnPlayerDealDamage(Player* plr, uint32 amount)
{
    m_gbk.DamageDone(plr->GetGUIDLow(), amount);
}

void instance_karazhan::OnPlayerHealDamage(Player* plr, uint32 amount)
{
    m_gbk.HealingDone(plr->GetGUIDLow(), amount);
}

void instance_karazhan::OnPlayerDeath(Player* plr)
{
        ...
        if (AnimalBossCheck.Expired(diff))
        {
            ...
            if (!found)
            {
                ...
                if (boss && boss->isAlive())
                {
                    ...
                    boss->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE, false);
                    boss->SetVisibility(VISIBILITY_ON);
                    boss->SetReactState(REACT_AGGRESSIVE);
                    switch(boss->GetEntry())
                    {
                        case 16179:
                            boss->MonsterTextEmote(-1200243, 0);
                            break;
                        case 16180:
                            boss->MonsterTextEmote(-1200244, 0);
                            break;
                        case 16181:
                            boss->MonsterTextEmote(-1200245, 0);
                            break;
                    }
                }
                AnimalBossCheck = 0;
...
Player* instance_karazhan::GetPlayerInMap()
{
    ...
    debug_log("HSCR: Karazhan: GetPlayerInMap, but PlayerList is empty!");
    return NULL;
}
