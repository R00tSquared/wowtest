...
void npc_tirion_fordring_in_dreamsAI::DoAttack(const uint32 uiDiff)
{
    ...
    {
        if(!me->HasAura(SPELL_DEVOTION_AURA, 0))
            DoCast(m_creature, SPELL_DEVOTION_AURA/*,CAST_AURA_NOT_PRESENT*/);
    ...
    {
        DoCast(m_creature->GetVictim(), SPELL_HOLY_CLEAVE);
        m_uiHolyCleaveTimer = urand(5000, 7000);
    }
    else
    ...
    {
        DoCast(m_creature, SPELL_HOLY_STRIKE);
        m_uiHolyStrikeTimer = urand(6000, 8000);
    }
    else
    ...
    switch (m_uiEventCount)
    {
        ...
        case 2:
            if (Creature* pTaelan = m_pInterface->GetCreature(m_creature, INDEX_TAELAN))
            {
                ...
                m_creature->GetMotionMaster()->MovePoint(99, pTaelan->GetPositionX()-2.0f, pTaelan->GetPositionY()-2.0f, pTaelan->GetPositionZ(), true);
                m_uiEventTimer = 3000;
            }            
            break;
        case 3:
        ...
        case 14:
        {
            if (m_uiEventTimer < uiDiff)
            {
                sLog.outLog(LOG_DEFAULT, "[WesternPlaguelands.InDreams] TaelanAI cast failed, retrying.");
                ...
                {
                    sLog.outLog(LOG_DEFAULT, "[WesternPlaguelands.InDreams] TaelanAI cast failed completely, aborting.");
                    FailEscort();
                }
            }
            else
...
void npc_isillienAI::DamageTaken(Unit* pDoneBy, uint32& uiDamage)
{
    ...
    if (m_uiEventCount <= 5 && m_uiSubEventCount < 3 && pDoneBy && ((pDoneBy->GetTypeId() == TYPEID_PLAYER) || (IS_PLAYER_GUID(pDoneBy->GetCharmerOrOwnerGUID()))))
    {
        uiDamage = 0;
    }
}
    ...
    for (auto itr = inner ? pCrimsonInner.begin() : pCrimsonOuter.begin(); itr != (inner ? pCrimsonInner.end() : pCrimsonOuter.end()); ++itr)
    {
        if (auto pCrimson = m_creature->GetMap()->GetCreature(*itr))
        {
            ...
            else
            {
                ...
                if (!pVictim || ((pVictim->GetTypeId() != TYPEID_PLAYER) || (!IS_PLAYER_GUID(pVictim->GetCharmerOrOwnerGUID()))))
                    pCrimson->AttackStop();
            }
        }
    }
    ...
    if (inner)
    {
        for (uint8 i = 0; i < 2; ++i)
        {
            auto pCrimson = m_creature->SummonCreature(NPC_CRIMSON,
                ...
                SpawnCoordsCrimsonInner[i].o, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 15 * MINUTE*MILLISECONDS, true);
            ...
            if (pCrimson)
            {
                ...
                pCrimson->GetMotionMaster()->MovePoint(0, MoveCoordsCrimsonInner[i].x, MoveCoordsCrimsonInner[i].y, MoveCoordsCrimsonInner[i].z, true);
            }
        }
    }
    else
    {
        for (uint8 i = 0; i < 3; ++i)
        {
            auto pCrimson = m_creature->SummonCreature(NPC_CRIMSON,
                ...
                SpawnCoordsCrimsonOuter[i].o, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 15 * MINUTE*MILLISECONDS, true);
        ...
        case 2:
            if (m_uiEventTimer < uiDiff)
            {
                if (auto pTaelan = m_pInterface->GetCreature(m_creature, INDEX_TAELAN))
                {
                    ...
                    DoCast(pTaelan, SPELL_TAELAN_DEATH, true);
...
void npc_isillienAI::Event5TalkBeforeTirionArrival(const uint32 uiDiff)
{
    switch (m_uiSubEventCount)
    {
        ...
        case 3:
            if (m_uiEventTimer < uiDiff)
            {
                if (auto pPlayer = Unit::GetPlayerInWorld(pPlayerGuid))
                {
                    m_creature->AddThreat(pPlayer, 20.0f);
                    m_creature->RemoveFlag(UNIT_FIELD_FLAGS, 896);
                ...
                m_uiEventCount = 6;
                m_uiSubEventCount = 0;
                m_uiEventTimer = 45 * MILLISECONDS;
            }
            else
                m_uiEventTimer -= uiDiff;
            break;
        ...
        case 0:
            if (m_uiEventTimer < uiDiff)
            {
                if (Creature* pTirion = m_creature->SummonCreature(NPC_TIRION, 
                    ...
                    SpawnCoordsTirion.y,
                    SpawnCoordsTirion.z,
                    SpawnCoordsTirion.o, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 15*MINUTE*MILLISECONDS, false))
                {
...
void npc_isillienAI::DoAttack(const uint32 uiDiff)
{
    if (m_uiGlobalCooldownTimer < uiDiff)
    {
        ...
        if (m_uiDominateMindTimer < uiDiff)
        {
            ...
            {
                DoCast(pVictim, SPELL_DOMINATE_MIND);
        ...
        if (m_creature->GetHealthPercent() < 90)
        {
            ...
            {
                DoCast(m_creature, urand(0, 1) ? SPELL_GREATER_HEAL : SPELL_FLASH_HEAL);
                m_uiGlobalCooldownTimer = 1500;
                m_uiHealTimer = urand(5000, 15000);
                return;
            }
...
{                
    switch (m_creature->GetObjectGuid().GetCounter())
    {
        case FIRST_CAVALIER_GUID:
            m_creature->GetMotionMaster()->Clear();
            m_creature->GetMotionMaster()->MovePoint(0, MoveCoordsCavaliers[0].x, MoveCoordsCavaliers[0].y, MoveCoordsCavaliers[0].z, true);
            break;
        case SECOND_CAVALIER_GUID:
            m_creature->GetMotionMaster()->Clear();
            m_creature->GetMotionMaster()->MovePoint(0, MoveCoordsCavaliers[1].x, MoveCoordsCavaliers[1].y, MoveCoordsCavaliers[1].z, true);
            DoScriptText(SAY_CAVALIER_5, m_creature);
            break;
        case THIRD_CAVALIER_GUID:
            m_creature->GetMotionMaster()->Clear();
            m_creature->GetMotionMaster()->MovePoint(0, MoveCoordsCavaliers[2].x, MoveCoordsCavaliers[2].y, MoveCoordsCavaliers[2].z, true);
            break;
        case FOURTH_CAVALIER_GUID:
            m_creature->GetMotionMaster()->Clear();
            m_creature->GetMotionMaster()->MovePoint(0, MoveCoordsCavaliers[3].x, MoveCoordsCavaliers[3].y, MoveCoordsCavaliers[3].z, true);
            break;
    }
}
...
void npc_scarlet_cavalierAI::DoAttack(const uint32 uiDiff)
{
    ...
    if(me->GetVictim())
    {
        ...
        if (m_creature->GetVictim() && m_creature->GetVictim()->GetHealthPercent() < 25.0f)
        {
...
void npc_taelanAI::Reset()
{
    ...
    DoCast(m_creature, SPELL_MURDER/*, CAST_AURA_NOT_PRESENT*/);
}
...
void npc_taelanAI::EnterEvadeMode()
{
    ...
    if (m_uiEventCount == -2)
    {
        ...
        if (m_killedCavaliers)
        {
            m_creature->setFaction(me->getFaction());
            m_creature->SetFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_QUESTGIVER);
            m_uiEventCount = -3;
        }
...
void npc_taelanAI::WaypointReached(uint32 uiPointId)
{
    switch (uiPointId)
    {
        case 0: // spawn point: after the quest is taken
            ...
            m_uiEventCount = 4;
            break;
        case 25: // mount point: castle exit
            SetEscortPaused(true);
            m_uiEventTimer = 3000;
            m_uiEventCount = 5;
            break;
        case 55: // final point: tower reached
            SetEscortPaused(true);
            m_uiEventTimer = 2000;
            m_uiEventCount = 6;
            break;
...
void npc_taelanAI::Event1ReceivedPresent()
{
    ...
    cavaliers = FindAllCreaturesWithEntry(NPC_CAVALIER, 20.0f);
...
void npc_taelanAI::Event2FightCavaliers(const uint32 uiDiff)
{
    if (m_uiEventTimer < uiDiff)
    {
        ...
        m_creature->SetStandState(UNIT_STAND_STATE_STAND);
        DoCast(m_creature, SPELL_TAELAN_SUFFERING, true);
        m_creature->setFaction(FACTION_ESCORT_N_FRIEND_PASSIVE);
        ...
        case 1:
            if (m_uiEventTimer < uiDiff)
            {
                ...
                if (Creature * pIsillien = m_creature->SummonCreature(NPC_ISILLIEN, 
                    ...
                    SpawnCoordsIsillien.o, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 20*MINUTE*MILLISECONDS, true))
                {
...
void npc_taelanAI::DoHeal(const uint32 uiDiff)
{
    ...
    if (auto pHealMe = SelectLowestHpFriendly(40.0f, 40, true))
    {
        if (pHealMe->GetHealthPercent() < 20.0f)
        {
            if (m_uiLayOnHandsTimer < uiDiff)
            {
                DoCast(pHealMe, SPELL_LAY_ON_HANDS);
                m_uiLayOnHandsTimer = HOUR * MILLISECONDS;
                return;
            }
            else
        ...
        {
            DoCast(pHealMe, SPELL_HOLY_LIGHT);
...
void npc_crimson_eliteAI::EnterEvadeMode()
{
    ...
    if (auto pTaelan = GetClosestCreatureWithEntry(me, NPC_TAELAN, 150.0f))
    {
        if (auto pTaelanAI = static_cast<npc_taelanAI*>(pTaelan->AI()))
        {
...
void npc_crimson_eliteAI::DoAttack(const uint32 uiDiff)
{
    ...
    {
        DoCast(m_creature, SPELL_RETRIBUTION_AURA, true);
    ...
    if (m_uiHolyStrikeTimer < uiDiff)
    {
        DoCast(m_creature->GetVictim(), SPELL_HOLY_STRIKE);
        m_uiHolyStrikeTimer = urand(6000, 8000);
    }
    else
        m_uiHolyStrikeTimer -= uiDiff;
...
bool QuestAccept_npc_taelan(Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_IN_DREAMS)
    {
        if (auto pTaelan = dynamic_cast<npc_taelanAI*>(pCreature->AI()))
        {
            ...
            pTaelan->Start(false, false, pPlayer->GetObjectGuid(), pQuest);
