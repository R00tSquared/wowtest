...
{
    boss_loathebAI(Creature *c) : BossAI(c, DATA_LOATHEB) {pInstance = (instance_naxxramas*)c->GetInstanceData();   }
    ...
    {
        if (summon->GetEntry() == 16286)
            summon->CastSpell(summon, SPELL_FUNGAL_BLOOM, true);
    }
    ...
    void JustDied(Unit* Killer)
    {
        ...
        {
            if(Creature *TeleportTrigger = objPlr->SummonTrigger(2909, -4025.02, 273.48, 3.14, 0, nullptr, true))
                TeleportTrigger->SummonGameObject(GO_TELEPORT_NAX_WORKING, 2909, -4025.02, 273.48, 3.14, 0, 0, 0, 0, 0);
        }
    }
