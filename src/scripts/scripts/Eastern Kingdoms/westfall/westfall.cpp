...
//Speech
#define SAY_KILL_HER  -1200307
#define SAY_MOVE      -1200308
#define SAY_BEGIN     -1200309
#define SAY_WAVE      -1200310
#define SAY_END       -1200311
#define SAY_MOVE_BACK -1200312
...
struct npc_daphne_stilwellAI : public npc_escortAI
{
    ...
    void WaypointReached(uint32 i)
    {
        ...
        switch (i)
        {
            case 0:
                ...
                break;
            case 2:
                SetEquipmentSlots(false, EQUIP_NO_CHANGE, EQUIP_NO_CHANGE, 20728);
            ...
            case 3:
                if (!yeller_spawned)
                {
                    ...
                    yeller->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE);
                    yeller->Yell(-1200307, LANG_UNIVERSAL, 0);
                    yeller_spawned = true;
                }
                break;
            case 5:
    ...
    void UpdateAI(const uint32 diff)
    {
        if (real_event_started)
        {
            if (initial_movement)
            {
                DoYell(-1200309, LANG_UNIVERSAL, NULL);
        ...
        {
            me->CastSpell(me->GetVictim(), 6660, false);
            ShootTimer = 2000;
        }
        CheckShooterNoMovementInRange(diff, 3.0);
    ...
    void NextEvent()
    {
        ...
        {
            DoSay(-1200311, LANG_UNIVERSAL, NULL);
            m_creature->HandleEmoteCommand(4);
            DoSay(-1200312, LANG_UNIVERSAL, NULL, true);
            say_end = true;
            real_event_started = false;
        }
        else
        {
            if(thug_wave!=1)
                DoSay(-1200310, LANG_UNIVERSAL, NULL);
...
bool QuestAccept_npc_daphne_stilwell(Player* player, Creature* creature, Quest const* quest)
{
    if (quest->GetQuestId() == QUEST_PROTECT_DAPHNE)
    {
        if (npc_escortAI* pEscortAI = CAST_AI(npc_daphne_stilwellAI, creature->AI()))
        {
            ...
            pEscortAI->DoSay(-1200308, LANG_UNIVERSAL, NULL);
        }
    }
...
CreatureAI* GetAI_npc_daphne_stilwell(Creature *_Creature)
{
    ...
    thisAI->AddWaypoint(5, -11461.2, 1539.06, 52.259, 40000);
