...
struct boss_muruAI : public Scripted_NoMovementAI
{
    ...
    Timer_UnCheked HumanoidStart;
    Timer_UnCheked TransitionTimer;
    bool EntropiusSummoned;
...
struct boss_entropiusAI : public ScriptedAI
{
    ...
    Timer_UnCheked TransitionTimer;
    Timer_UnCheked DarknessTimer;
    Timer_UnCheked BlackHole;
    ...
    void EnterEvadeMode()
    {
		Creature* Muru = (Unit::GetCreature((*me), pInstance->GetData64(DATA_MURU)));
    ...
    {
		Creature* Muru = (Unit::GetCreature((*me), pInstance->GetData64(DATA_MURU)));
			((boss_muruAI*)Muru->AI())->JustSummoned(summoned);
        Summons.Summon(summoned);
    }
    ...
    void JustDied(Unit* killer)
    {
        ...
        // kill all Muru portal npcs to fix combat bug
        std::list<Creature*> portalnpc = FindAllCreaturesWithEntry(25770, 200);
        for (std::list<Creature*>::iterator itr = portalnpc.begin(); itr != portalnpc.end(); ++itr)
            if (*itr)
                (*itr)->Kill((Unit*)*itr, false);
    }
        ...
        if (TransitionTimer.GetInterval())
        {
            if (TransitionTimer.Expired(diff))
            {
                if(Unit* Muru = me->GetUnit(pInstance->GetData64(DATA_MURU)))
                {
                    ...
                    Muru->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE);
                }
                me->SetRooted(false);
...
struct npc_dark_fiendAI : public ScriptedAI
{
    ...
    ScriptedInstance* pInstance;
    Timer_UnCheked ActivationTimer;
    Timer_UnCheked CheckTimer;
...
struct npc_blackholeAI : public ScriptedAI
{
    ...
    ScriptedInstance* pInstance;
    Timer_UnCheked DespawnTimer;
    Timer_UnCheked VisualTimer;
    Timer_UnCheked ActivationTimer;
    Timer_UnCheked ChasingTimer;
    uint64 victimGUID;
