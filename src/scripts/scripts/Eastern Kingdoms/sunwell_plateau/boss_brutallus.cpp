    ...
    void DoIntro()
    {
        ...
        switch (IntroPhase)
        {
            ...
            case 0:
                ...
                pMadrigosa->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE);
            ...
            case 1:
                ...
                // pMadrigosa->GetMotionMaster()->MoveFall(); // might need, might not. Release
            ...
            case 3:
                ...
                break;
            case 4:
                pMadrigosa->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE, false);
        ...
        if (Phase == 1)
        {
            if (_Timer.Expired(diff))
            {
                ...
                if (Unit* pMadrigosa = me->GetUnit(pInstance->GetData64(DATA_MADRIGOSA)))
                {
                    ...
                    else if (!me->IsWithinDist(pMadrigosa, 10.0f) && MoveCounter <= 10)
                    {
                        me->GetNearPoint(x, y, z, 0.0f, 10.0f, me->GetOrientationTo(pMadrigosa));
                        me->GetMap()->CreatureRelocation(me, x, y, z+2, 0);
                        Creature* Trigger = me->SummonTrigger(x, y, z+2, 0, SummonTimer.GetTimeLeft()*4);
                        if(Trigger)
...
struct brutallus_intro_triggerAI : public Scripted_NoMovementAI
{
    ...
    void MoveInLineOfSight(Unit *who)
    {
        ...
        if(!pInstance)
            return;
        if(who->GetTypeId() == TYPEID_PLAYER && me->IsWithinDist(who, 40) && pInstance && pInstance->GetData(DATA_BRUTALLUS_INTRO_EVENT) == NOT_STARTED && pInstance->GetData(DATA_KALECGOS_EVENT) == DONE)
        {
