...
enum SunbladeArchMage
{
    ...
    SPELL_BLINK                     = 46573,
    SPELL_BLINK_TP                  = 55165
};

struct mob_sunblade_arch_mageAI : public ScriptedAI
{
    mob_sunblade_arch_mageAI(Creature *c) : ScriptedAI(c) { 
        pInstance = c->GetInstanceData(); 
        me->SetAggroRange(AGGRO_RANGE);
    }

    Timer_UnCheked ArcaneExplosion;
    Timer_UnCheked FrostNova;
    Timer_UnCheked Blink;

    ScriptedInstance* pInstance;
    void JustDied(Unit* killer)
    {
        pInstance->SetData(DATA_START_TRASH_EVENT, 1);
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
            AddSpellToCast(SPELL_BLINK_TP, CAST_SELF);
...
{
    mob_sunblade_cabalistAI(Creature *c) : ScriptedAI(c), summons(c) { 
        pInstance = c->GetInstanceData(); 
        me->SetAggroRange(AGGRO_RANGE);
    }

    Timer_UnCheked IgniteMana;
    Timer_UnCheked ShadowBolt;
    Timer_UnCheked SummonImp;
    SummonList summons;
...
{
    mob_sunblade_dawn_priestAI(Creature *c) : ScriptedAI(c) {
        pInstance = c->GetInstanceData();
        me->SetAggroRange(AGGRO_RANGE);
    }

    Timer_UnCheked HolyNova;
    Timer_UnCheked Renew;
    Timer_UnCheked SelfRenew;
    bool canRenew;
    bool canSelfRenew;
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if(HealthBelowPct(70.0f) && canSelfRenew)
        {
...
{
    mob_sunblade_dusk_priestAI(Creature *c) : ScriptedAI(c) { 
        pInstance = c->GetInstanceData(); 
        me->SetAggroRange(AGGRO_RANGE);
    }

    Timer_UnCheked Fear;
    Timer_UnCheked WordPain;
...
#define PROTECTOR_YELL      -1200292
#define PROTECTOR_AGGRO_1   -1200293
#define PROTECTOR_AGGRO_2   -1200294
#define PROTECTOR_ACTIVATED -1200295
...
struct mob_sunblade_protectorAI : public ScriptedAI
{
    ...
    {
        if(!isInactive)
            DoYell((urand(0,1) ? -1200293 : -1200294), 0, me);
        else
            DoYell(-1200295, 0, me);
...
{
    mob_sunblade_scoutAI(Creature *c) : ScriptedAI(c) { 
        me->SetAggroRange(30);
        pInstance = c->GetInstanceData();
    }

    Timer_UnCheked SinisterStrike;
    ...
    {
        if(Unit* Protector = GetClosestCreatureWithEntry(me, 25507, 90, true, true))
        {
            if(Protector->GetMotionMaster()->GetCurrentMovementGeneratorType() == IDLE_MOTION_TYPE)
            {
                ...
                Protector->GetNearPoint(x, y, z, 0, urand(10, 20), Protector->GetOrientationTo(me));
                me->SetWalk(false);
...
{
    mob_sunblade_slayerAI(Creature *c) : ScriptedAI(c) { 
        pInstance = c->GetInstanceData();
        me->SetAggroRange(AGGRO_RANGE); 
    }

    Timer_UnCheked ScatterShot;
    Timer_UnCheked Shoot;
    Timer_UnCheked SlayingShot;
...
{
    mob_sunblade_vindicatorAI(Creature *c) : ScriptedAI(c) { 
        pInstance = c->GetInstanceData(); 
        me->SetAggroRange(AGGRO_RANGE);
    }
...
struct mob_shadowsword_assassinAI : public ScriptedAI
{
    ...
    void EnterEvadeMode()
    {
        ...
        if (plList.isEmpty())
            return;
        uint32 AliveCounter = 0;
        uint32 AllPlayers = plList.getSize();
        for (Map::PlayerList::const_iterator i = plList.begin(); i != plList.end(); ++i)
        {
            if (Player* plr = i->getSource())
            {
                ...
                {
                    ++AliveCounter;
                    if (AliveCounter * 4 >= AllPlayers)
                        break;
                }
            }
        }
        if (AliveCounter * 4 < AllPlayers && pInstance->GetData(DATA_TRASH_GAUNTLET_EVENT) == IN_PROGRESS)
        ...
        for (Map::PlayerList::const_iterator i = plList.begin(); i != plList.end(); ++i)
        {
            if (Player* plr = i->getSource())
            {
                if (plr->isGameMaster() || plr->IsFriendlyTo(me))
                    continue;
                if (plr->isAlive() && me->IsWithinDistInMap(plr, 35) && me->IsWithinLOSInMap(plr))
                {
    ...
    void MoveInLineOfSight(Unit *who)
    {
        if (!me->isAlive() || pInstance->GetData(DATA_FELMYST_EVENT) != DONE)
            return;
        if(!me->IsInCombat() && me->IsWithinLOSInMap(who))
            if(!CastedShadowStep)
...
struct mob_shadowsword_commanderAI : public ScriptedAI
{
    ...
    ScriptedInstance* pInstance;
    Timer_UnCheked ShieldSlam;
    Timer_UnCheked Yell_timer;
    Timer_UnCheked ShoutTimer;
    uint64 TriggerGUID;
...
struct mob_shadowsword_deathbringerAI : public ScriptedAI
{
    ...
    Timer_UnCheked DiseaseBuffet;
    Timer_UnCheked VolatileDisease;
...
struct mob_shadowsword_lifeshaperAI : public ScriptedAI
{
    mob_shadowsword_lifeshaperAI(Creature *c) : ScriptedAI(c)
    {
        ...
        // firehex: disables pathfinding for a GUID in SWP 
        if (me->GetGUID() == 288390 || me->GetGUID() == 286016)
    ...
    ScriptedInstance* pInstance;
    Timer_UnCheked DrainLife;
    Timer_UnCheked HealthFunnel;
    bool canFunnelHP;
...
enum ShadowswordSoulbinder
{
    ...
    SPELL_BLAZING_INFERNAL          = 46229,
    SPELL_FELGUARD                  = 46241,
};

struct mob_shadowsword_soulbinderAI : public ScriptedAI
{
    mob_shadowsword_soulbinderAI(Creature *c) : ScriptedAI(c), summons(c)
    {
        me->SetAggroRange(AGGRO_RANGE);
        pInstance = c->GetInstanceData();
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
            AddSpellToCast(RAND(SPELL_BLAZING_INFERNAL, SPELL_FELGUARD), CAST_TANK);
            SummonTimer = 0;
        }
...
struct mob_volatile_fiendAI : public ScriptedAI
{
    ...
    Timer_UnCheked explosion_timer;
    ScriptedInstance* pInstance;
...
struct mob_apocalypse_guardAI : public ScriptedAI
{
    ...
    ScriptedInstance* pInstance;
    Timer_UnCheked Cleave;
    Timer_UnCheked CorruptingStrike;
...
struct mob_cataclysm_houndAI : public ScriptedAI
{
    ...
    Timer_UnCheked CataclysmBreath;
...
struct mob_chaos_gazerAI : public ScriptedAI
{
    ...
    ScriptedInstance* pInstance;
    Timer_UnCheked DrainLifeCD;
    Timer_UnCheked Petrify;
    Timer_UnCheked TentacleSweep;
    bool canDrainLife;
...
struct mob_priestess_of_tormentAI : public ScriptedAI
{
    ...
    {
        if(Aur && Aur->GetId() == SPELL_WHIRLWIND)
        {
            moving = false;
            if(me->GetVictim())
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (moving)
        {
            if (MoveTimer.Expired(diff))
            {
                ...
                {
                    Position pos;
                    target->GetValidPointInAngle(pos, 5.0, 3.14*RAND(0, 1/6, 2/6, 3/6, 4/6, 5/6, 1), true);
...
struct npc_gauntlet_imp_triggerAI : public Scripted_NoMovementAI
{
    ...
    WorldLocation wLoc;
    SummonList summons;
    Timer_UnCheked Deathbringer_timer;
    Timer_UnCheked Imp_timer;
