...
struct instance_sunwell_plateau : public ScriptedInstance
{
    ...
    void OnCreatureCreate(Creature* creature, uint32 entry)
    {
        switch(entry)
        {
            ...
            case 24892:
                ...
                break;
            case 24882:
                Brutallus = creature->GetGUID();
                if (GetData(DATA_KALECGOS_EVENT) != DONE)
                    creature->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE);
                break;
            case 25166:
                Alythess = creature->GetGUID();
                if (GetData(DATA_FELMYST_EVENT) != DONE)
                    creature->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE);
                break;
            case 25165:
                Sacrolash = creature->GetGUID();
                if (GetData(DATA_FELMYST_EVENT) != DONE)
                    creature->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE);
                break;
            case 25741:
                Muru = creature->GetGUID();
    ...
    uint32 GetData(uint32 id)
    {
        switch(id)
        {
            ...
            case DATA_START_TRASH_EVENT:        return TrashKilled;
            //case DATA_OPEN_PATH:                return PathOpened;
        }
    ...
    void SetData(uint32 id, uint32 data)
    {
        switch(id)
        {
            case DATA_KALECGOS_EVENT:
                ...
                if(data == DONE)
                    if (Creature *pBrutallus = GetCreature(GetData64(DATA_BRUTALLUS_EVENT)))
                        pBrutallus->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE, false);
                break;
            ...
            case DATA_FELMYST_EVENT:
                if(data == DONE)
                {
                    ...
                    if (Creature *pAlythess = GetCreature(GetData64(DATA_ALYTHESS)))
                        pAlythess->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE, false);
                }
            ...
            case DATA_EREDAR_TWINS_EVENT:
                ...
                if(data == DONE)
                {
                    HandleGameObject(Gate[0], GO_STATE_ACTIVE);
                    HandleGameObject(Gate[2], GO_STATE_ACTIVE);
                    if (Creature *muru = GetCreature(GetData64(DATA_MURU)))
                    {
                        muru->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                        muru->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE, false);
    ...
    void Load(const char* in)
    {
        ...
        stream >> Encounters[0] >> Encounters[1] >> Encounters[2] >> Encounters[3]
            >> Encounters[4] >> Encounters[5] >> Encounters[6] >> PathOpened;
        for(uint8 i = 0; i < ENCOUNTERS; ++i)
