...
#define GOSSIP_HAH 16196
#define GOSSIP_SAH 16197
...
bool GossipHello_npc_astor_hadren(Player *player, Creature *_Creature)
{
    if (player->GetQuestStatus(1886) == QUEST_STATUS_INCOMPLETE)
        player->ADD_GOSSIP_ITEM( 0, player->GetSession()->GetHellgroundString(GOSSIP_HAH), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
...
bool GossipSelect_npc_astor_hadren(Player *player, Creature *_Creature, uint32 sender, uint32 action)
{
    switch (action)
    {
        case GOSSIP_ACTION_INFO_DEF + 1:
            player->ADD_GOSSIP_ITEM( 0, player->GetSession()->GetHellgroundString(GOSSIP_SAH), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
...
#define NPCSAY_INIT -1230048 // "Get ready, they'll be arriving any minute..." //no blizzlike
#define NPCSAY_END  -1230049 // "Thanks for your help!" //no blizzlike
...
struct pyrewood_ambushAI : public ScriptedAI
{
    ...
    void SummonCreatureWithRandomTarget(uint32 creatureId, int position)
    {
        ...
        if(pSummoned)
        {
            ...
            if (PlayerGUID)
            {
                pPlayer = Unit::GetPlayerInWorld(PlayerGUID);
        ...
        switch(Phase){
            case 0:
                //if(WaitTimer == WAIT_SECS)
                    DoScriptText(NPCSAY_INIT, m_creature, 0); //no blizzlike
            ...
            case 5: //end
                if (PlayerGUID)
                {
                    ...
                    {
                        DoScriptText(NPCSAY_END, m_creature, 0); //no blizzlike
