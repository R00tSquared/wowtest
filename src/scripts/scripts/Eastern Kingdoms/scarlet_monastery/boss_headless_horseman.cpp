...
struct boss_headless_horsemanAI : public ScriptedAI
{
    ...
    void DamageTaken(Unit *done_by, uint32 &damage)
    {
        if (damage >= m_creature->GetHealth() && withhead)
        {
            ...
            {
                Head->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE, false);
                ...
                case 0: {
                    if (!IsFlying)
                    {
                        if (say_timer.Expired(diff))
                        {
                            ...
                            {
                                if (plr)
                                    plr->Say(plr->GetSession()->GetHellgroundString(Text[count]), LANG_UNIVERSAL);
                            }
                            else
                            {
                                DoCast(m_creature, SPELL_RHYME_BIG);
