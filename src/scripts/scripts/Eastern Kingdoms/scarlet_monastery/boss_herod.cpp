...
struct boss_herodAI : public ScriptedAI
{
    ...
    {
        Enrage = false;
        _ChangedNameCleave_Timer.Reset(12000);
        _ChangedNameWhirlwind_Timer.Reset(60000);
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (!Enrage && me->GetHealthPercent() <= 30 && !m_creature->IsNonMeleeSpellCast(false))
        {
        ...
        if (_ChangedNameWhirlwind_Timer.Expired(diff))
        {
            ...
            _ChangedNameWhirlwind_Timer = 30000;
        }
