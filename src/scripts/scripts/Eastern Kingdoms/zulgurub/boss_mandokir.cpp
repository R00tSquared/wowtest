            ...
            switch (eventId)
            {
                ...
                case EVENT_WATCH_END:
                {
                    if (Unit* target = me->GetUnit(WatchedTarget))
                    {
                        ...
                        // target must not move, must not cast and must not attack
                        if (tPos != currentPos && target->isAlive() && (target->HasUnitState(UNIT_STAT_CASTING) || target->IsNonMeleeSpellCast(false) || target->HasUnitState(UNIT_STAT_MELEE_ATTACKING)))
                        {
