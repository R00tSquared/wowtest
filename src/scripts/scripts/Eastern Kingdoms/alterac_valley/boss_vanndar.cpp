...
struct boss_vanndarAI : public ScriptedAI
{
    ...
    Timer_UnCheked AvatarTimer;
    Timer_UnCheked ThunderclapTimer;
    Timer_UnCheked StormboltTimer;
        ...
        if (!announced && me->GetHealthPercent() < 90)
        {
            ...
            if (unit)
            {
                ...
                if (player)
                {
                    ...
                    if (bg)
                    {
                        bg->SendNotifyToTeam(ALLIANCE, 16734);
                        bg->PlaySoundToTeam(10843, ALLIANCE);
                        announced = true;
                    }
                }
            }
