...
{
    sLog.outString("---------------------------------------");
    sLog.outString("            STARTING SERVER            ");
    sLog.outString("---------------------------------------");
    sLog.outString("REVISION: %s", REVISION);
    sLog.outString("REVISION_DATE: %s", REVISION_DATE);
    sLog.outString();

    // print current working direcroty
    char buffer[FILENAME_MAX];
    if (getcwd(buffer, FILENAME_MAX) != NULL) {
        sLog.outString("--- Current Working Directory: %s", buffer);
    }
    ...
    // check MySQL transactions before we start because it can be broken due to wrong lib
    uint32 test_spell = 99999;

    static SqlStatementID checkTransaction;
    RealmDataDatabase.BeginTransaction();
    SqlStatement stmt = RealmDataDatabase.CreateStatement(checkTransaction, "INSERT INTO spell_disabled (entry) values (?)");
    stmt.PExecute(test_spell);
    RealmDataDatabase.CommitTransaction();

    QueryResultAutoPtr check = RealmDataDatabase.PQuery("SELECT 1 FROM `spell_disabled` WHERE `entry` = %u", test_spell);
    if (!check)
    {
        sLog.outString("MySQL transactions may be broken, can't continue! Using debig lib instead release?");
        exit(1);
    }
    else
        RealmDataDatabase.PQuery("DELETE FROM spell_disabled WHERE `entry` = %u", test_spell);
    ...
    // set realmbuilds depend on realm expected builds, and set server online
    {
        ...
        AccountsDatabase.DirectPExecute("UPDATE realms SET flags = flags & ~(%u), allowed_builds = '%s'  WHERE realm_id = '%u'", REALM_FLAG_OFFLINE, builds.c_str(), realmID);
    }
    ...
    sLog.outString("SERVER STARTUP TIME: %u seconds (%u ms)", (uStartInterval % 60000) / 1000, uStartInterval);
...
void Master::_OnSignal(int s)
{
    switch (s)
    {
        ...
        case SIGABRT:
        {
            ...
            {
                sLog.outLog(LOG_CRASH, "CRASH[%i]: mapid: %u, instanceid: %u", s, mapUpdateInfo->GetId(), mapUpdateInfo->GetAnyInstanceId());
                sLog.outLog(LOG_CRASH, "\r\n************ BackTrace *************\r\n%s\r\n***********************************\r\n", stackTrace.c_str());

                if (Map *map = sMapMgr.FindMap(mapUpdateInfo->GetId(), mapUpdateInfo->GetAnyInstanceId()))
                    map->SetBroken(true);
