    ...
    namespace XP
    {
        ...
        inline uint32 Gain(Player *pl, Unit *u)
        {
            if (u->GetTypeId()==TYPEID_UNIT && (
                (((Creature*)u)->isArmorer() || ((Creature*)u)->isServiceProvider() ||
            ...
            if (u->GetTypeId()==TYPEID_UNIT)
            {
                xp_gain *= ((Creature*)u)->GetXPMod();
                if (((Creature*)u)->isElite())
                    xp_gain *= 2;
            }

            float expCalc = xp_gain * pl->GetXPRate(RATE_XP_KILL);
            uint32 exp = (uint32)(expCalc);

			if (sLog.IsLogEnabled(LOG_EXP))
			{
				CreatureInfo const * tmpInfo = ObjectMgr::GetCreatureTemplate(u->GetEntry());
				sLog.outLog(LOG_EXP, "Exp calculation for Player %s (guid %u) and Unit %s (id %u): exp: %u, plLvl: %u, uLvl: %u, XPRate: %f, XPMod: %f (template: %f), exp calculated: %f, exp after cast: %u, xp_gain: %u",
					pl->GetName(), pl->GetGUIDLow(), u->GetName(), u->GetEntry(), exp, pl->GetLevel(), u->GetLevel(), pl->GetXPRate(RATE_XP_KILL), u->GetTypeId() == TYPEID_UNIT ? ((Creature*)u)->GetXPMod() : 1.0f, tmpInfo ? tmpInfo->xpMod : -1.0f, expCalc, xp_gain);
			}
