...
static const class staticActionInfo
{
    ...
    staticActionInfo()
    {
        ...
        actionInfo[UNIT_ACTION_DISTRACT](UNIT_ACTION_PRIORITY_DISTRACT,ACTION_TYPE_NONRESTOREABLE);
...
public:
    ...
    void Initialize(Unit &u)
    {
        ...
        if (target->ToPlayer() && target->IsInWorld()) // ToPlayer() meaning IsMovedByPlayer()
        {
            target->SendRootedToMover(target, true);
            target->SendRootedToObservers(target, true);
        }
        else
            target->SendRootedToAll(target, true);
    ...
    void Finalize(Unit &u)
    {
        ...
        if (target->GetTypeId() == TYPEID_PLAYER)
            target->SetSelection(((Player*)target)->GetSavedSelection()); // restore target

        if (target->ToPlayer() && target->IsInWorld()) // ToPlayer() meaning IsMovedByPlayer()
        {
            target->SendRootedToMover(target, false);
            target->SendRootedToObservers(target, false);
        }
        else
            target->SendRootedToAll(target, false);
...
public:
    ...
    void Initialize(Unit &u)
    {
        ...
        //target->SetSelection(0); // why should we remove target on root state? no reason for that

        target->StopMoving();

        //Save last orientation
        if (target->GetVictim())
            target->SetOrientation(target->GetOrientationTo(target->GetVictim()));
    ...
    bool Update(Unit &unit, const uint32 &)
    {
        if (unit.HasUnitState(UNIT_STAT_CONFUSED_NOT_MOVING | UNIT_STAT_FLEEING_NOT_MOVING)) // root state is higher than confuse and fear, must check and set no target here
            unit.SetSelection(0);
        return true;
    }
    ...
    void Finalize(Unit &u)
    {
        ...
        if (target->ToPlayer() && target->IsInWorld()) // ToPlayer() meaning IsMovedByPlayer()
        {
            ...
            target->SetSelection(((Player*)target)->GetSavedSelection()); // restores target if player was confused or feared, should do nothing if there was only root
        }
        else
        {
            target->SendRootedToAll(target, false);

            if (((Creature*)target)->hasIgnoreVictimSelection())
        ...
        //    target->SetSelection(((Player*)target)->GetSavedSelection());
        //}
        //else
        //{
...
public:
    ...
    void Finalize(Player &u)
    {
        ...
        else if (u.IsPvP() && !u.HasFlag(PLAYER_FLAGS,PLAYER_FLAGS_IN_PVP))
            u.UpdatePvP(false);
