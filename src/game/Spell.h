...
class Spell
{
    ...
    public:
        ...
        void DoEffectSummonWild(uint32 i, bool DieWithSummoner = false);
        ...
        void EffectFriendSummon(uint32 i);
        ...
        int32 CalculateDamage(uint8 i) { return m_caster->CalculateSpellDamage(GetSpellEntry(),i,m_currentBasePoints[i]); }
        ...
        /* Determines targets / AoE targets
         * Careful calling if before cast end, cause decides hit result at call*/
        void FillTargetMap();
        ...
        //void SendSpellCooldownPacket();
        void SendLogExecute();
        void SendInterrupted(uint8 result);
        void SendChannelUpdate(uint32 time);
        void SendChannelStart(uint32 duration, bool initial);
        ...
        bool IsAutoRepeatStart() const { return m_autoRepeat && !m_IsTriggeredSpell; }
        bool IsAutoRepeatTrigger() const { return m_autoRepeat && m_IsTriggeredSpell;  }
        ...
        bool CastableOnMove() const
        {
            return GetSpellEntry()->AttributesCu & SPELL_ATTR_CU_ON_MOVE;
        }
        ...
        void InterruptSpellOnMove(bool start = false);
        void ProcessFrontLeap(uint32 i);
    ...
    protected:
        ...
        bool DoSpellHitOnUnit(Unit *unit, uint32 effectMask);
        ...
        bool initial_channell; // decides if the channelling cast is initial (that has no aura after it) or not
            ...
            for (typename GridRefManager<T>::iterator itr = m.begin(); itr != m.end(); ++itr)
            {
                ...
                switch (i_push_type)
                {
                    case PUSH_IN_FRONT:
                        ...
                        break;
                    case PUSH_IN_FRONT_WIDE:
                        if (i_caster->isInFront((Unit*)(itr->getSource()), i_radius, M_PI/6*4))
                    ...
                    case PUSH_IN_LINE:
                        ...
                        break;
                    default:
                        if ((itr->getSource()->GetExactDistSq(i_x, i_y, i_z) < i_radiusSq))
                            i_data->push_back((GameObject*)itr->getSource());
                        break;
                }
            }
            ...
            for (typename GridRefManager<T>::iterator itr = m.begin(); itr != m.end(); ++itr)
            {
                ...
                switch (i_push_type)
                {
                    ...
                    case PUSH_IN_LINE:
                        ...
                        break;
                    default:
                        //if (i_TargetType != SPELL_TARGETS_ENTRY && i_push_type == PUSH_SRC_CENTER && i_caster)
                        //{
                        //        if (i_caster->GetDistanceSq(itr->getSource()) < i_radiusSq)
                        //            i_data->push_back(itr->getSource());
                        //}
                        //else
                        //{
                        ...
                        {
                            if (i_caster->IsWithinDistInMapForSrcCaster(itr->getSource(), i_radius))
                                i_data->push_back(itr->getSource());
                        }
                        else
                        {
