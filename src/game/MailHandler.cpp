...
void WorldSession::HandleSendMail(WorldPacket & recv_data)
{
	...
	if (!processChatmessageFurtherAfterSecurityChecks(subject, LANG_UNIVERSAL))
		return;

	if (ChatHandler(_player).ContainsNotAllowedSigns(subject))
		return;

    std::string to = "Send_Mail";
    sChatLog.WhisperMsg(GetPlayer(), to, subject);

    if (subject == "Lexics Cutter Detected Bad Words")
        return;
	...
	uint32 account = GetPlayer()->GetSession()->GetAccountId();
	auto it = sWorld.account_mail_limit.find(account);
	if (it != sWorld.account_mail_limit.end())
		++sWorld.account_mail_limit[account];
	else
		sWorld.account_mail_limit[account] = 1;

	// after 5 mails set empty text
	bool fill_empty_text = (sWorld.account_mail_limit[account] > 5) ? true : false;
    ...
    if (!body.empty())
    {
        if (!processChatmessageFurtherAfterSecurityChecks(body, LANG_UNIVERSAL))
            return;

        if (ChatHandler(_player).ContainsNotAllowedSigns(body))
            return;

        std::string to = "Send_Mail";
        sChatLog.WhisperMsg(GetPlayer(), to, body);

        if (body == "Lexics Cutter Detected Bad Words")
            return;
    }
    ...
    if (money >= MAX_MONEY_AMOUNT || COD >= MAX_MONEY_AMOUNT)
        return;
    ...
    Player *receive = sObjectMgr.GetPlayerInWorld(rc);
    ...
    if (mails_count > 100)
    {
		ChatHandler(GetPlayer()).PSendSysMessage(16596);
	...
	if (fill_empty_text)
	{
		subject = "...";
		body.clear();
		ChatHandler(GetPlayer()).PSendSysMessage(16585);
	}
    ...
    if (items_count > 0 || money > 0)
    {
        if (items_count > 0)
        {
            for(uint8 i = 0; i < items_count; ++i)
            {
				...
				pl->MoveItemFromInventory(item->GetBagSlot(), item->GetSlot(), true, "MAIL");
...
void WorldSession::HandleMailDelete(WorldPacket & recv_data)
{
    ...
    if (m)
    {
        ...
        if (m->HasItems())
        {
            for (MailItemInfoVec::iterator itr = m->items.begin(); itr != m->items.end(); ++itr)
            {
                if (Item* item = pl->GetMItem(itr->item_guid))
                    items += std::to_string(itr->item_template) + ":" + std::to_string(item->GetCount()) + " ";
            }
        ...
        sLog.outLog(LOG_CHARACTER_ACTIONS, "#mail Player %s (guid %u) removed mail with items %s and money %u", pl->GetName(), pl->GetGUIDLow(), items.c_str(), m->money);
...
void WorldSession::HandleReturnToSender(WorldPacket & recv_data)
{
	...
	{
		pl->SendMailResult(mailId, MAIL_RETURNED_TO_SENDER, MAIL_ERR_RECIPIENT_NOT_FOUND);
		return;
	}
    ...
    if (m->messageType == MAIL_NORMAL && m->sender)
    {
        ...
        sLog.outLog(LOG_CHARACTER_ACTIONS, "#mail Player %s (guid %u) mail returned to sender (guid %u) with items %s and money %u", pl->GetName(), pl->GetGUIDLow(), m->sender, items.c_str(), m->money);
...
void WorldSession::HandleTakeItem(WorldPacket & recv_data)
{
    ...
    if (msg == EQUIP_ERR_OK)
    {
        ...
        if (m->COD > 0)                                     //if there is COD, take COD money from player and send them to sender by mail
        {
            ...
            Player *sender = sObjectMgr.GetPlayerInWorld(sender_guid);
        ...
        // remove mails after 1 day if all items was taken
        if (m->items.size() == 0 && m->money == 0)
            m->expire_time = time(NULL) + DAY;
		...
		//sLog.outLog(LOG_CHARACTER_ACTIONS, "#item Player %s (guid %u) took item from mail %u (count %u), COD %u", pl->GetName(), pl->GetGUIDLow(), it->GetEntry(), count, cod);
    }
    else
...
void WorldSession::HandleTakeMoney(WorldPacket & recv_data)
{
    ...
    if (m->money + _player->GetMoney() >= MAX_MONEY_AMOUNT)
    {
        pl->SendEquipError(EQUIP_ERR_TOO_MUCH_GOLD,NULL,NULL);
        pl->SendMailResult(mailId, MAIL_MONEY_TAKEN, MAIL_ERR_INTERNAL_ERROR);
        return;
    }
    ...
    pl->_SaveMail();
    RealmDataDatabase.CommitTransaction();
	sLog.outLog(LOG_CHARACTER_ACTIONS, "#money Player %s (guid %u) took money from mail %u", pl->GetName(), pl->GetGUIDLow(), money);
}
