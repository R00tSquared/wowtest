...
PathFinder::PathFinder(const Unit* owner) :
    ...
    m_useStraightPath(false), m_forceDestinationForIncomplete(false), m_pointPathLimit(MAX_POINT_PATH_LENGTH),
    m_sourceUnit(owner), m_navMesh(NULL), m_navMeshQuery(NULL), startEndDiffTerrain(false)
{
    ...
    if (m_sourceUnit->GetTerrain() && m_sourceUnit->GetTerrain()->IsPathFindingEnabled())
    {
        ...
        m_navMeshQuery = mmap->GetNavMeshQuery(mapId, m_sourceUnit->GetAnyInstanceId());
    }
...
bool PathFinder::calculate(float destX, float destY, float destZ, bool forceDestForIncomplete)
{
    float x, y, z;
    ...
    m_forceDestinationForIncomplete = forceDestForIncomplete;

    updateFilter();

    // Old path is only in TargetedMovementGeneratorMedium, everywhere else each time new Path is created
    // Check if target has moved
    /*float dist = m_sourceUnit->GetObjectBoundingRadius();
    if (inRange(oldDest, dest, dist, dist))
    {
        sWorld.SendServerMessage(SERVER_MSG_SHUTDOWN_TIME, "Enjoy precalculated path???!");
        Vector3 currDest = m_sourceUnit->movespline->CurrentDestination();
        while (m_pathPoints.size() > 2) // at the worst we will leave only end-destination
        {
            if (m_pathPoints.front() == currDest)
                break; // remove all previous points up to the one we're running to at the moment
            
            m_pathPoints.erase(m_pathPoints.begin());
            sWorld.SendServerMessage(SERVER_MSG_SHUTDOWN_TIME, "Enjoy precalculated path removed point");
        }
        return true;
    }
    else
    {*/
        ...
        // Test how often does it calculate the path
        // sWorld.SendServerMessage(SERVER_MSG_SHUTDOWN_TIME, "Enjoy NEW path???!");
...
dtPolyRef PathFinder::getPolyByLocation(const float* point, float *distance) const
{
    ...
    if(polyRef != INVALID_POLYREF && (*distance < VALID_DISTANCE_TO_POLY)) // here we check by the path we had
        return polyRef;
...
void PathFinder::BuildPolyPath(const Vector3 &startPos, const Vector3 &endPos)
{
    ...
    if (startPoly == INVALID_POLYREF || endPoly == INVALID_POLYREF)
    {
        ...
        m_type = FlyOrWaterShortCut() ? PathType(PATHFIND_NORMAL | PATHFIND_NOT_USING_PATH) : PATHFIND_NOPATH;
        return;
    }

    // we may need a better number here
    bool farFromPoly = distToStartPoly > VALID_DISTANCE_TO_POLY || distToEndPoly > VALID_DISTANCE_TO_POLY;
    if (farFromPoly)
    {
        //DEBUG_FILTER_LOG(LOG_FILTER_PATHFINDING, "++ BuildPolyPath :: farFromPoly distToStartPoly=%.3f distToEndPoly=%.3f\n", distToStartPoly, distToEndPoly);

        if (FlyOrWaterShortCut())
        {
            BuildShortcut();
            m_type = PathType(PATHFIND_NORMAL | PATHFIND_NOT_USING_PATH);
            return;
    ...
    } // This part of code makes Z to change to broken - if FarFromPoly, but WHY FAR FROM POLY???
...
void PathFinder::BuildPointPath(const float *startPoint, const float *endPoint)
{
    ...
    {
        BuildShortcut();
        m_type = PathType(PATHFIND_NOPATH | PATHFIND_SHORT);
        return;
    }
    ...
    if (startEndDiffTerrain)
        ConnectInWaterPoints();
    ...
    // force the given destination, if needed. This only forces PATHFIND_INCOMPLETE to end where needed, this will not affect not-found paths or too-long paths
    if(m_forceDestinationForIncomplete &&
        (!(m_type & PATHFIND_NORMAL) || !inRange(getEndPosition(), getActualEndPosition(), 2.0f, 2.0f)))
    {
...
void PathFinder::NormalizePath()
{
    for (uint32 i = 0; i < m_pathPoints.size(); ++i)
        // Some caves underground have no VMaps and so creatures run towards ceiling. 
        // Can't get creatures out of underground for that reason...
        //m_sourceUnit->UpdateAllowedPositionZ_WithUndermap(m_pathPoints[i].x, m_pathPoints[i].y, m_pathPoints[i].z); // poly height might be over or under a little
        m_sourceUnit->UpdateAllowedPositionZ(m_pathPoints[i].x, m_pathPoints[i].y, m_pathPoints[i].z); // poly height might be over or under a little
}
...
void PathFinder::updateFilter()
{
    ...
    // Trentone --- Optimization --- IsSwimming() is a CPU-hungry method (like LOS). Try to optimize.
    // I don't think we even need this, includedFlags already have NAV types needed if creature can swim, and if it cannot swim it shouldn't have them?
    // Why should we let creature cheat?..

    // A creature will get out of combat instantly by IsOutOfThreatArea, there's actually no point for this. Pets can swim by default. Others will evade.
    // Is there a situation where creature can get into water by accident and still should be able to move?
    uint16 includedFlags = m_filter.getIncludeFlags();
    if (!(includedFlags & NAV_WATER)) // only check IsSwimming() if we dont already have WATER flags
    {
        if (m_sourceUnit->IsSwimming())
        {
            includedFlags |= (NAV_WATER | NAV_MAGMA | NAV_SLIME);           // swim
...
bool PathFinder::HaveTile(const Vector3 &p) const // Trinity has it a little different
{
    int tx, ty;
...
bool PathFinder::FlyOrWaterShortCut()
{
    /*If able to fly*/
    if ((m_sourceUnit->GetTypeId() == TYPEID_UNIT && m_sourceUnit->ToCreature()->CanFly()) ||
        (m_sourceUnit->GetTypeId() == TYPEID_PLAYER && m_sourceUnit->ToPlayer()->IsFlying()))
        return true;

    Vector3 end = getActualEndPosition();
    bool startInWater = ((m_sourceUnit->GetTypeId() == TYPEID_UNIT && m_sourceUnit->ToCreature()->CanSwim() && m_sourceUnit->IsSwimming()) ||
                         (m_sourceUnit->GetTypeId() == TYPEID_PLAYER && m_sourceUnit->ToPlayer()->IsSwimming()));
    bool endInWater = m_sourceUnit->GetTerrain()->IsInWater(end.x, end.y, end.z);
    startEndDiffTerrain = startInWater != endInWater;    
    /*If currently is in water and end point is in water*/
    return startInWater && endInWater;
}

void PathFinder::ConnectInWaterPoints()
{
    const TerrainInfo* t = m_sourceUnit->GetTerrain();
    if (!t)
        return;

    // first point on ground, in best case other two are in water, but they're already straight by definition
    if (m_pathPoints.size() < 4)
        return;

    // we are searching for in-water point starting from the ground, and then connect the water point to it
    bool endInWater = t->IsInWaterOrSlightlyAbove(m_pathPoints[m_pathPoints.size()-1].x, m_pathPoints[m_pathPoints.size()-1].y, m_pathPoints[m_pathPoints.size()-1].z);
    if (endInWater) // start from start, start is on ground
    { 
        // Except last two points, cause if they are in water, then nothing to do, nothing to shortcut
        for (uint32 i = 0; i < (m_pathPoints.size() - 2); ++i)
        {
            if (t->IsInWaterOrSlightlyAbove(m_pathPoints[i].x, m_pathPoints[i].y, m_pathPoints[i].z))
            {
                //m_pathPoints[i + 1] = m_pathPoints[m_pathPoints.size() - 1];
                // setting to end point and not the generated end point, because the generated end point is by poly -> which is on the surface of water
                m_pathPoints[i + 1] = getEndPosition();
    ...
    {
        // Except first two points, cause if they are in water, then nothing to do, nothing to shortcut
        for (uint32 i = m_pathPoints.size() - 1; i > 1; --i)
        {
            if (t->IsInWaterOrSlightlyAbove(m_pathPoints[i].x, m_pathPoints[i].y, m_pathPoints[i].z))
            {
                m_pathPoints[i-1] = m_pathPoints[0];

                uint32 newSize = m_pathPoints.size() - (i - 1);
                for (uint32 j = 0; j < newSize; ++j)
                    m_pathPoints[j] = m_pathPoints[i - 1 + j];

                m_pathPoints.resize(newSize);
                break;
            }
        }
    }
