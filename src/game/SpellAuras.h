...
class HELLGROUND_IMPORT_EXPORT Aura
{
    ...
    public:
        ...
        void HandleLongSight(bool Apply, bool Real);
        ...
        void HandleAuraPermanentTransform(bool apply, bool Real);
        void HandleSendSpellVisualPeriodic(bool apply, bool Real);
        void HandleSpectatorVisible(bool apply, bool Real);
        void HandleAuraXpPct(bool apply, bool Real);
        void HandleAuraRepPct(bool apply, bool Real);
        ...
        void SetLoadedState(uint64 caster_guid,int32 damage,int32 maxduration,int32 duration,int32 charges,uint64 castItemGuid = 0)
        {
        ...
        bool IsSpellFlightShapeshift() const
        {
            return (GetId() == 33943 ||
                GetId() == 40120);
            /*if (GetSpellProto()->Id == 33943)
                return true;
            if (GetSpellProto()->Id == 40120)
                return true;
            return false;*/
        }
        ...
        bool IsSingleTarget() const { return m_isSingleTargetAura; } // like Polymorph, Blind
        void SetIsSingleTarget(bool val) { m_isSingleTargetAura = val; }

        AuraSpecialFlag special_flag;

        void SetPassive(bool val) { m_isPassive = val; };
        void SetPeriodic(bool val) { m_isPeriodic = val; }

        bool IsStealed() { return m_isStealed; }
        void SetStealed(bool val) { m_isStealed = val; }
    ...
    protected:
        ...
        bool m_isStealed:1;                                 // moonwell: aura is stealed by Spellsteal or other similar spell, don't add cooldown after using it
        ...
        int32 m_stackAmount;
    private:
        void SetAura(uint32 slot, bool remove) { m_target->SetUInt32Value(UNIT_FIELD_AURA + slot, remove ? 0 : sSpellMgr.GetSpellAnalog(GetSpellProto())); }
