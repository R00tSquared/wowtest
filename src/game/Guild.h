...
class Guild
{
    public:
        ...
        void BroadcastToGuild(WorldSession *session, const std::string& msg, uint32 language, bool bad_lexics = false);
        void BroadcastToOfficers(WorldSession *session, const std::string& msg, uint32 language, bool bad_lexics = false);
        ...
        void BroadcastWorker(Do& _do, Player* except = NULL)
        {
            for(MemberList::iterator itr = members.begin(); itr != members.end(); ++itr)
                if(Player *player = ObjectAccessor::GetPlayerInWorld(MAKE_NEW_GUID(itr->first, 0, HIGHGUID_PLAYER)))
                    if(player != except)
                        _do(player);
        }
        ...
        void   LogBankEvent(uint8 LogEntry, uint8 TabId, Player* pl, uint32 ItemOrMoney, uint8 ItemStackCount=0, uint8 DestTabId=0);
        void   RenumBankLogs();
