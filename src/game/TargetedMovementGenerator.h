...
#ifndef HELLGROUND_TARGETEDMOVEMENTGENERATOR_H
#define HELLGROUND_TARGETEDMOVEMENTGENERATOR_H
...
protected:
	TargetedMovementGeneratorMedium(Unit &target, float offset, float angle) :
		...
		_targetReached(false), _recheckTimer(0),
		_path(NULL), m_fTargetLastX(0), m_fTargetLastY(0), m_fTargetLastZ(0), CantReachEvadeTimer(0)
	{
	}
...
protected:
	...
	float GetVelocitySpeed(T &);

	TimeTracker _recheckTimer;
	float _offset;
	float _angle;
	bool _targetReached : 1;
	uint32 CantReachEvadeTimer; // doesn't reset, evade after 10 sec
