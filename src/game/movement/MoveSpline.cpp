...
Location MoveSpline::ComputePosition() const
{
    ...
    bool facing2 = splineflags.isFacing();
    ...
    else
    {
        ...
        // Gensen: in some cases c.oreintation can be negative, so it should be fixed
        if (c.orientation < 0)
            c.orientation = M_PI * 2 - abs(c.orientation);
    }
    return c;
}
...
void MoveSpline::init_spline(const MoveSplineInitArgs& args)
{
    ...
    {
        sLog.outLog(LOG_DEFAULT, "ERROR: MoveSpline::init_spline: zero length spline, wrong input data?");
...
#define CHECK(exp) \
    if (!(exp))\
    {\
        sLog.outLog(LOG_DEFAULT, "ERROR: MoveSplineInitArgs::Validate: expression '%s' failed for Entry: %u", #exp, unit->GetEntry());\
        return false;\
    }
    CHECK(path.size() > 1);
    CHECK(velocity > 0.0f);
...
bool MoveSplineInitArgs::_checkPathBounds() const
{
    if (!(flags & MoveSplineFlag::Mask_CatmullRom) && path.size() > 2)
    {
        ...
        for (uint32 i = 1; i < path.size()-1; ++i)
        {
            ...
            {
                sLog.outLog(LOG_DEFAULT, "ERROR: MoveSplineInitArgs::_checkPathBounds check failed");
                return false;
            }
        }
    }
