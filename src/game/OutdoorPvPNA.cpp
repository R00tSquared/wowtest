...
void OPvPCapturePointNA::SpawnNPCsForTeam(PlayerTeam team)
{
...
void OPvPCapturePointNA::SpawnGOsForTeam(PlayerTeam team)
{
...
void OPvPCapturePointNA::FactionTakeOver(PlayerTeam team)
{
    ...
    if (m_ControllingFaction == ALLIANCE)
        sWorld.SendZoneText(NA_HALAA_GRAVEYARD_ZONE,sObjectMgr.GetHellgroundStringForDBCLocale(LANG_OPVP_NA_LOOSE_A));
    else if (m_ControllingFaction == HORDE)
        sWorld.SendZoneText(NA_HALAA_GRAVEYARD_ZONE,sObjectMgr.GetHellgroundStringForDBCLocale(LANG_OPVP_NA_LOOSE_H));
    ...
    else
    {
        ...
        m_PvP->SendUpdateWorldState(NA_UI_GUARDS_LEFT, m_GuardsAlive);
        sWorld.SendZoneText(NA_HALAA_GRAVEYARD_ZONE,sObjectMgr.GetHellgroundStringForDBCLocale(LANG_OPVP_NA_CAPTURE_H));
    }
...
bool OPvPCapturePointNA::HandleCustomSpell(Player * plr, uint32 spellId, GameObject * /*go*/)
{
    ...
    if (retval)
    {
        ...
        Item* item = plr->StoreNewItem(dest, itemid, true, 0, "CUSTOM_SPELL");
