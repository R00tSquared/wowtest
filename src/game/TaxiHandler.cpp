...
void WorldSession::SendTaxiMenu(Creature* unit)
{
    ...
    data << uint32(curloc);
    GetPlayer()->m_taxi.AppendTaximaskTo(data,GetPlayer()->IsPlayerCustomFlagged(PL_CUSTOM_TAXICHEAT));
    SendPacket(&data);
...
void WorldSession::HandleMoveSplineDoneOpcode(WorldPacket& recv_data)
{
    ...
    if (!curDest)
    {
        ...
        {
            GetPlayer()->Relocate(movementInfo.GetPos()->x, movementInfo.GetPos()->y, movementInfo.GetPos()->z);
            GetPlayer()->m_movementInfo = movementInfo;
            GetPlayer()->m_anti_lastmovetime = movementInfo.time;
        }
        if (GetPlayer()->GetDummyAura(54839))
        {
            if (!GetPlayer()->GetCharm())
                GetPlayer()->Say("TaxiHandler did setposition", LANG_UNIVERSAL);
            else 
                GetPlayer()->Say("TaxiHandler didn't setposition", LANG_UNIVERSAL);
        }
        // end anticheat
        return;
    }
    ...
    if (destinationnode > 0)                              // if more destinations to go
    {
        ...
        if (GetPlayer()->IsPlayerCustomFlagged(PL_CUSTOM_TAXICHEAT))
        {
    ...
    /*if(curDest == 95 && GetPlayer()->GetQuestStatus(9718) == QUEST_STATUS_INCOMPLETE)
    {
        Creature* target = NULL;
        Hellground::NearestCreatureEntryWithLiveStateInObjectRangeCheck check(*_player, 17841, true, 10.0f, false);
        ...
        GetPlayer()->GetMap()->ScriptsStart(sEventScripts, 25000, target, _player);
        GetPlayer()->Say("TaxiHandler", LANG_UNIVERSAL);
    }*/
