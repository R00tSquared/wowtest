...
enum CreatureFlagsExtra
{
    ...
    CREATURE_FLAG_EXTRA_NO_COMBAT_SKILL_UPDATE = 0x10000000,    // 268435456 - weapon/defense skill is never increased by fighting this creature
    CREATURE_FLAG_REACT_TO_STAMINA_BUFF     = 0x20000000,       // 536870912
};
...
#define HEROIC_RAIDS_ENTRY_START 9300000
...
class HELLGROUND_IMPORT_EXPORT Creature : public Unit
{
    public:
        ...
        /*Same as ForcedDespawn()*/
        void DisappearAndDie();

        bool Create(uint32 guidlow, Map *map, uint32 Entry, PlayerTeam team, float x, float y, float z, float ang, const CreatureData *data = NULL, GameEventCreatureData const* eventData = NULL);
        bool LoadCreaturesAddon(bool reload = false);
        void SelectLevel(const CreatureInfo *cinfo);
        void LoadEquipment(uint32 equip_entry, bool force=false);

        /*Pets DO have DBTableGuid as well as creatures, be careful at using that*/
        ...
        /*Sends appropriate npc text error message and returns true if BG should not be available by level restriction, otherwise returns false (no gossip sent)*/
        bool sendBgNotAvailableByLevel(Player* pPlayer, BattleGroundTypeId bgTypeId) const;
        bool isCanTrainingAndResetTalentsOf(Player* pPlayer) const;
        bool IsOutOfThreatArea(Unit* pVictim) const;
        bool IsImmunedToSpell(SpellEntry const* spellInfo, bool includeCharges = true);
        ...
        bool UpdateEntry(uint32 entry, PlayerTeam team=ALLIANCE, const CreatureData* data=NULL, GameEventCreatureData const* eventData = NULL);
        ...
        /*used by npc modification GM commands*/
        virtual void SaveUpdateToDB();                            // overwrited in TemporarySummon (to do nothing)
                                                            // overwrited in Pet and TemporarySummon
        /*MUST ONLY be called by SaveUpdateToDB() or .npc add command. Any other usage is FORBIDDEN*/
        virtual void SaveUpdateOrNewToDB_npc_add(uint32 mapid, uint8 spawnMask);
        /*Must ONLY be called by ".npc del" command. Any other usage is FORBIDDEN*/
        virtual void DeleteFromDB_npc_del();                        // overwrited in Pet
        ...
        bool IsPlayerAllowedToLoot(Player *player) const;
        void FillPlayersAllowedToLoot(std::set<uint64> *s) const { *s = m_PlayersAllowedToLoot; }
        //void FillMotherAccsAllowedToLoot(std::set<uint32> *s) const { *s = m_MotherAccsAllowedToLoot; }

        void FillPlayersAllowedToLootAfterLoad(std::set<uint64> *s) { m_PlayersAllowedToLoot = *s; }
        //void FillMotherAccsAllowedToLootAfterLoad(std::set<uint32> *s) { m_MotherAccsAllowedToLoot = *s; }
		...
		uint8 ReachForAutoAttackErrorCode(Unit* u, bool isReachable);

        float GetAttackDistance(Unit const* pl) const;

        /* dist MUST be lower or same as AggroRange,
        because canStartAttack takes AggroRange in consideration when searching for available targets.*/
        ...
        void Respawn();
        void DynamicRespawnMod(uint32 &baseDelay) const;
        ...
        void SetPlayerDamageReqInPct(float pct) { m_PlayerDamageReq = GetMaxHealth() * pct; }
        ...
        void SetIsDistanceToHomeEvadable(bool b) { IsDistancToHomeEvadable = b; } // USE ONLY for timing mobs or event purposes.
        ...
        float GetXPMod() const { return m_xpMod; }
        float GetHPMod() const { return m_hpMod; } // 1.2 meaning +20%, creature_template assigned
        float GetDMGMod() const { return m_dmgMod; } // 1.2 meaning +20%, creature_template assigned

        bool ModIsInHeroicRaid; // MAP dependent - cant change with time

        float GetCreatureHealthMod();
        float GetCreatureDamageMod();
        //static float GetMapDependentMod(uint32 mapId, bool IsHeroic);
        //static float GetHeroicRaid60DependentMod(uint32 mapId);
        //static float GetHeroicRaid70DependentMod(uint32 mapId);        
        float GetConfigHealthMod();
        float GetConfigDamageMod();
        /* This is to check if mod is from config and not from creature or map. Returns config mod, or 1.0 if not affected by config*/
        //float GetPercentageDamageMod();
        /*Same as RemoveAllAuras but does not remove creature_addon or creature_template_addon auras*/
        void RemoveAllAurasNotCreatureAddon();

        void SetIsDoZoneInCombatThreat() { _IsDoZoneInCombatThreat = true; };
        bool IsDoZoneInCombatThreat() { return _IsDoZoneInCombatThreat; };

        std::set<uint64> m_PlayersAllowedToLoot; // only creatures have this.

		bool GenerateLoot(Unit* loot_recipient, bool alive);
		bool pulled;

		void ClearEmote() { SetUInt32Value(UNIT_NPC_EMOTESTATE, 0); };
		void RestoreDBEmote() { if (db_emote) SetUInt32Value(UNIT_NPC_EMOTESTATE, db_emote); };
		bool HasActiveEmote() { return (bool)GetUInt32Value(UNIT_NPC_EMOTESTATE); };

        //std::set<uint32> m_MotherAccsAllowedToLoot; // only creatures have this. for REALM_X100

    protected:
        bool CreateFromProto(uint32 guidlow,uint32 Entry, PlayerTeam team, const CreatureData *data = NULL, GameEventCreatureData const* eventData =NULL);
        bool InitEntry(uint32 entry, PlayerTeam team=ALLIANCE, const CreatureData* data=NULL, GameEventCreatureData const* eventData = NULL);
        ...
        /// Timers
		time_t m_deathTimer;                                // (msecs)timer for death or corpse disappearance
    ...
    private:
        TimeTrackerSmall m_schoolCooldowns[MAX_SPELL_SCHOOL];
    ...
    private:
        bool IsDistancToHomeEvadable; // USE ONLY for timing mobs or event purposes.
        bool _IsDoZoneInCombatThreat;
