...
inline void LoadDBC(uint32& availableDbcLocales, StoreProblemList& errlist, DBCStorage<T>& storage, const std::string& dbc_path, const std::string& filename)
{
    ...
    else
    {
        ...
        {
            char buf[100];
            snprintf(buf,100," (exist, but have %d fields instead %llu) Wrong client version DBC file?",storage.GetFieldCount(),strlen(storage.GetFormat()));
...
void LoadDBCStores(const std::string& dataPath)
{
    ...
    //LoadDBC(availableDbcLocales,bad_dbc_files,sLFGDungeons,              dbcPath,"LFGDungeons.dbc");
    ...
    {
        for (uint32 j = 0; j < sMapStore.GetNumRows(); ++j)
        {
            if (j == MAP_DEADMINES)
            {
                MapEntry const* map = sMapStore.LookupEntry(j);
                if (map)
                    const_cast<MapEntry*>(map)->resetTimeHeroic = 259200; //3 days
            }
        }
    }
    ...
    //    {
    //        entry = (SpellEntry*)sSpellTemplate.LookupEntry<SpellEntry>(i);
    //        if (!entry)
    //            continue;

    //        switch (entry->Id)
    //        {
    //        case 28580: entry->CategoryRecoveryTime = 60000;
    //        }
    //    }
    //}
    ...
    for(uint32 i = 0; i < sSpellItemEnchantmentStore.GetNumRows(); ++i)
    {
        ...
        case 2506: entry->spellid[0] = 42528; entry->type[1] = 3; entry->spellid[1] = 42528; break; // elemental sharpening stone
    ...
    else if(!bad_dbc_files.empty() )
    {
        ...
        sLog.outLog(LOG_DEFAULT, "ERROR: \nSome required *.dbc files (%llu from %d) not found or not compatible:\n%s",bad_dbc_files.size(),DBCFilesCount,str.c_str());
        exit(1);
    }
...
HELLGROUND_EXPORT TalentSpellPos const* GetTalentSpellPos(uint32 spellId)
{
...
//new
//HELLGROUND_EXPORT DBCStorage <TalentEntry>        const* GetTalentEntryStore()         { return &sTalentStore; }
//HELLGROUND_EXPORT DBCStorage <TalentTabEntry>    const* GetTalentTabEntryStore()     { return &sTalentTabStore; }
