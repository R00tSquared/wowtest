...
void WorldSession::HandleDestroyItemOpcode(WorldPacket & recv_data)
{
    ...
    if (count)
    {
        ...
        _player->DestroyItemCount(pItem, i_count, true, "DRAG_DESTROY");
    }
    else
        _player->DestroyItem(bag, slot, true, "DRAG_DESTROY");
}
    ...
    if (pProto)
    {
        ...
        for (int s = 0; s < MAX_ITEM_PROTO_SPELLS; ++s)
        {
            ...
            if (spell)
            {
                ...
                if (s == 4 && pProto->Spells[s-1].SpellId == SPELL_ID_GENERIC_LEARN) // spellid_5, if spellid_4 is learn
                {
                    data << uint32(7267); // Grovel
                    data << uint32(6);
                }
                else
                {
...
void WorldSession::HandleSellItemOpcode(WorldPacket & recv_data)
{
    ...
    if (!pCreature)
    {
        if (vendorguid == _player->GetTransmogManager()->GetItemGUID())
        {
            _player->GetTransmogManager()->HandleAddTransmogPacket(itemguid);
            return;
        }
        sLog.outDebug("WORLD: HandleSellItemOpcode - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(vendorguid)));
        _player->SendSellError(SELL_ERR_CANT_FIND_VENDOR, NULL, itemguid, 0);
    ...
    if (pCreature->GetScriptName() == "DeathSide_npc_shop")
    {
    ...
    if (pItem)
    {
        ...
        if (pProto)
        {
            ...
            {
                if (uint64(pProto->SellPrice) * uint64(count) + uint64(_player->GetMoney()) >= MAX_MONEY_AMOUNT)
                {
                ...
                if (count < pItem->GetCount())               // need split items
                {
                    ...
                    if (!pNewItem)
                    {
                        sLog.outLog(LOG_DEFAULT, "ERROR: WORLD: HandleSellItemOpcode - could not create clone of item %u; count = %u", pItem->GetEntry(), count);
                        _player->SendSellError(SELL_ERR_CANT_SELL_ITEM, pCreature, itemguid, 0);
                        return;
                    }

                    pItem->SetCount(pItem->GetCount() - count);
                    _player->ItemRemovedCheck(pItem->GetEntry(), count, "VENDOR_SELL");
                ...
                {
                    _player->ItemRemovedCheck(pItem->GetEntry(), pItem->GetCount(), "VENDOR_SELL");
                    _player->RemoveItem(pItem->GetBagSlot(), pItem->GetSlot(), true);
                    pItem->RemoveFromUpdateQueueOf(_player);
                    _player->AddItemToBuyBackSlot(pItem);
                }

                // for items with 1 copper
                if (!(sWorld.isEasyRealm() && pProto->SellPrice == 1))
...
void WorldSession::SendListInventoryCustomVendor(uint64 vendorguid, uint32 creature_entry)
{
    // vendor item list @!leg_vendor
    std::vector<CustomVendor> const* items = sWorld.GetCustomVendorItems(creature_entry);
    if (!items || items->size() == 0)
    {
        ChatHandler(GetPlayer()).SendSysMessage("Error: empty vendor!");
    ...
    uint8 numitems = items->size(); //total items in vendor
    uint8 count = 0;
    ...
    for (const CustomVendor& vdata : *items) {
        if (ItemPrototype const* pProto = ObjectMgr::GetItemPrototype(vdata.item))
        {
            ++count;

            currency1 = vdata.need_item1;
            currency2 = vdata.need_item2;

            uint32 price = vdata.need_count1 * 10000;

            if (currency2)
                price += vdata.need_count2 * 100;

            data << uint32(count);
            data << uint32(vdata.item);
    ...
    if (currency2)
        msg = ChatHandler(GetPlayer()).PGetSysMessage(16629, GetPlayer()->GetItemLink(currency1).c_str(), GetPlayer()->GetItemLink(currency2).c_str());
    else
        msg = ChatHandler(GetPlayer()).PGetSysMessage(16630, GetPlayer()->GetItemLink(currency1).c_str());

    ChatHandler(GetPlayer()).SendSysMessage(msg.c_str());
    SendNotification(msg.c_str());
}

void WorldSession::SendListInventory(uint64 vendorguid, uint32 multivendor_entry)
{
    ...
    // send ventor item list and return @!leg_vendor
    if (pCreature->GetEntry() == 693127 || pCreature->GetEntry() == 693017)
    {
        SendListInventoryCustomVendor(vendorguid, pCreature->GetEntry());
        return;
    }

    uint32 vendor_entry = multivendor_entry ? multivendor_entry : pCreature->GetEntry();
    ...
    uint8 numitems = vItems->GetItemCount(); //total items in vendor
    uint8 count = 0;
    ...
    // if we have more than 150 items in total
    // add first items that fits player class
    // then add all other items to container to show with Transmog
    std::vector<const VendorItem*> random_items;
    if (_player->HasItemCount(TRANSMOGRIFICATOR, 1) || !_player->isGameMaster())
    {
        // message
        addRandom = true;
    }
        
    // 150 items maximum (15 pages)
    for (int i = 0; i < numitems; i++)
    {
        if (VendorItem const* crItem = vItems->GetItem(i))
        {
            ...
            {
                // don't add to vendor, just count and add to container
                if ((pProto->AllowableClass & _player->GetClassMask()) == 0 && pProto->Bonding == BIND_WHEN_PICKED_UP)
                {
                    // skip all jewelery
                    if (addRandom && 
                        pProto->InventoryType != 2 &&
                        pProto->InventoryType != 11 &&
                        pProto->InventoryType != 12)
                        random_items.push_back(crItem);
    ...
    if (addRandom && !random_items.empty())
    {
        if (numitems > 150)
        {
            std::random_shuffle(random_items.begin(), random_items.end());
            ChatHandler(GetPlayer()).PSendSysMessage(LANG_VENDOR_MAX_ITEMS);
        }         
        
        for (auto ri : random_items)
        {
            if (count >= 150)
                break;
            
            if (ItemPrototype const *pProto = ObjectMgr::GetItemPrototype(ri->item))
            {
                ++count;
                ...
                data << uint32(ri->item);
                data << uint32(pProto->DisplayInfoID);
                data << uint32(ri->maxcount <= 0 ? 0xFFFFFFFF : pCreature->GetVendorItemCurrentCount(ri));
                data << uint32(price);
                data << uint32(pProto->MaxDurability);
                data << uint32(pProto->BuyCount);
                data << uint32(ri->ExtendedCost);
            }
        }
    }
...
void WorldSession::HandleWrapItemOpcode(WorldPacket& recv_data)
{
    ...
    uint32 count = 1;
    _player->DestroyItemCount(gift, count, true, "WRAP");
}
    ...
    for (int i = 0; i < MAX_GEM_SOCKETS; ++i)
    {
        if (GemEnchants[i])
        {
            ...
            if (Item* guidItem = _player->GetItemByGuid(gem_guids[i]))
                _player->DestroyItem(guidItem->GetBagSlot(), guidItem->GetSlot(), true, "SOCKET_DESTROY");
        }
    }
