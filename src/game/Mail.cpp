...
void MailDraft::SendMailTo(MailReceiver const& receiver, MailSender const& sender, MailCheckMask checked, uint32 deliver_delay)
{
    ...
    if (sender.GetStationery() == MAIL_STATIONERY_GM)
        expire_delay = 180 * DAY;
    // auction mail without any items and money (auction sale note) pending 1 hour
    else if (sender.GetMailMessageType() == MAIL_AUCTION && m_items.empty() && !m_money)
        expire_delay = HOUR;
    ...
    RealmDataDatabase.PExecute("INSERT INTO mail (id,messageType,stationery,mailTemplateId,sender,receiver,subject,itemTextId,has_items,expire_time,deliver_time,money,cod,checked) "
        "VALUES ('%u', '%u', '%u', '%u', '%u', '%u', '%s', '%u', '%u', '%llu','%llu', '%u', '%u', '%u')",
...
void Mail::prepareTemplateItems( Player* receiver )
{
    ...
    // can be empty
    mailLoot.FillLoot(mailTemplateId, LootTemplates_QuestMail, receiver, true, 0);
