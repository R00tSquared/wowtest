...
void WorldSession::HandleGuildInviteOpcode(WorldPacket& recvPacket)
{
    ...
    if (normalizePlayerName(Invitedname))
        player = sObjectAccessor.GetPlayerByNameInWorld(Invitedname);
    ...
    if (!sWorld.getConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GUILD) && (sWorld.getConfig(CONFIG_BATTLEGROUND_ALLOW_DIFFERENT_FACTION) && (GetPlayer()->InBattleGroundOrArena() || player->InBattleGroundOrArena())))
    {
...
void WorldSession::HandleGuildAcceptOpcode(WorldPacket& /*recvPacket*/)
{
    ...
    if (!sWorld.getConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GUILD) && (sWorld.getConfig(CONFIG_BATTLEGROUND_ALLOW_DIFFERENT_FACTION) && player->InBattleGroundOrArena()))
    {
        if (player->TeamForRace(player->GetRace()) != sObjectMgr.GetPlayerTeamByGUID(guild->GetLeader()))
            return;
    }
...
void WorldSession::HandleGuildLeaveOpcode(WorldPacket& /*recvPacket*/)
{
    ...
    guild = sGuildMgr.GetGuildById(_player->GetGuildId());
    if (!guild)
    {
        SendGuildCommandResult(GUILD_CREATE_S, "", GUILD_PLAYER_NOT_IN_GUILD);
        return;
...
void WorldSession::HandleGuildLeaderOpcode(WorldPacket& recvPacket)
{
    ...
    guild = sGuildMgr.GetGuildById(oldLeader->GetGuildId());
...
void WorldSession::HandleGuildMOTDOpcode(WorldPacket& recvPacket)
{
    ...
    if (!recvPacket.empty())
    {
        ...
        if (!processChatmessageFurtherAfterSecurityChecks(MOTD, LANG_UNIVERSAL))
            return;

        if (ChatHandler(this).ContainsNotAllowedSigns(MOTD))
            return;

        std::string to = "Guild_MOTD";
        sChatLog.WhisperMsg(GetPlayer(), to, MOTD);

        if (MOTD == "Lexics Cutter Detected Bad Words")
            return;
    }
    else
        MOTD = "";
...
void WorldSession::HandleGuildBankDeposit(WorldPacket & recv_data)
{
    ...
    // log
    pGuild->LogBankEvent(GUILD_BANK_LOG_DEPOSIT_MONEY, uint8(0), GetPlayer(), money);
    pGuild->DisplayGuildBankTabsInfo(this);
...
void WorldSession::HandleGuildBankWithdraw(WorldPacket & recv_data)
{
    ...
    if (money >= MAX_MONEY_AMOUNT || GetPlayer()->GetMoney() + money >= MAX_MONEY_AMOUNT)
    {
    ...
    // Log
    pGuild->LogBankEvent(GUILD_BANK_LOG_WITHDRAW_MONEY, uint8(0), GetPlayer(), money);
    pGuild->SendMoneyInfo(this, GetPlayer()->GetGUIDLow());
    pGuild->DisplayGuildBankTabsInfo(this);
    pGuild->DisplayGuildBankContent(this, 0);
    pGuild->DisplayGuildBankMoneyUpdate();
...
void WorldSession::HandleGuildBankDepositItem(WorldPacket & recv_data)
{
    ...
    if (BankToBank)
    {
        ...
        {                                                   // Bank -> Bank item split (in empty or non empty slot
            ...
            RealmDataDatabase.BeginTransaction();
            pGuild->LogBankEvent(GUILD_BANK_LOG_MOVE_ITEM, BankTab, pl, pItemSrc->GetEntry(), SplitedAmount, BankTabDst);

            pl->ItemRemovedCheck(pItemSrc->GetEntry(), SplitedAmount, "GB_DEPOSIT");
        ...
        else                                                // non split
        {
            ...
            {
                RealmDataDatabase.BeginTransaction();
                pGuild->LogBankEvent(GUILD_BANK_LOG_MOVE_ITEM, BankTab, pl, pItemSrc->GetEntry(), pItemSrc->GetCount(), BankTabDst);
            ...
            else                                            // swap
            {
                ...
                RealmDataDatabase.BeginTransaction();
                pGuild->LogBankEvent(GUILD_BANK_LOG_MOVE_ITEM, BankTab,    pl, pItemSrc->GetEntry(), pItemSrc->GetCount(), BankTabDst);
                pGuild->LogBankEvent(GUILD_BANK_LOG_MOVE_ITEM, BankTabDst, pl, pItemDst->GetEntry(), pItemDst->GetCount(), BankTab);
    ...
    if (ToChar)                                             // Bank -> Char cases
    {
        ...
        {                                                   // Bank -> Char split to slot (patly move)
            ...
            RealmDataDatabase.BeginTransaction();
            pGuild->LogBankEvent(GUILD_BANK_LOG_WITHDRAW_ITEM, BankTab, pl, pItemBank->GetEntry(), SplitedAmount);
        ...
        else                                                // Bank -> Char swap with slot (move)
        {
            ...
            if (msg == EQUIP_ERR_OK)                       // merge case
            {
                ...
                RealmDataDatabase.BeginTransaction();
                pGuild->LogBankEvent(GUILD_BANK_LOG_WITHDRAW_ITEM, BankTab, pl, pItemBank->GetEntry(), pItemBank->GetCount());
            ...
            else                                            // Bank <-> Char swap items
            {
                ...
                pGuild->LogBankEvent(GUILD_BANK_LOG_WITHDRAW_ITEM, BankTab, pl, pItemBank->GetEntry(), pItemBank->GetCount());
                if (pItemChar)
                    pGuild->LogBankEvent(GUILD_BANK_LOG_DEPOSIT_ITEM, BankTab, pl, pItemChar->GetEntry(), pItemChar->GetCount());

                pGuild->RemoveItem(BankTab, BankTabSlot);
                if (pItemChar)
                {
                    pl->MoveItemFromInventory(PlayerBag, PlayerSlot, true, "GUILD_BANK");
    ...
    {                                                       // Char -> Bank split to empty or non-empty slot (partly move)
        ...
        RealmDataDatabase.BeginTransaction();
        pGuild->LogBankEvent(GUILD_BANK_LOG_DEPOSIT_ITEM, BankTab, pl, pItemChar->GetEntry(), SplitedAmount);

        pl->ItemRemovedCheck(pItemChar->GetEntry(), SplitedAmount, "GB_DEPOSIT");
...
void WorldSession::HandleGuildBankBuyTab(WorldPacket & recv_data)
{
    ...
    uint32 TabCost = sGuildMgr.GetGuildBankTabPrice(TabId) * GOLD;
    if (!TabCost)
        return;
