...
void OPvPCapturePointEP::ChangeState()
{
    ...
    GameObject* flag = m_PvP->GetMap()->GetGameObject(m_Objects[m_TowerType]);
    GameObject* flag2 = m_PvP->GetMap()->GetGameObject(m_Objects[m_TowerType+EP_TOWER_NUM]);
...
void OPvPCapturePointEP::UpdateTowerEvents()
{
    if (m_State == OBJECTIVESTATE_ALLIANCE || m_State == OBJECTIVESTATE_HORDE)
    {
        PlayerTeam team = (m_State == OBJECTIVESTATE_ALLIANCE) ? ALLIANCE : HORDE;
        switch (m_TowerType)
        {
...
void OPvPCapturePointEP::LinkGraveYard(PlayerTeam team)
{
...
void OPvPCapturePointEP::SummonSupportUnits(PlayerTeam team)
{
...
void OPvPCapturePointEP::SummonShrine(PlayerTeam team)
{
    if (EP_TOWER_EVENT_TEAM[EP_TOWER_NORTHPASS] != team)
    {
        ...
        GameObject * go = m_PvP->GetMap()->GetGameObject(m_Objects[EP_NPT_SHRINE]);
        if (go)
...
void OPvPCapturePointEP::SummonFlightMaster(PlayerTeam team)
{
