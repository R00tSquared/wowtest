...
void WorldSession::HandleNameQueryOpcode(WorldPacket & recv_data)
{
    ...
    Player *pChar = sObjectMgr.GetPlayerInWorld(guid);
    /*char chr[256];
    sprintf(chr, "Player %s, map %u, zone %u, asked for player %s name, who is in map %u, zone %u", 
        GetPlayer() ? GetPlayer()->GetName() : "offline", GetPlayer() ? GetPlayer()->GetMapId() : 999, GetPlayer() ? GetPlayer()->GetCachedZone() : 999,
        pChar ? pChar->GetName() : "offline", pChar ? pChar->GetMapId() : 999, pChar ? pChar->GetCachedZone() : 999); // Trentone BG race change fake

    _player->Say(chr, LANG_UNIVERSAL);*/
    if (pChar)
...
void WorldSession::HandleNpcTextQueryOpcode(WorldPacket & recv_data)
{
    ...
    /*if (IS_UNIT_GUID(guid))
        GetPlayer()->SetSelection(guid);*/ // hope commenting it out doesn't break things. Setting target does nothing for client
    ...
    // try for gossip 68 -> which is basic "Greetings" but localized, it should exist.
    if (!pGossip)
        pGossip = sObjectMgr.GetGossipText(68);
    ...
    else
    {
        ...
        int loc_idx = isRussian() ? 0 : GetSessionDbLocaleIndex();
