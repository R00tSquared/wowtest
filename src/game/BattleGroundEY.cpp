...
//            SpawnBGCreature(EY_SPIRIT_MAIN_HORDE, RESPAWN_IMMEDIATELY);
            ...
            // spawn multi npcs
            AddCreature(693070, 6, HORDE, 1808.14, 1545.60, 1267.56, 3.9);
            AddCreature(693070, 7, ALLIANCE, 2521.92, 1592.28, 1268.98, 1.21);
        ...
        {
            m_Events |= 0x04;
            SendMessageToAll(LANG_BG_EY_ONE_MINUTE);
        }
        // After 1,5 minute, warning is signalled
        else if (GetStartDelayTime() <= START_DELAY2 && !(m_Events & 0x08))
        {
            m_Events |= 0x08;
            SendMessageToAll(LANG_BG_EY_HALF_MINUTE);
        }
...
void BattleGroundEY::AddPoints(PlayerTeam Team, uint32 Points)
{
...
void BattleGroundEY::CheckSomeoneJoinedPoint()
{
    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (obj)
        {
            ...
            {
                Player *plr = sObjectMgr.GetPlayerInWorld(m_PlayersNearPoint[EY_POINTS_MAX][j]);
                if (plr && plr->isAllowUseBattleGroundObject() && plr->IsWithinDistInMap(obj, BG_EY_POINT_RADIUS))
                {
                    //player joined point!
                    //show progress bar
...
void BattleGroundEY::CheckSomeoneLeftPoint()
{
    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (obj)
        {
            ...
            {
                Player *plr = sObjectMgr.GetPlayerInWorld(m_PlayersNearPoint[i][j]);
                if (!plr)
                {
                    sLog.outLog(LOG_DEFAULT, "ERROR: BattleGroundEY: Player %llu not found!", m_PlayersNearPoint[i][j]);
...
void BattleGroundEY::UpdatePointStatuses()
{
    for (uint8 point = 0; point < EY_POINTS_MAX; ++point)
    {
        ...
        {
            Player *plr = sObjectMgr.GetPlayerInWorld(m_PlayersNearPoint[point][i]);
            if (plr)
            {
...
void BattleGroundEY::UpdateTeamScore(PlayerTeam Team)
{
    uint32 score = GetTeamScore(Team);
    if (score >= EY_MAX_TEAM_SCORE)
    {
        score = EY_MAX_TEAM_SCORE;
        //RewardHonorToTeam(WIN_HONOR_COINT, Team);
...
void BattleGroundEY::UpdatePointsCount(PlayerTeam Team)
{
    if (Team == ALLIANCE)
...
void BattleGroundEY::UpdatePointsIcons(PlayerTeam Team, uint32 Point)
{
...
void BattleGroundEY::ResetBGSubclass()
{
    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        m_PointOwnedByTeam[i] = PlayerTeam(EY_POINT_NO_OWNER);
...
void BattleGroundEY::RespawnFlag(bool send_message)
{
    ...
    if (send_message)
    {
        SendMessageToAll(LANG_BG_EY_RESETED_FLAG);
...
void BattleGroundEY::EventPlayerDroppedFlag(Player *Source)
{
    ...
    {
        message = LANG_BG_EY_DROPPED_FLAG;
...
void BattleGroundEY::EventPlayerClickedOnFlag(Player *Source, GameObject* target_obj)
{
    ...
    uint8 type = 0;
    int32 message = LANG_BG_EY_HAS_TAKEN_FLAG;
...
void BattleGroundEY::EventTeamLostPoint(Player *Source, uint32 Point)
{
    ...
    int32 message;
    PlayerTeam Team = m_PointOwnedByTeam[Point];
    ...
    if (Team == ALLIANCE)
    {
        ...
        message = m_LoosingPointTypes[Point].MessageIdAlliance;
    ...
    else
    {
        ...
        message = m_LoosingPointTypes[Point].MessageIdHorde;
    ...
    m_PointOwnedByTeam[Point] = PlayerTeam(EY_POINT_NO_OWNER);
    m_PointState[Point] = EY_POINT_NO_OWNER;

    SendMessageToAll(message, message_type, Source->GetSession(), Source->GetGUID());
...
void BattleGroundEY::EventTeamCapturedPoint(Player *Source, uint32 Point)
{
    ...
    uint8 type = 0;
    int32 message;
    PlayerTeam Team = Source->GetBGTeam();
    ...
    if (Team == ALLIANCE)
    {
        m_TeamPointsCount[TEAM_ALLIANCE]++;
        type = CHAT_MSG_BG_SYSTEM_ALLIANCE;
        message = m_CapturingPointTypes[Point].MessageIdAlliance;
    ...
    else
    {
        m_TeamPointsCount[TEAM_HORDE]++;
        type = CHAT_MSG_BG_SYSTEM_HORDE;
        message = m_CapturingPointTypes[Point].MessageIdHorde;
...
void BattleGroundEY::EventPlayerCapturedFlag(Player *Source, uint32 BgObjectType)
{
    ...
    if (Source->GetBGTeam() == ALLIANCE)
    {
        ...
        message = LANG_BG_EY_CAPTURED_FLAG_A;
        type = CHAT_MSG_BG_SYSTEM_ALLIANCE;
    }
    else
    {
        ...
        team_id = TEAM_HORDE;
        message = LANG_BG_EY_CAPTURED_FLAG_H;
        type = CHAT_MSG_BG_SYSTEM_HORDE;
    }
...
WorldSafeLocsEntry const *BattleGroundEY::GetClosestGraveYard(float x, float y, float z, PlayerTeam team)
{
