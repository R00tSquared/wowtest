    ...
    if (m_uint32Values)                                      // field array can be not exist if GameOBject not loaded
    {
        ...
        if (uint64 owner_guid = GetOwnerGUID())
        {
            ...
            if (IS_PLAYER_GUID(owner_guid))
                owner = ObjectAccessor::GetPlayerInWorldOrNot(owner_guid);
            else
    ...
    switch (m_lootState)
    {
            ...
            switch (GetGoType())
            {
                case GAMEOBJECT_TYPE_TRAP:
                {
                    ...
                    if (Unit* owner = GetOwner())
                        m_cooldownTime = WorldTimer::getMSTime() + GetGOInfo()->trap.startDelay*MILLISECONDS;
        ...
        case GO_READY:
        {
            ...
            if (goInfo && goInfo->type == GAMEOBJECT_TYPE_TRAP)
            {
                ...
                if (m_cooldownTime > WorldTimer::getMSTime())
                    return;
                ...
                // monwell: fire damage objects (in AV, Orgrimmar) have same range as trap spell
                const SpellEntry* spellEntry = sSpellTemplate.LookupEntry<SpellEntry>(goInfo->trap.spellId);
                if (spellEntry && spellEntry->Effect[0] == SPELL_EFFECT_ENVIRONMENTAL_DAMAGE)
                    radius = SpellMgr::GetSpellRadius(spellEntry, 0, false) * 0.5f;
                ...
                else                                                                 // creature spawned traps
                {
                    ...
                    m_cooldownTime = WorldTimer::getMSTime() + goInfo->trap.cooldown*MILLISECONDS;
                }

                if (ok && (ok->GetTypeId() != TYPEID_PLAYER || ok->GetTypeId() == TYPEID_PLAYER && !ok->ToPlayer()->IsSpectator()))
                {
                    if (goInfo->trap.spellId == 11409 || goInfo->trap.spellId == 11362) //  Teleport to Booty Bay && Teleport to Gnomeregan
                    {
                        if (!((Player*)ok)->HasItemCount(9173 , 1))
                            return;
                    }

                    CastSpell(ok, goInfo->trap.spellId);
                    m_cooldownTime = WorldTimer::getMSTime() + (goInfo->trap.cooldown ? goInfo->trap.cooldown : 4)*MILLISECONDS;  // default 4 sec cooldown??
                    SendCustomAnimation();
                    ...
                    if (IsBattleGroundTrap && ok->GetTypeId() == TYPEID_PLAYER)
                    {
                        ...
                        if (((Player*)ok)->InBattleGroundOrArena())
        ...
        case GO_JUST_DEACTIVATED:
        {
            switch (GetGoType())
            {
                case GAMEOBJECT_TYPE_SUMMONING_RITUAL:
                {
                    if(m_target)
                    {
                        if(Player* target = Player::GetPlayerInWorld(m_target))
                        {
                    ...
                    if (!m_unique_users.empty())
                    {
                        ...
                        for(; i != m_unique_users.end(); i++)
                        {
                            if (Unit* caster = Unit::GetUnit(*this, uint64(*i)))
                            {
                                ...
                                {
                                    if (spell->GetSpellEntry()->Id == info->summoningRitual.animSpell || GetOwner() == caster)
                                    {
...
void GameObject::SaveUpdateToDB()
{
    ...
    SaveUpdateOrNewToDB_gob_add(GetMapId(), data->spawnMask);
}

void GameObject::SaveUpdateOrNewToDB_gob_add(uint32 mapid, uint8 spawnMask)
{
    ...
    stmt.addFloat(data.rotation0);
    stmt.addFloat(data.rotation1);
    stmt.addFloat(data.rotation2);
    stmt.addFloat(data.rotation3);
    stmt.addUInt32(m_respawnDelayTime);
    stmt.addUInt32(data.animprogress);
    stmt.addUInt32(data.go_state);
...
bool GameObject::LoadFromDB(uint32 guid, Map *map)
{
    ...
    else
    {
        if (data->spawntimesecs >= 0)
        {
            ...
            m_respawnTime = sObjectMgr.GetGORespawnTime(m_DBTableGuid, map->GetInstanciableInstanceId());

            // ready to respawn
            if (m_respawnTime && m_respawnTime <= time(NULL))
            {
                m_respawnTime = 0;
                sObjectMgr.SaveGORespawnTime(m_DBTableGuid, GetInstanciableInstanceId(),0);
            }
        }
        else
        {
...
void GameObject::DeleteFromDB_gob_del()
{
...
{
public:
    DynamicRespawnPlayerCounter(const GameObject* const go) : count(0), me(go) {}
    void operator()(Player* u)
    {
        ++count;
    }
    uint32 count;
    const GameObject* const me;
};

void GameObject::DynamicRespawnMod(uint32 &baseDelay) const
{
    ...
    DynamicRespawnPlayerCounter checker(this);
...
void GameObject::SaveRespawnTime()
{
    if (m_goData && m_goData->dbData && m_respawnTime > time(NULL) && m_spawnedByDefault)
        sObjectMgr.SaveGORespawnTime(m_DBTableGuid, GetInstanciableInstanceId(),m_respawnTime);
}
...
void GameObject::Despawn()
{
    ...
    else
    {
        ...
        uint16 poolid = 0;
        if (m_spawnedByDefault)
            poolid = sPoolMgr.IsPartOfAPool<GameObject>(GetGUIDLow());

        uint32 respTime = m_respawnDelayTime;
        if (!poolid)
            DynamicRespawnMod(respTime);

        m_respawnTime = m_spawnedByDefault ? time(NULL) + respTime : 0;
        ...
        if (poolid)
            sPoolMgr.UpdatePool<GameObject>(poolid, GetGUIDLow()); // will delete this object or leave it be
    }
        ...
        case GAMEOBJECT_TYPE_CHEST:
        {
            ...
            if (GetEntry() == BG_AV_OBJECTID_MINE_N || GetEntry() == BG_AV_OBJECTID_MINE_S)
                if (BattleGround *bg = pTarget->GetBattleGround())
                    if (bg->GetTypeID() == BATTLEGROUND_AV && !(((BattleGroundAV*)bg)->PlayerCanDoMineQuest(GetEntry(),pTarget->GetBGTeam())))
                        return false;
...
void GameObject::UseDoorOrButton(uint32 time_to_restore, bool alternative /* = false */)
{
    ...
    m_cooldownTime = WorldTimer::getMSTime() + time_to_restore*MILLISECONDS;
}
...
void GameObject::Use(Unit* user)
{
    ...
    // Gensen: fixing infinity GO script calls (Shallow Grave) .go obj 27134
    // DANGEROUS?
    //if (GetGoType() == GAMEOBJECT_TYPE_CHEST && GetCooldownTimeLeft() > 0)
    //{
    //    if (pPlayer)
    ...
    switch (GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_DOOR:                          //0
            if (GetGoState() == GO_STATE_READY)                     //if closed -> open
                ...
                SetGoState(GO_STATE_READY);
            m_cooldownTime = WorldTimer::getMSTime() + GetAutoCloseTime()*MILLISECONDS;
            Activate();
            break;
        ...
        case GAMEOBJECT_TYPE_CHEST:                         // 3
        {
            ...
            m_cooldownTime = time(NULL) + ((GetMap()->IsDungeon()) ? HOUR : 180);
        ...
        case GAMEOBJECT_TYPE_GOOBER:                        //10
        {
            ...
            if (pPlayer)
            {
                ...
                SetFlag(GAMEOBJECT_FLAGS, GO_FLAG_IN_USE);
                m_cooldownTime = WorldTimer::getMSTime() + info->goober.cooldown*MILLISECONDS;
            }
        ...
        case GAMEOBJECT_TYPE_SUMMONING_RITUAL:              // 18
        {
            ...
            {
                // do nothing - error
                sLog.outLog(LOG_DEFAULT, "ERROR: no animation in Use of ritual gameobject id %u", info->id);
            }

            uint32 participants = GetGOInfo()->summoningRitual.reqParticipants;
            if (sWorld.isEasyRealm() && GetEntry() == 36727)
                participants = 2;

            if(m_unique_users.size() == participants)
            {
                SetFlag(GAMEOBJECT_FLAGS, GO_FLAG_IN_USE);
                Player* target = Player::GetPlayerInWorld(m_target);
                ...
                {
                    if(info->summoningRitual.casterTargetSpell == 20625) // Ritual of Doom kill random player
                    {
                        auto itr = m_unique_users.begin();
                        std::advance(itr, urand(0, m_unique_users.size() - 1));
                        auto guid = *itr;
                        if (Player* p = sObjectMgr.GetPlayerInWorld(guid))
                            p->CastSpell(p, 20625, true);
                    }
                    else
        ...
        case GAMEOBJECT_TYPE_MEETINGSTONE:                  // 23
        {
            ...
            Player* targetPlayer = ObjectAccessor::GetPlayerInWorld(player->GetSelection());
...
void GameObject::CastSpell(Unit* target, uint32 spell, uint64 originalCaster)
{
    ...
    if (target)
    {
        const SpellEntry* spellInfo = sSpellTemplate.LookupEntry<SpellEntry>(spell);
        if (SpellMgr::isSpellBreakCasterStealth(spellInfo))
            target->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_CAST);
        else
            target->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_CAST_IN_STEALTH);
    }
    ...
    {
        if (Unit* origCaster = GetMap()->GetUnit(originalCaster))
        {
            trigger->setFaction(origCaster->getFaction());
            trigger->CastSpell(target, spell, true, 0, 0, origCaster->GetGUID());
        }
        else
        {
            trigger->setFaction(14);
            trigger->CastSpell(target, spell, true, 0, 0, (target && target != nullptr) ? target->GetGUID() : 0);
        }
    }
    else
    {
...
void GameObject::HandleNonDbcSpell(uint32 spellId, Player* pUser)
{
    switch(spellId)
    {
        ...
        case 37264: // Power Converter
        {
            ...
            if (Creature *pSummon = pUser->SummonCreature(entry, x, y, z, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 60000))
