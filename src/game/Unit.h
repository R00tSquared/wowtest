...
enum SpellAuraInterruptFlags
{
    ...
    AURA_INTERRUPT_FLAG_CAST                = 0x00002000,   // 13   triggered by spell casts, except SPELL_ATTR_EX_NOT_BREAK_CASTER_STEALTH
    ...
    /* 25   triggered by spell casts with SPELL_ATTR_EX_NOT_BREAK_CASTER_STEALTH,
    used to accompany AURA_INTERRUPT_FLAG_CAST to remove aura from ANY spell cast*/
    AURA_INTERRUPT_FLAG_CAST_IN_STEALTH     = 0x02000000,   

    AURA_INTERRUPT_FLAG_ONLY_IN_RAID = 0x08000000,    // SPELL_ATTR_EX6_ONLY_IN_RAID
    AURA_INTERRUPT_FLAG_DIRECT_DAMAGE_OR_NON_ABSORBED_DOT = 0x10000000,    // new flag - for stealth - removed by any direct damage, but if DoT is absorbed - it doesn't remove stealth, non-absorbed DoT still remove stealth
    AURA_INTERRUPT_SPELL_ATTR_EX6_NOT_IN_RAID_INSTANCE    = 0x20000000,    // aura interrupt for spells with SPELL_ATTR_EX6_NOT_IN_RAID_INSTANCE attribute
    AURA_INTERRUPT_SPELL_ATTR_EX6_NOT_PVP                 = 0x40000000,    // aura interrupt for spells with SPELL_ATTR_EX6_NOT_PVP attribute
    AURA_INTERRUPT_SPELL_ATTR_EX6_NOT_HUNTER_PVP          = 0x80000000,    // aura interrupt for spells with SPELL_ATTR_EX6_NOT_HUNTER_PVP attribute

    AURA_INTERRUPT_FLAG_NOT_VICTIM = (AURA_INTERRUPT_FLAG_HITBYSPELL | AURA_INTERRUPT_FLAG_DAMAGE | AURA_INTERRUPT_FLAG_DIRECT_DAMAGE | AURA_INTERRUPT_FLAG_DIRECT_DAMAGE_OR_NON_ABSORBED_DOT),
};
...
enum UnitState
{
    ...
    UNIT_STAT_FLEEING_MOVING     = 0x00000020,        // unit is feared and can move (not rooted)
    ...
    UNIT_STAT_CONFUSED_MOVING    = 0x00000200,        // unit is disoriented and can move (not rooted)
    ...
    UNIT_STAT_ROAMING            = 0x01000000,        // unit is moving
    UNIT_STAT_FLEEING_NOT_MOVING = 0x02000000,        // unit is feared and cannot move (possibly rooted)
    UNIT_STAT_CONFUSED_NOT_MOVING = 0x04000000,        // unit is feared and cannot move (possibly rooted)

    UNIT_STAT_FLEEING = (UNIT_STAT_FLEEING_MOVING | UNIT_STAT_FLEEING_NOT_MOVING),
    UNIT_STAT_CONFUSED = (UNIT_STAT_CONFUSED_MOVING | UNIT_STAT_CONFUSED_NOT_MOVING),
    ...
    UNIT_STAT_MOVING          = (UNIT_STAT_ROAMING | UNIT_STAT_CONFUSED_MOVING | UNIT_STAT_FLEEING_MOVING | UNIT_STAT_CHASE | UNIT_STAT_FOLLOW),
...
// not found or not used or just plain wrong states
/*
    PLAYER_STATE_DANCE
    ...
    UNIT_STAND_STATE_STAND // not used or just no state, just standing there?
    PLAYER_STATE_STUN
...
enum DamageEffectType
{
    ...
    DOT                     = 2,
    HEAL                    = 3,
    NODAMAGE                = 4,                            // used also in case when damage applied to health but not applied to spell channelInterruptFlags/etc // unused
    SELF_DAMAGE             = 5                             // used in environmental damage
};
...
enum UnitFlags
{
    ...
    UNIT_FLAG_RESTART_PACIFIED    = 0x80000000 // make creature pacified if there's a planned server restart
};
...
enum MovementFlags
{
    ...
    MOVEFLAG_FALLING            = 0x00001000,               // jumped -> until landing
    MOVEFLAG_JUMPING            = 0x00002000,               // when actually falling for a second (disable fly -> 1 second 0x00001000, 2 0x00003000)
    MOVEFLAG_FALLINGFAR         = 0x00004000,               // when started falling from place
    ...
    // GENSENTODO: add MOVEFLAG_JUMPING to below as mangos did?
    MOVEFLAG_MOVING         =
...
3:32 ����� �� ����� ������� ������ ������ 5.5
8:47 - 6.2-6.3
8:50 - 5.0-5.2 (������� �� � ����� ������� �����, ���� � ����� ������� - �� ���-�� 5.5-5.7, �� ����� ����� ������� �� ������� ���� � ����, � � ����� ������� ������ 5.0-5.2)
�� �������� �������� ���������� ����� ������ 7 � ���� 10! ������ �� ������ �� �������. � ���� �� ������ ��� ���� ������ � 8.8 ���, ��, ��� ����������, ����� ���������� ������, � �������� ���� ������ ������*/
#define COMBAT_TIME_PVP_MIN 5000
...
class HELLGROUND_IMPORT_EXPORT Unit : public WorldObject
{
    public:
        ...
        typedef std::map<Aura *, int16> AuraMapDamageCapped;
        ...
        void DecrDiminishing(DiminishingGroup group);
		...
		bool inRangedAttackMode() { return m_attackTimer[RANGED_ATTACK]; };
        ...
        bool IsWithinCombatRange(const Unit *obj, float dist2compare) const; // Same as IsWithinDistInMap with 3D (which is true by default)
        ...
        void CombatStopExceptDoZoneInCombat();
        ...
        uint32 GetAttackTime(WeaponAttackType att, bool ModdedAttackSpeed = false) const { return (uint32)(GetFloatValue(UNIT_FIELD_BASEATTACKTIME+att)/(ModdedAttackSpeed ? 1 : m_modAttackSpeedPct[att])); }
        ...
        Unit* GetNextRandomRaidMember(float radius, bool PlayerOnly = false, bool LosIgnore = true);
        ...
        void RemoveSpellbyDamageTaken(uint32 damage, uint32 except, bool direct);

        void SendDamageLog(DamageLog *damageInfo);

        uint32 DealDamage(DamageLog *damageInfo, DamageEffectType damagetype, SpellEntry const *spellProto, bool durabilityLoss, bool& duel_just_ended);
        ...
        void CombatStartOnCast(Unit* target, bool initialAggro, uint32 minCombatTime);
        void SetInCombatState(Unit* enemy, uint32 minCombatTime = 0);
        void SetInCombatWith(Unit* enemy);
        void ClearInCombat();
        uint32 GetCombatTimer() const { return m_CombatTimer; }
        void SetCombatTimer(uint32 minDuration) { m_CombatTimer = minDuration + m_regenTimer; }
        /*If min combat timer is set to 5000, with current implementation combat will actually fade after 5000 to 6999 ms*/
        void CombatTimerTick(uint32 time_diff);
        ...
        bool virtual HasSpellChained(uint32 /*spellID*/) { return false; }
        ...
        /*For units returns GetTerrain()->IsInWater(), for players returns opcode-dependent swim flag (are we swimming or not)*/
        ...
        void AddAura(uint32 spellId);
        void AddAura(uint32 spellId, Unit *target);
        void AddAura(uint32 spellId, Unit *target, uint32 duration);
        void AddAuraCreate(uint32 spellId, Unit* target);
        template <typename Fu> void loop_auras(Fu func, uint32 id = 0) const;
        void DeMorph();
        ...
        void NearTeleportTo(float x, float y, float z, float orientation = 0.0f, bool casting = false);
        ...
        /*Also returns false if player is under control of a creature*/
        ...
        bool isPossessedByPlayer() const { return HasUnitState(UNIT_STAT_POSSESSED) && IS_PLAYER_GUID(GetCharmerGUID()); } // IsPlayerControlled()
        ...
        void RemoveAuraFromStackByDispel(uint32 spellId, uint64 casterGUID);
        void RemoveSingleAuraFromStackByDispel(uint32 spellId, uint64 casterGUID, uint8 eff);
        ...
        void RemoveAurasWithAttribute_NonPassive(uint32 flags);
        void RemoveAurasDueToDurationAndRecoveryTime();            // DUEL UNBUFF
        void RemoveAurasWithInterruptFlags(uint32 flags, uint32 except = 0, bool PositiveOnly = false);
        /*dispelmask is conjured by SpellMgr::GetDispellMask(type);*/
        bool HasAurasWithDispelMask(uint32 dispelMask);
        void RemoveAurasWithDispelType(DispelType type);
        void RemoveAllAurasButPermanent();    // WARLOCK PET unbuff after resummon with current PET
        void RemoveMovementImpairingAuras();

        void RemoveAllAuras();
        void RemoveArenaAuras(bool removeOnlyNegative = false);
        void RemoveAllAurasOnDeath();
        void DelayAura(uint32 spellId, uint32 effindex, int32 delaytime);

        bool HasMorePowerfulBuff(Unit* caster, SpellEntry const* newSpellInfo);
...
#pragma region VisibilityRelocation
        ...
        void ApplyTabardAura();
        bool InClassForm();
        float SpellSpecialMod(uint32 spell_id);
...
#pragma endregion VisibilityRelocation
    ...
    public:
        ...
        bool IsImmunedToSpellMechanic(uint32 mechanic, bool includeCharges);
        // Trentone: cant just use exception aura, cause in that scenario if you have two same-typed auras, they will say that the other one 
        // is adding you immunity -> thus none of them will ever be removed. Gotta remove all charged auras at once, no other choice
        virtual bool IsImmunedToSpell(SpellEntry const* spellInfo, bool includeCharges = true); // includeCharges is false only from ::procDamageAndSpellFor to check if immunity comes from charged aura
                                                            // redefined in Creature
        bool IsImmunedToDamage(SpellSchoolMask meleeSchoolMask, bool includeCharges = true); // includeCharges is false only from ::procDamageAndSpellFor to check if immunity comes from charged aura
        ...
        void SendSpeedChangeToMover(Unit* unit, UnitMoveType mtype, float rate);
        void SendSpeedChangeToObservers(Unit* unit, UnitMoveType mtype, float rate);
        void SendSpeedChangeToAll(Unit* unit, UnitMoveType mtype, float rate);
        
        void SendRootedToMover(Unit* unit, bool apply);
        void SendRootedToObservers(Unit* unit, bool apply);
        void SendRootedToAll(Unit* unit, bool apply);
        ...
        int32 CalculateSpellDamage(SpellEntry const* spellProto, uint8 effect_index, int32 basePoints, bool spendCharges = true);
        ...
        static Player* GetPlayerInWorld(uint64 guid);
        ...
        void SendSpellVisual(uint32 kitID);
        ...
        // this is called for PLAYER-ONLY movement
        // calls from anticheat take distTravelled into consideration
        void ActivateFoundTrapsIfNeeded(Position const & lineStart, Position const & lineEnd, float distance);

        void ApplyAdditionalSpell(Unit* caster, const SpellEntry* spellInfo);

        bool m_AutoRepeatPrioritized;
        uint32 m_lastSanctuaryTime;

		bool bot_FakeGroup;
        uint32 m_custom; // used in some cases, for example in gossips
...
template <typename Fu> void Unit::loop_auras(Fu func, uint32 id) const
{
	if (id)
	{
		for (int i = 0; i < 3; ++i)
		{
			auto bounds = m_Auras.equal_range(spellEffectPair(id, i));
