...
void BattleGroundAB::Update(uint32 diff)
{
    ...
    if (GetStatus() == STATUS_WAIT_JOIN && GetPlayersSize())
    {
        ...
        if (!(m_Events & 0x01))
        {
            ...
            AddCreature(693070, 7, HORDE, 698.62, 713.28, -16.18, 4.1);
            AddCreature(693070, 8, ALLIANCE, 1293.62, 1278.42, -14.63, 1.37);
    ...
    else if (GetStatus() == STATUS_IN_PROGRESS)
    {
            ...
            if (m_NodeTimers[node].Expired(diff))
                {
                    ...
                    sprintf(buf, GetHellgroundString(LANG_BG_AB_NODE_TAKEN), (teamIndex == 0) ? GetHellgroundString(LANG_BG_AB_ALLY) : GetHellgroundString(LANG_BG_AB_HORDE), _GetNodeName(node));
                    WorldPacket data;
        ...
        if (m_TeamScores[TEAM_ALLIANCE] >= 2000) {
            //RewardHonorToTeam(WIN_HONOR_COINT, ALLIANCE);
            //RewardHonorToTeam(20, HORDE);
            EndBattleGround(ALLIANCE);
        }
        if (m_TeamScores[TEAM_HORDE] >= 2000) {
            //RewardHonorToTeam(WIN_HONOR_COINT, HORDE);
...
const char* BattleGroundAB::_GetNodeName(uint8 node)
{
    switch (node)
    {
        case BG_AB_NODE_STABLES:
            return GetHellgroundString(LANG_BG_AB_NODE_STABLES);
        case BG_AB_NODE_BLACKSMITH:
            return GetHellgroundString(LANG_BG_AB_NODE_BLACKSMITH);
        case BG_AB_NODE_FARM:
            return GetHellgroundString(LANG_BG_AB_NODE_FARM);
        case BG_AB_NODE_LUMBER_MILL:
            return GetHellgroundString(LANG_BG_AB_NODE_LUMBER_MILL);
        case BG_AB_NODE_GOLD_MINE:
            return GetHellgroundString(LANG_BG_AB_NODE_GOLD_MINE);
        default:
            ASSERT(0);
    }
    return "";
...
void BattleGroundAB::_NodeOccupied(uint8 node, PlayerTeam team)
{
...
//   SpawnBGCreature(node,RESPAWN_IMMEDIATELY);
    ...
    for (uint8 i = 0; i < BG_AB_DYNAMIC_NODES_COUNT; ++i)
    {
        if (m_Nodes[node] == GetTeamIndexByTeamId(team) + BG_AB_NODE_TYPE_OCCUPIED && !m_NodeTimers[i].GetInterval())
            ++capturedNodes;
    }
    if (capturedNodes >= 5)
...
void BattleGroundAB::EventPlayerClickedOnFlag(Player *source, GameObject* /*target_obj*/)
{
    ...
    if (m_Nodes[node] == BG_AB_NODE_TYPE_NEUTRAL)
    {
        ...
        SendObjectiveComplete(BG_AB_CREDITMARKER[node], source->GetBGTeam()/*, BG_AB_NodePositions[node][0], BG_AB_NodePositions[node][1]*/);
    }
    ...
    else if ((m_Nodes[node] == BG_AB_NODE_STATUS_ALLY_CONTESTED) || (m_Nodes[node] == BG_AB_NODE_STATUS_HORDE_CONTESTED))
    {
        ...
        else
        {
            ...
            m_NodeTimers[node] = 0;
            _NodeOccupied(node,(teamIndex == 0) ? ALLIANCE:HORDE);
            sprintf(buf, GetHellgroundString(LANG_BG_AB_NODE_DEFENDED), _GetNodeName(node));
        }
...
WorldSafeLocsEntry const* BattleGroundAB::GetClosestGraveYard(float x, float y, float /*z*/, PlayerTeam team)
{
