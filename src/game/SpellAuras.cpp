...
pAuraHandler AuraHandler[TOTAL_AURAS]=
{
    ...
    &Aura::HandleLongSight,                                 // 76 SPELL_AURA_LONG_SIGHT
    ...
    &Aura::HandleAuraRepPct,                                //156 SPELL_AURA_MOD_REPUTATION_GAIN
    ...
    &Aura::HandleAuraXpPct,                                 //200 SPELL_AURA_MOD_XP_PCT implemented in Player::GiveXP
    ...
    &Aura::HandleAuraPermanentTransform,                    //255 SPELL_AURA_PERMANENT_TRANSFORM
    &Aura::HandleSendSpellVisualPeriodic,                   //256 SPELL_AURA_SEND_SPELL_VISUAL_PERIODIC
    &Aura::HandleUnused,                                    //257 unused
    &Aura::HandleUnused,                                    //258 unused
    &Aura::HandleUnused,                                    //259 unused
    &Aura::HandleSpectatorVisible,                          //260 SPELL_AURA_SPECTATOR_VISIBLE
    &Aura::HandleUnused                                     //261 UNUSED some phased state (44856 spell)
};
...
m_periodicTimer(0), m_isStealed(false), m_amplitude(0), m_PeriodicEventId(0), m_AuraDRGroup(DIMINISHING_NONE), m_heartbeatTimer(0)
, m_tickNumber(0), special_flag(AURA_SPECIAL_NONE)
{
    ASSERT(target);

    ASSERT(spellproto && spellproto == sSpellTemplate.LookupEntry<SpellEntry>(spellproto->Id) && "`info` must be pointer to sSpellStore element");
    ...
    if (!caster)
    {
        ...
        m_maxduration = target->GetDummyAura(54839) ? 15000 : target->CalculateSpellDuration(m_spellProto, m_effIndex, target);
    }
    else
    {
        ...
        //damage        = caster->CalculateSpellDamage(m_spellProto,m_effIndex,m_currentBasePoints);
        m_maxduration = caster->GetDummyAura(54839) ? 15000 : caster->CalculateSpellDuration(m_spellProto, m_effIndex, target);
...
Unit *caster, Item* castItem) : Aura(spellproto, eff, currentBasePoints, target, caster, castItem)
{
    ...
    switch (spellproto->Effect[eff])
    {
        ...
        case SPELL_EFFECT_APPLY_AREA_AURA_OWNER:
            ...
            break;
        default:
            sLog.outLog(LOG_CRASH, "ERROR: Wrong spell effect in AreaAura constructor");
            ASSERT(false);
            break;
    }
}
...
void Aura::Update(uint32 diff)
{
        ...
        if (GetEffIndex()==0 && m_timeCla <= 0)
        {
            if (Unit* caster = GetCaster())
            {
                ...
                if (manaPerSecond)
                {
                    ...
                    else // there are actually no spells that consume mana per second, only health
                    {
                        // checks for 0 and max power are in ModifyPower. Unlike Health per second, we don't need to check that
    ...
    if (SpellMgr::IsChanneledSpell(m_spellProto))
    {
        ...
        Unit *pRealTarget = (GetSpellProto()->EffectApplyAuraName[m_effIndex] == SPELL_AURA_PERIODIC_TRIGGER_SPELL &&
                             sSpellTemplate.LookupEntry<SpellEntry>(GetSpellProto()->EffectTriggerSpell[m_effIndex]) &&
                             !SpellMgr::IsAreaOfEffectSpell(sSpellTemplate.LookupEntry<SpellEntry>(GetSpellProto()->EffectTriggerSpell[m_effIndex])) &&
        ...
        if(!pRealTarget->isPossessed() && pRealTarget->GetGUID() != GetCasterGUID())
        {
            ...
            if (caster->GetUInt64Value(UNIT_FIELD_CHANNEL_OBJECT) == m_target->GetGUID())
            {
                ...
                if (Player* modOwner = caster->GetSpellModOwner())
                    modOwner->ApplySpellMod(GetId(), SPELLMOD_RANGE, max_range, NULL, false);

                if (!caster->IsWithinCombatRange(m_target, max_range * 1.2f))
                {
                    m_target->RemoveAura(GetId(), GetEffIndex());
                    return;
                }
	...
	// removed because some NPCs goes out of sheep for no reason! multithreading?
	// DANGEROUS?
...
bool AreaAura::CheckTarget(Unit *target)
{
    ...
    switch (GetId())
    {
        ...
        case 39331: // Game in Session - Karazhan.
            // When starting to control piece, it's faction changes and Game In Session starts affecting piece. Don't allow it
        {
            if (target->GetTypeId() == TYPEID_UNIT && target->HasAura(30019, 0)) // 'control piece' aura.
                return false;
            break; // to return true;
        }
        case 29521:
        {
            if (target->GetTypeId() == TYPEID_UNIT && target->GetEntry() >= 16406 && target->GetEntry() <= 16415 && target->GetEntry() != 16407)
                return true;
...
void AreaAura::Update(uint32 diff)
{
    ...
    else                                                    // aura at non-caster
    {
        ...
        // GENSENTODO: this breaks NPC 21224 "casting on 20% HP"
...
void PersistentAreaAura::Update(uint32 diff)
{
    ...
    if (caster)
    {
        ...
        if (dynObj)
        {
            // exact dist returns same distance no matter who is the callse of this method
            if (!m_target->IsInMap(dynObj) || fabs(dynObj->GetPositionZ() - m_target->GetPositionZ()) > DST_AREA_AURA_REACH_Z 
                || m_target->GetExactDistSq(dynObj->GetPositionX(), dynObj->GetPositionY(), dynObj->GetPositionZ()) > dynObj->GetRadiusSq())
                remove = true;
        }
        else
...
void Aura::UpdateAuraDuration()
{
    ...
    if (m_target->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        data << uint8(m_auraSlot);
        data << uint32(sSpellMgr.GetSpellAnalog(GetSpellProto()));
...
void Aura::_AddAura()
{
	...
	// Feign Death bug on max auras fix
	// 41 slot is a debuff slot, but who cares! it's an easy fix!
	if (GetId() == 5384)
		++max_slot;
        ...
        (m_spellProto->Effect[GetEffIndex()] != SPELL_EFFECT_APPLY_AREA_AURA_ENEMY || m_target != caster))
    {
        ...
        if (SpellMgr::IsSealSpell(GetSpellProto()) && special_flag == AURA_SPECIAL_NONE)
...
void Aura::_RemoveAura()
{
    ...
    if (!samespell)
    {
        ...
        if (caster && GetSpellProto()->Attributes & SPELL_ATTR_DISABLED_WHILE_ACTIVE && caster->GetCharmerOrOwnerPlayerOrPlayerItself() && !IsStealed())
        {
                    ...
                    case 30427:
                    {
                        ...
                        if (caster->GetTypeId() ==TYPEID_PLAYER && target->GetTypeId() == TYPEID_UNIT && ((Creature*)target)->GetCreatureInfo()->type == CREATURE_TYPE_GAS_CLOUD)
                        {
                            ...
                            for (uint8 i=0;i<loot->items.size();i++)
                            {
                                ...
                                if (msg == EQUIP_ERR_OK)
                                {
                                    Item * newitem = player->StoreNewItem(dest, item->itemid, true, item->randomPropertyId, "SPELL");
...
//                    case 38794: break;
                    ...
                    case 39105:
                    {
                        if (!target)
                            return;
                        if (target->GetTypeId() == TYPEID_PLAYER && (((Player*)target)->GetQuestStatus(10832) == QUEST_STATUS_INCOMPLETE))
                        {
                            float fX, fY, fZ;
                            target->GetNearPoint(fX, fY, fZ, target->GetObjectBoundingRadius(), 20.0f);
                            if (Creature* sum = target->SummonCreature(22408, fX, fY, fZ, target->GetOrientation(), TEMPSUMMON_DEAD_DESPAWN, 0))
...
//                    case 51510: break;
                    ...
                    case 42582:                             // Spout (right)
                    {
                        ...
                        target->CastSpell(target, auraSpellEntry->CalculateSimpleValue(m_effIndex), true);
                        return;
                    }
...
//                        return;
        ...
        triggeredSpellEntry = sSpellTemplate.LookupEntry<SpellEntry>(trigger_spell_id);
        if (triggeredSpellEntry == NULL)
        {
    ...
    else
    {
        ...
        switch (auraId)
        {
            ...
            case 28158: // Poison Cloud (Grobbulus)
            case 30914: // Broggok Poison Cloud
            case 38462: // Broggok Poison Cloud Heroic
            {
                caster->CastCustomSpell(trigger_spell_id, SPELLVALUE_RADIUS_MOD, (((((float)m_tickNumber / 60) * 0.9f + 0.1f) * 10000 * 2 / 3)), NULL, true, NULL, this, originalCasterGUID);
                return;
            }
            ...
            case 44197:
            {
                uint32 damage = caster->CalculateSpellDamage(triggeredSpellEntry, 0, 1750);
            ...
            case 46680:
            {
                ...
                if (p_Creature)
                    ...
                    m_target->CastSpell(target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), triggeredSpellEntry->Id, true, 0, this, originalCasterGUID);
                return;
            }
        }
    }
...
{
    Unit* target = m_target->GetMap()->GetUnit(m_target->GetSelection());
...
void Aura::HandleAuraDummy(bool apply, bool Real)
{
    ...
    if (apply)
    {
        switch (GetId())
        {
            ...
            case 31399:
                if(m_target)
                    m_target->CastSpell(m_target, 6869, true);
                return;
            case 34367:
                if(!caster)
                    return;
            ...
            case 7057:                                      // Haunting Spirits
                m_isPeriodic = true;
                m_modifier.periodictime = 30*MILLISECONDS;
                m_periodicTimer = m_modifier.periodictime;
                m_amplitude = m_modifier.periodictime;
                return;
            case 10255:                             // Stoned
            ...
            case 6946:                                     // Curse of Bleakheart
                ...
                return;
            //case SPELL_AURA_PLAYER_INACTIVE:                                     // Inactive
            //    m_periodicTimer = sWorld.getConfig(CONFIG_BATTLEGROUND_KICK_AFTER_INACTIVE_TIME);
            //    if (m_periodicTimer)
            //        m_isPeriodic = true;
            //    return;
            ...
            //case 54863:
            //{
            //    if (caster && m_target->GetTypeId() == TYPEID_UNIT && m_target->ToCreature()->GetCreatureInfo()->Entry == 800001)
            //    {
            //        if (Unit* victim = caster->GetVictim())
            //        {
            //            m_target->ToCreature()->AI()->AttackStart(victim);
            //            victim->CastSpell(victim, 54864, true); // Faction disabler for the player
            //        }
            //    }
            //    return;
            //}
            //case 54863:
            //{
            //    if (caster && m_target->GetTypeId() == TYPEID_PLAYER)
            //        caster->AddAura(38044, caster);
    ...
    else
    {
        ...
        if ((IsQuestTameSpell(GetId())) && caster && caster->isAlive() && m_target->isAlive() && m_removeMode == AURA_REMOVE_BY_EXPIRE)
        {
        ...
        switch (GetId())
        {
            case 11826:
                ...
                if (Unit* caster = GetCaster())
                    if (caster->GetTypeId() == TYPEID_PLAYER)
                        caster->CastSpell(m_target, 11828, true, ((Player*)caster)->GetItemByGuid(GetCastItemGUID()), this);
                break;
            case 34367:
                if(!caster)
                    return;
            ...
            case 35831:
            {
                ...
                if (m_removeMode != AURA_REMOVE_BY_EXPIRE)
                    m_target->CastSpell(m_target, 35995, false);
                return;
            }
                    ...
                    if (m_removeMode == AURA_REMOVE_BY_EXPIRE)
                    {
                        if (urand(0,10) > 2)
                        {
                            ...
                            for(int i = 0; i < count; ++i)
                                caster->SummonCreature(urand(0,1) ? 22482 : 22483, m_target->GetPositionX()+(rand()%5), m_target->GetPositionY()+(rand()%5), m_target->GetPositionZ(), m_target->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 60000);
                        }
                        else
                            caster->SummonCreature(22038, m_target->GetPositionX()+(rand()%5), m_target->GetPositionY()+(rand()%5), m_target->GetPositionZ(), m_target->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 60000);
                    }
                }
                return;
            }
            case 39238:                                     // Fumping
            {
                ...
                if (Unit* caster = GetCaster())
                {
                    ...
                    {
                        if(urand(0,1) == 0)
                            caster->SummonCreature(22482, m_target->GetPositionX()+(rand()%5), m_target->GetPositionY()+(rand()%5), m_target->GetPositionZ(), m_target->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 60000);
                        else
                        {
                            caster->SummonCreature(22483, m_target->GetPositionX()+(rand()%5), m_target->GetPositionY()+(rand()%5), m_target->GetPositionZ(), m_target->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 60000);
                            caster->SummonCreature(22483, m_target->GetPositionX()+(rand()%5), m_target->GetPositionY()+(rand()%5), m_target->GetPositionZ(), m_target->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 60000);
                            caster->SummonCreature(22483, m_target->GetPositionX()+(rand()%5), m_target->GetPositionY()+(rand()%5), m_target->GetPositionZ(), m_target->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 60000);
                        }
                    }
            ...
            {
                if (caster)
                    caster->CastSpell(m_target, 29321, true);
                return;
            }
            case 45716:
            case 46630:
            {
                m_target->RemoveAurasDueToSpell(45724);
                return;
            }
            case 10255:                                     // Stoned
            {
            ...
            case 55148:
                {
                    ...
                    Player* targ = ((Player*)m_target);

                    for (uint8 i = 0; i < 3; ++i)
                    {
                        Item* item = targ->GetItemByPos(255, EQUIPMENT_SLOT_MAINHAND + i);
                        if (item && item->GetEntry() != targ->GetUInt32Value(PLAYER_VISIBLE_ITEM_16_0 + i * 16))
                            targ->ForceValuesUpdateAtIndex(PLAYER_VISIBLE_ITEM_16_0 + i * 16);
                    }
                    return;
                }
            case 55182:
                {
                    if (m_target->GetTypeId() != TYPEID_PLAYER || !m_castItemGuid || m_removeMode != AURA_REMOVE_BY_EXPIRE)
                        return;

                    m_target->CastSpell(m_target, m_modifier.m_amount, true); // recast mount on us
                    return;
                }
        }
    }
    ...
    switch (m_spellProto->SpellFamilyName)
    {
            ...
            if (GetSpellProto()->SpellFamilyFlags & 0x1000000000LL)
            {
                if (apply)
                {
                    if (caster)
                        // prevent double apply bonuses
                        if (m_target->GetTypeId()!=TYPEID_PLAYER || !((Player*)m_target)->GetSession()->PlayerLoading())
                            m_modifier.m_amount = caster->SpellHealingBonus(GetSpellProto(), caster->CalculateSpellDamage(GetSpellProto(), GetEffIndex(), GetBasePoints()), SPELL_DIRECT_DAMAGE, m_target);
                }
                ...
                if (apply)
                {
                    ...
                    switch (m_effIndex)
                    {
                        ...
                        case 1:
                            ...
                            mod->mask = 0x00002000000LL;    // Flametongue Totem
                            break;
                    }
                    mod->charges = 0;
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if (GetSpellProto()->SpellIconID == 237)
            {
                if (m_target->GetTypeId() != TYPEID_PLAYER)
                    return;
                ...
                if (apply)
                {
                    ...
                    mod->effectId = m_effIndex;
                    mod->lastAffected = NULL;
                    mod->mask = 0x800000000000LL;    // Crusader Strike
                    mod->charges = 0;
...
void Aura::HandleAuraPeriodicDummy(bool apply, bool Real)
{
    ...
    switch (spell->Id)
    {
        case 55193: // shop buy spell
        {
            ((Player*)m_target)->ForceValuesUpdateAtIndex(PLAYER_FIELD_COINAGE);
            if (!apply)
            {
                ACE_GUARD(ACE_Thread_Mutex, guard, sWorld.GetShop()->plrVariableMutex);
                auto itr = sWorld.GetShop()->plrVariable.find(m_target->GetGUID());
                if (itr != sWorld.GetShop()->plrVariable.end())
                    sWorld.GetShop()->plrVariable.erase(itr);

                ChatHandler(((Player*)m_target)).SendSysMessage(((Player*)m_target)->GetSession()->GetHellgroundString(LANG_SHOWING_GOLD));
            }
            else
            {
                Creature* target = NULL;
                Hellground::NearestCreatureEntryWithLiveStateInObjectRangeCheck check(*m_target, GetModifierValue()/*creature id*/, true, 10.0f, false);
                Hellground::ObjectSearcher<Creature, Hellground::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(target, check);
                Cell::VisitAllObjects(m_target, searcher, 10.0f);

                ChatHandler(((Player*)m_target)).SendSysMessage(((Player*)m_target)->GetSession()->GetHellgroundString(LANG_SHOWING_COINS));

                ((Player*)m_target)->SendCenterMessage(LANG_SHOWING_COINS_BIG);
                ChatHandler(((Player*)m_target)).SendSysMessage(((Player*)m_target)->GetSession()->GetHellgroundString(LANG_SHOWING_COINS_BIG));
            }
            break;
        }
        // called in PeriodicDummyTick(); by a SPELL_AURA_DUMMY on apply            
        /*case 55153: // Warden Warning Freeze
        {
            if (apply)
            {
                ChatHandler(((Player*)m_target)).SendSysMessage(LANG_WARDEN_CHEAT_DETECTED);
                ((Player*)m_target)->GetSession()->SendNotification(((Player*)m_target)->GetSession()->GetHellgroundString(LANG_WARDEN_CHEAT_DETECTED));
                ((Player*)m_target)->SendPlaySound(5805, true);
            }
            break;
        }*/
    }
...
void Aura::HandleAuraMounted(bool apply, bool Real)
{
    ...
    if (apply)
    {
        ...
        if (m_target->GetDummyAura(54839))
            display_id = m_target->GetHealth();
...
void Aura::HandleAuraModShapeshift(bool apply, bool Real)
{
    ...
    if (apply)
    {
        //if (m_target->HasAura(55119))
        //    m_target->RemoveAurasDueToSpell(55119);
        ...
        if (!(form == FORM_BATTLESTANCE || form == FORM_DEFENSIVESTANCE || form == FORM_BERSERKERSTANCE))
        ...
        if (PowerType != POWER_MANA)
        {
            ...
            switch (form)
            {
                ...
                case FORM_DIREBEAR:
                {
                    ...
                    // disabled because it's annoying to turn it off each time
                    // reverted, because it's blizzlike :)
                    if (form == FORM_CAT)
                    {
                    ...
                    if (m_target->GetTypeId() == TYPEID_PLAYER)
                    {
                        ...
                        for (PlayerSpellMap::const_iterator itr = sp_list.begin(); itr != sp_list.end(); ++itr)
                        {
                            ...
                            if (spellInfo && spellInfo->SpellFamilyName == SPELLFAMILY_WARRIOR && spellInfo->SpellIconID == 139)
                                Rage_val += m_target->CalculateSpellDamage(spellInfo,0,spellInfo->EffectBasePoints[0]) * 10;
                        }
                    }
    ...
    else
    {
        ...
        {            
            ...
            {
                //if (!m_target->HasAura(55119))
                //    m_target->CastSpell(m_target, 55119, true);
            ...
            //// race shirt
            //bool has_morph_shirt = m_target->HasAura(55401);

            //if (has_morph_shirt)
            //{
            //    Item *pItem = ((Player*)m_target)->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_BODY);
            //    ItemPrototype const *proto = pItem->GetProto();
            //    if (proto->Spells[0].SpellId == 55401 && proto->Spells[0].SpellPPMRate > 0)
            //    {
            //        m_target->SetDisplayId(proto->Spells[0].SpellPPMRate);
            //    }
            //}
        }
...
{    
    if (m_target->InClassForm())
        return;
    
    if (apply)
    {       
        if (GetSpellProto()->Category == SPELL_CATEGORY_MORPH_SHIRT)
        {
            if (Item *pItem = ((Player*)m_target)->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_BODY))
        ...
        else
        {
            ...
            else
            {
                ...
                if (uint32 modelid = ci->GetRandomValidModelId())
                {
                    ...
                    if (m_target->GetTypeId() == TYPEID_PLAYER)
                    {
                        if ((((Player*)m_target)->GetTransmogManager()->IsAcceptableTransmogrModel(modelid) && ((Player*)m_target)->GetTransmogManager()->HasActiveTransmogrification()) || ((Player*)m_target)->GetTransmogManager()->IsRaceShirtModel(modelid))
                        {
                            if (!m_target->HasAura(55119))
                            {
                                m_target->CastSpell(m_target, 55119, true);
                                if (GetId() != 55118)
                                {
                                    m_target->CastSpell(m_target, 55118, true);
                                    DoSet = false;
                                }
                            }
                        }
                        else if (m_target->HasAura(55119))
                            m_target->RemoveAurasDueToSpell(55119);
                    }
                    if (DoSet)
                        m_target->SetDisplayId(modelid);
    ...
    else
    {
        ...
        if (otherTransforms.empty())
        {
            ...
            if (m_target->GetTypeId() == TYPEID_PLAYER)
            {
                if (((Player*)m_target)->GetTransmogManager()->HasActiveTransmogrification())
                {
                    if (!m_target->HasAura(55119))
                    {
                        m_target->CastSpell(m_target, 55119, true);
...
void Aura::HandleChannelDeathItem(bool apply, bool Real)
{
    if (Real && !apply)
    {
        ...
        if (spellInfo->EffectItemType[m_effIndex] == 6265 &&
            ...
            (victim->GetTypeId()==TYPEID_UNIT && ((Creature*)victim)->isTotem())))
            return;
        ...
        if (Item* newitem = ((Player*)caster)->StoreNewItem(dest, spellInfo->EffectItemType[m_effIndex], true, 0, "AURA"))
...
void Aura::HandleLongSight(bool apply, bool Real)
{
    /*Unit* caster = GetCaster();
    if (!caster || caster->GetTypeId() != TYPEID_PLAYER)
        return;*/

    // This should increase player's view distance?
}
...
void Aura::HandleModPossess(bool apply, bool Real)
{
    ...
    else
    {
        ...
        if (GetId() == 45839)
        {
            ...
            if(caster->GetTypeId() == TYPEID_PLAYER)
            {
                if (((Player*)caster)->GetLastPetNumber() && caster->isAlive())
                {
                    Pet* NewPet = new Pet();
    
                    if (!NewPet->LoadPetFromDB(caster, 0, ((Player*)caster)->GetLastPetNumber(), true))
                        delete NewPet;
                    //restore pet's Health and Mana
                    else if (caster->GetClass() == CLASS_HUNTER)
                    {
                        NewPet->SetHealth(NewPet->GetMaxHealth());
                        NewPet->SetPower(POWER_HAPPINESS ,NewPet->GetMaxPower(POWER_HAPPINESS));
                    }
                    else if (caster->GetClass() == CLASS_WARLOCK)
                    {
                        NewPet->SetHealth(NewPet->GetMaxHealth());
    ...
    {
        if (apply)
            m_target->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE);
        else
        {
            m_target->ToCreature()->AI()->EnterEvadeMode();
            m_target->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE, false);
        }
    }
    if(GetId() == 29989)
    {
        ...
        else
        {
            ...
            m_target->ToCreature()->DisappearAndDie();
        }
    }
}
...
void Aura::HandleModCharm(bool apply, bool Real)
{
    ...
    {
        m_target->RemoveCharmedOrPossessedBy(caster);
        // after enslave demon -> remove soul link
...
void Aura::HandleModFear(bool apply, bool Real)
{
    ...
    // moved to Unit.cpp 4260
    //if (GetTarget()->HasAuraType(SPELL_AURA_PREVENTS_FLEEING))
    //    return;
...
void Aura::HandleFeignDeath(bool apply, bool Real)
{
    ...
    {       
        ...
        for (std::list<Unit*>::iterator iter = targets.begin(); iter != targets.end(); ++iter)
        {
            ...
            if((*iter)->GetTypeId() != TYPEID_PLAYER &&// this is not needed for players. 1. FeignDeath removes target client-side. 2. Setting 0 selection won't affect client at all.
                (*iter)->GetSelection() == m_target->GetGUID()) 
                (*iter)->SetSelection(0);
        ...
        if (m_target->GetTypeId()==TYPEID_PLAYER)
            p_target->SendAttackSwingCancelAttack();     // melee and ranged forced attack cancel

        // feign death in pve
        HostileReference *ref = m_target->getHostileRefManager().getFirst();
        bool encounter = m_target->GetInstanceData() && m_target->GetInstanceData()->IsEncounterInProgress();
        ...
        while(ref)
        {
            ...
            if (target->IsContestedGuard() || target->isGuard())
                modHitChance = -1; // 100% resist
            else
                modHitChance = 100;
            ...
            if(rand <= modHitChance) // hit
            {
                ...
                if (!encounter)
                {
                    ...
                    //sLog.outLog(LOG_SPECIAL, "Feign Death used by %s (#0, apply %u)", m_target->GetName(), apply);
                }
                else
                {
                    if (target->GetTypeId() == TYPEID_UNIT && ((Creature*)target)->IsDoZoneInCombatThreat())
                    {
                        ref->addThreatPercent(-100); //ref->setThreat(0);
                        leaveCombat = false;
                        //sLog.outLog(LOG_SPECIAL, "Feign Death used by %s (#1, apply %u)", m_target->GetName(), apply);
                    }
                    else
                    {
                        ref->removeReference();
                        ...
                        {
                            target->AttackStop();
                            //sLog.outLog(LOG_SPECIAL, "Feign Death used by %s (#2, apply %u)", m_target->GetName(), apply);
                        }

                        //sLog.outLog(LOG_SPECIAL, "Feign Death used by %s (#3, apply %u)", m_target->GetName(), apply);
                    }
                }
            }
            else // miss
            {
                ...
                //sLog.outLog(LOG_SPECIAL, "Feign Death used by %s (#4, apply %u)", m_target->GetName(), apply);
            }
...
void Aura::HandleModStealth(bool apply, bool Real)
{
    ...
    if (apply)
    {
        ...
        if (Real)
        {
            pTarget->SetByteFlag(UNIT_FIELD_BYTES_1, 2, 0x02);
            ...
            // add Shadowmeld
            if (m_target->GetTypeId() == TYPEID_PLAYER && m_target->GetRace() == RACE_NIGHTELF && (m_target->GetClass() == CLASS_ROGUE || m_target->GetClass() == CLASS_DRUID))
    ...
    else
    {
        ...
        if (Real)
        {
            ...
            {
                pTarget->RemoveByteFlag(UNIT_FIELD_BYTES_1, 2, 0x02);
                if (pTarget->GetTypeId() == TYPEID_PLAYER)
            ...
            {
                if (pTarget->HasAura(21009))
                    pTarget->RemoveAurasDueToSpell(21009);

                if (pTarget->HasAura(20580))
                    pTarget->RemoveAurasDueToSpell(20580);
            }
...
void Aura::HandleAuraModRoot(bool apply, bool Real)
{
    ...
    if (GetId() == 35247 && apply)
        GetTarget()->RemoveAurasDueToSpell(35244);
...
//    case 12788:
//    case 12789:
//        if (apply && (target->m_form != FORM_DEFENSIVESTANCE || target->m_form == FORM_NONE))
...
//        {
//            float val = float(m_modifier.m_amount) / 100.f;
//            target->m_threatModifier[x] += (apply) ? val : -val;
//        }
//        //ApplyPercentModFloatVar(target->m_threatModifier[x], float(m_modifier.m_amount), apply);
//
//    Player* player = (Player*)target;
//    ChatHandler(player).PSendSysMessage("Threat %f %f %f %f %f %f %f", target->m_threatModifier[0], target->m_threatModifier[1], target->m_threatModifier[2], target->m_threatModifier[3], target->m_threatModifier[4], target->m_threatModifier[5], target->m_threatModifier[6], target->m_threatModifier[7]);
//}
        ...
        for (int8 x=0;x < MAX_SPELL_SCHOOL;x++)
            if (m_modifier.m_miscvalue & int32(1<<x))
            {
                if (!apply && !m_target->m_threatModifier[x])
                {
                    ...
                    for (Unit::AuraList::const_iterator i = threatAuras.begin();i != threatAuras.end(); ++i)
                    {
						if ((*i)->GetModifier()->m_miscvalue & int32(1 << x) && (*i) != this)
						{
							...
							// PALADIN_THREATBUG
							//if (m_target->GetTypeId() == TYPEID_PLAYER && m_target->GetClass() == CLASS_PALADIN)
								//sLog.outLog(LOG_SPECIAL, "Paladin %s HandleModThreat#1! aura: %u (%s), x: %d, m_amount %d, apply: %d", m_target->GetName(), m_spellProto->Id, GetSpellProto()->SpellName[0], (*i)->GetModifier()->m_amount, apply);
						}
                    }
                }
				else
				{
					...
					// PALADIN_THREATBUG
					//if (m_target->GetTypeId() == TYPEID_PLAYER && m_target->GetClass() == CLASS_PALADIN)
						//sLog.outLog(LOG_SPECIAL, "Paladin %s HandleModThreat#2! aura: %u (%s), m_amount %d, apply: %d", m_target->GetName(), m_spellProto->Id, GetSpellProto()->SpellName[0], m_modifier.m_amount, apply);
				}
            }
    }

    //Player* player = (Player*)m_target;
    //ChatHandler(player).PSendSysMessage("Threat %f %f %f %f %f %f %f", m_target->m_threatModifier[0], m_target->m_threatModifier[1], m_target->m_threatModifier[2], m_target->m_threatModifier[3], m_target->m_threatModifier[4], m_target->m_threatModifier[5], m_target->m_threatModifier[6], m_target->m_threatModifier[7]);
}
...
void Aura::HandleAuraModDecreaseSpeed(bool apply, bool Real)
{
    ...
    if (apply)
    {
        ...
        {
            if (GetStackAmount() >= 5)
                m_target->CastSpell(m_target, 35247, true);
        }
    }
...
void Aura::HandleModMechanicImmunity(bool apply, bool Real)
{
    if (apply && GetSpellProto()->AttributesEx & SPELL_ATTR_EX_DISPEL_AURAS_ON_IMMUNITY)
    {
        ...
        if (GetId() == 42292 || GetId() == 46227) // 46227 - NPC version in MgT
        {
            ...
            if (m_target->GetTypeId() == TYPEID_PLAYER)
                m_target->ToPlayer()->SendGladdyNotification();
            if (m_target->HasAuraType(SPELL_AURA_USE_NORMAL_MOVEMENT_SPEED))
                m_target->RemoveAurasDueToSpell(31896); // Judgement of Justice level 2 - Unique interaction with trinket. Not considered movement impairing, but is removed by trinket
        }
    ...
    // apply immunity to CC
    if (GetId() == 19574 || //The Beast Within
        GetId() == 34471) //Bestial Wrath
    {
        if (apply)
        {
...
void Aura::HandleAuraModEffectImmunity(bool apply, bool Real)
{
    if (!apply)
    {
        if (m_target->GetTypeId() == TYPEID_PLAYER)
        {
            if (((Player*)m_target)->InBattleGroundOrArena())
            {
                if (BattleGround *bg = ((Player*)m_target)->GetBattleGround())
                {
                    switch (bg->GetTypeID())
...
void Aura::HandlePeriodicTriggerSpell(bool apply, bool Real)
{
    ...
    if (!apply)
    {
        switch (m_spellProto->Id)
        {
            case 42483: // Ooze Channeling casts Ooze Channeling End
            {
                ...
                if (Unit* caster = GetCaster())
                {
                    if(caster->HasAura(42490))
                    {
                        caster->CastSpell(m_target, 42485, true);
                        if(m_target->GetEntry() == 4394 || m_target->GetEntry() == 4393)
                            ((Player*)caster)->KilledMonster(23797, 0);
                    }
                }
                break;
            }
...
void Aura::HandlePeriodicDamage(bool apply, bool Real)
{
    ...
    switch (m_spellProto->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            if(!apply && m_spellProto->Id == 35201)
            {
                if (m_removeMode == AURA_REMOVE_BY_EXPIRE)
                    m_target->CastSpell(m_target, 35202, false, NULL, this);
                return;
            }
            // Pounce Bleed
        ...
        {
            // Curse of Doom Doomguard summon
            if (m_spellProto->SpellFamilyFlags & 0x0000000200000000LL)
            {
                if (!apply && (m_duration <= 0) && !loading && caster && (m_target->GetHealth() <= m_modifier.m_amount))
                {
                    if (urand(0, 4) < 1) // 20%
...
{
    if (m_spellProto->Id == 5024) // Flee - Skull of Impending Doom
        m_modifier.m_amount = m_target->GetMaxPower(POWER_MANA) * 3 / 25; // 12% per tick
    ...
    if (apply)
    {
        ...
        if (m_periodicTimer <= 0)
        {
            ...
            if (Unit *caster = GetCaster())
            {
                ...
                if (spellProto)
                    m_target->getHostileRefManager().threatAssist(caster, float(gain) * 0.5f * sSpellMgr.GetSpellThreatMultiplier(spellProto), spellProto);
            }
        }
    }
...
void Aura::HandleAuraModIncreaseEnergy(bool apply, bool Real)
{
    ...
    if (int32(powerType) != m_modifier.m_miscvalue) // cant check for powerType -> druid powerType is dynamic. Can exploit that
        return;*/ // On intellect change power_mana is always updated, no matter which form you are in or which power you have
	...
	// NetherSpite green beam hackfix
	if (m_spellProto->Id == 30422 && m_target->getPowerType() == POWER_MANA)
...
void Aura::HandleRangedAmmoHaste(bool apply, bool Real)
{
    ...
    else
    {
        ...
        if (Real)
        {
            if (Item* pItem = ((Player*)m_target)->GetWeaponForAttack(RANGED_ATTACK))
                if (!pItem->IsBroken())
                    ((Player*)m_target)->_ApplyWeaponDependentAuraRangedHasteMod(pItem,this,apply);
        }
        // done in Player::_ApplyWeaponDependentAuraMods
    }
}
...
void Aura::HandleModDamageDone(bool apply, bool Real)
{
    ...
    if ((m_modifier.m_miscvalue & SPELL_SCHOOL_MASK_NORMAL) != 0)
    {
        ...
        if (GetSpellProto()->EquippedItemClass == -1 || m_target->GetTypeId() != TYPEID_PLAYER)
        {
            ...
            if (m_target->GetTypeId() == TYPEID_PLAYER) // EquippedItemClass == -1
            {
                if (m_positive)
...
void Aura::HandleModPowerCostPCT(bool apply, bool Real)
{
    ...
    if (m_spellProto->Id == 54814 && m_target->GetClass() == CLASS_ROGUE) // cause of envenom...
        return;
...
{
    if (m_target->GetTypeId() != TYPEID_PLAYER)
        sLog.outLog(LOG_DB_ERR, "WARNING: Creature id %u is affected by Pacify spell %u.", m_target->GetEntry(), GetId());
...
void Aura::HandleModUnattackable(bool Apply, bool Real)
{
    if (Real && Apply)
    {
        // WoWHEad comment on petrification: 
        // "you'll be removed from combat when you use it, but if it's a boss fight you'll enter combat immediately afterward if your team-mates are still in battle."
        // so it should remove you from combat, but not for the boss-fights. Same as vanish or feign death.

        // Bosses in BC do DoZoneInCombat() with pulses, and it adds in combat those who have mod_unattackable, 
        // so we are free NOT TO REMOVE COMBAT from NPCs who added combat by DoZoneInCombat (same logic as feign death/vanish)

        m_target->CombatStopExceptDoZoneInCombat();

        m_target->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_UNATTACKABLE);
    }

    m_target->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE,Apply);
}
...
void Aura::HandleAuraAoeCharm(bool apply, bool Real)
{
    ...
    // apply immunity to CC
    if (GetId() == 38511 || GetId() == 36797) // Vashj Persuasion & TK Mind Control
    {
        if (apply)
        {
            m_target->CastSpell(m_target, 24395, true);
...
void Aura::HandleSchoolAbsorb(bool apply, bool Real)
{
    ...
    if (apply && (m_target->GetTypeId()!=TYPEID_PLAYER || !((Player*)m_target)->GetSession()->PlayerLoading()))
    {
        if (Unit* caster = GetCaster())
        {
            ...
            if (caster->GetTypeId() == TYPEID_PLAYER)
                m_target->getHostileRefManager().threatAssist(caster, float(m_modifier.m_amount) * 0.25f * sSpellMgr.GetSpellThreatMultiplier(GetSpellProto()), GetSpellProto());
        }
    }
}
...
void Aura::PeriodicTick()
{
    ...
    switch (spell->Id)
    {
        ...
        {
            m_target->RemoveAurasDueToSpell(SPELL_AURA_BG_PLAYER_IDLE);
    ...
    switch (m_modifier.m_auraname)
    {
        ...
        case SPELL_AURA_PERIODIC_DAMAGE_PERCENT:
        {
                ...
                switch (GetId())
                {
                    ...
                    case 38772:
                    {
                        uint32 percent =
                            ...
                            pCaster->CalculateSpellDamage(GetSpellProto(),GetEffIndex()+1,GetSpellProto()->EffectBasePoints[GetEffIndex()+1]) :
                            100;
            ...
            else
            {
                ...
                if (pCaster->GetObjectGuid().IsCreature())
                    damageInfo.damage *= ((Creature*)pCaster)->GetCreatureDamageMod();
            }
            ...
            pCaster->DealDamage(&damageInfo, DOT, spellProto, true, duel_just_ended);
            if (duel_just_ended)
                return;
        ...
        case SPELL_AURA_PERIODIC_LEECH:
        {
            ...
            SpellDamageLog damageInfo(sSpellMgr.GetSpellAnalog(GetSpellProto()), GetCaster(), GetTarget(), GetSpellProto()->SchoolMask, 1);
            ...
            if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_WARLOCK && (GetSpellProto()->SpellFamilyFlags & 0x8))
            {
                ...
                for (Unit::AuraList::const_iterator i = mClassScriptAuras.begin(); i != mClassScriptAuras.end(); ++i)
                {
                    if ((*i)->GetModifier()->m_miscvalue == 4992 || (*i)->GetModifier()->m_miscvalue == 4993)
                    {
                        ...
                        // effect 0 m_amount
                        int32 stepPercent = pCaster->CalculateSpellDamage((*i)->GetSpellProto(),0,(*i)->GetSpellProto()->EffectBasePoints[0]);
...
//            pCaster->SendSpellNonMeleeDamageLog(m_target, GetId(), pdamage, GetSpellSchoolMask(GetSpellProto()), absorb, resist, false, 0);
            ...
            // @spell_fix how to reproduce: lock1 casts siphon life or drain life on low HP lock2. =Lock2 never dies
            // this is moved from upper lines just below
            ...
            bool duel_just_ended = false;
            uint32 new_damage = pCaster->DealDamage(&damageInfo, DOT, spellProto, false, duel_just_ended);
            ...
            if (pCaster->isAlive())
            {
                ...
                // change it
                pCaster->SendHealSpellLog(pCaster, sSpellMgr.GetSpellAnalog(spellProto), heal);
            }
            break;
        }
        ...
        case SPELL_AURA_OBS_MOD_HEALTH:
        {
            ...
            if (pCaster->IsInCombat())
                m_target->getHostileRefManager().threatAssist(pCaster, float(gain) * 0.5f * sSpellMgr.GetSpellThreatMultiplier(GetSpellProto()), GetSpellProto());
        ...
        case SPELL_AURA_PERIODIC_MANA_LEECH:
        {
            ...
            if (gain_amount)
            {
                ...
                if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_WARLOCK && GetSpellProto()->SpellFamilyFlags & 0x10)
                    ...
                    {
                        // Mana Feed
                        int32 manaFeedVal = pCaster->CalculateSpellDamage(GetSpellProto(),1, GetSpellProto()->EffectBasePoints[1]);
            ...
            {
                m_target->CastSpell(m_target, 31463, true, 0, NULL, m_target->GetGUID());
            ...
            // Mark of Kazzak
            if (GetId() == 21056 && m_target->getPowerType() == POWER_MANA && !m_target->GetPower(POWER_MANA))
            {
        ...
        case SPELL_AURA_POWER_BURN_MANA:
        {
            ...
            SpellDamageLog damageInfo(sSpellMgr.GetSpellAnalog(spellProto), pCaster, m_target, spellProto->SchoolMask);
        ...
        case SPELL_AURA_PERIODIC_TRIGGER_SPELL_WITH_VALUE:
            ...
            break;
        case SPELL_AURA_SEND_SPELL_VISUAL_PERIODIC:
            m_target->SendSpellVisual(GetModifier()->m_miscvalue);
            break;
        default:
            break;
    }
        ...
        case 42312:
        {
            ...
            for (Unit::AuraList::const_iterator i = aura.begin(); i != aura.end(); ++i)
            {
                if ((*i)->GetId() == GetId())
                {
                    ...
                    // on 2 tick -   0  (handled in 4 second)
                    // on 3 tick - 166% (handled in 6 second)
                    // on 4 tick - 166% (handled in 8 second)
                    // on 5 tick - 166% (handled in 10 second)
                    // on 6+ tick - 100% (handled in 12 second)
                    // Not need update after 4 tick
                    switch (m_tickNumber)
                    {
                        case 1:   // 0
                            (*i)->GetModifier()->m_amount = 0; // set to 0
                            break;
                        case 3:   // 166%
                            (*i)->GetModifier()->m_amount = m_modifier.m_amount * 5 / 3; // set to 166%
                            break;
                        case 6:
                            (*i)->GetModifier()->m_amount = m_modifier.m_amount; // return to normal
                            break;
                        default:
                            return;
                    }
...
//        case 33831: break;
        ...
        case 34074:
        {
            ...
            float mana = m_target->GetPower(POWER_MANA);
            float max_mana = m_target->GetMaxPower(POWER_MANA);
            float intellect = m_target->GetStat(STAT_INTELLECT);
            float level = m_target->GetLevel();
            int32 mp5 = intellect * 22.0f / 35.0f * (0.9f - mana / max_mana) + intellect * 0.11f + 0.35f * level;

            m_modifier.m_amount = mp5;
...
//        case 45390: break;
        ...
        case 45406:
        {
            if (m_tickNumber >= 3)
            {
                m_target->RemoveAurasDueToSpell(45406);
                m_target->CastSpell(m_target, 29175, true);
            }
            break;
        }
//      // Parachute
...
//        case 49472: break;
        ...
        {
            if (GetTickNumber() == 1) // first tick is on initial cast. PeriodicDummyTick() works like that.
                break;

            if (m_target->GetTypeId() == TYPEID_UNIT)
            {
                Creature* cre = m_target->ToCreature();
                if (cre->IsAIEnabled)
                    cre->AI()->DoAction(1); // call script text
            }

            m_target->CastSpell(m_target, 45657, true);
            break;
        }
        case 55193:
        {
            ...
            // not too far
            if (!target || !target->IsWithinDistInMap(m_target, INTERACTION_DISTANCE))
                m_target->RemoveAurasDueToSpell(55193);
...
void Aura::HandleArenaPreparation(bool apply, bool Real)
{
    ...
    if (m_target->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if (apply)
            m_target->CastSpell(m_target, 55169+((Player*)m_target)->GetClass(), true); // cast selected aura
        else
            m_target->RemoveAurasDueToSpell(55169+((Player*)m_target)->GetClass());
    }
}
...
void Aura::HandleAuraMirrorImage(bool apply, bool Real)
{
    ...
    Unit* pUnit = GetTarget();
    
    if(!pUnit)
        return;

    Creature* pCreature = pUnit->GetTypeId() == TYPEID_UNIT ? (Creature*)pUnit : NULL;
    ...
    if (apply)
    {
        ...
        pUnit->SetFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_CLONED);
    ...
    else // there is unit, but not creature
        pUnit->RemoveFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_CLONED);
}
...
void Aura::UnregisterSingleCastAura()
{
    if (IsSingleTarget())
    {
        ...
        {
            sLog.outLog(LOG_CRASH, "ERROR: Couldn't find the caster of the single target aura, may crash later!");
            ASSERT(false);
        }
...
void Aura::HandleAuraPermanentTransform(bool apply, bool Real)
{
    if (m_target->GetTypeId() != TYPEID_PLAYER)
        return;
    ...
    if (apply)
    {
        ...
        if (!ci)
        {
            ...
            sLog.outLog(LOG_DEFAULT, "Auras: unknown creature id = %d (only need its modelid) Form Spell Aura Transform in Spell ID = %d", m_modifier.m_miscvalue, GetId());
        }
        else
        {
            if (uint32 modelid = ci->GetRandomValidModelId())
            {
                m_target->SetDisplayId(modelid);
                m_target->SetNativeDisplayId(modelid);
            }
        }
        m_target->setTransForm(GetId());
    }
    ...
    m_target->SetByteValue(UNIT_FIELD_BYTES_0, 2, gender);
    m_target->SetByteValue(PLAYER_BYTES_3, 0, gender);
    m_target->SetNativeDisplayId(OriginalModelId);
    m_target->SetDisplayId(OriginalModelId);
    m_target->setTransForm(0);
    }
}

void Aura::HandleSendSpellVisualPeriodic(bool apply, bool Real)
{
    m_isPeriodic = apply;
}

void Aura::HandleSpectatorVisible(bool apply, bool Real)
{
    if (!Real)
        return;

    HandleAuraModPacifyAndSilence(apply, Real);
    ...
    if (apply)
    {
        ...
        if (m_target->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            if (pet)
                ((Player*)m_target)->RemovePet(NULL, PET_SAVE_NOT_IN_SLOT);
            ((Player*)m_target)->SetTemporaryUnsummonedPetNumber(0);
        }
        m_target->CastSpell(m_target, 54824, true); // Fly and morph
        m_target->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE);
    }
    else
    {
        m_target->RemoveAurasDueToSpell(54824); // Fly and morph
        m_target->SetNonAttackableFlag(UNIT_FLAG_NON_ATTACKABLE, false);
    }
}

void Aura::HandleAuraXpPct(bool apply, bool Real)
{
    // all applied/removed only at real aura add/remove
    // if (!Real)
    //     return;
...
void Aura::HandleAuraRepPct(bool apply, bool Real)
{
    // all applied/removed only at real aura add/remove
    // if (!Real)
    //     return;
