...
void WorldSession::HandleInspectArenaStatsOpcode(WorldPacket & recv_data)
{
    ...
    uint64 guid;
    recv_data >> guid;
    sLog.outDebug("Inspect Arena stats %llu", guid);

    if (Player *plr = sObjectMgr.GetPlayerInWorld(guid))
    {
        for (uint8 i = 0; i < MAX_ARENA_SLOT; i++)
        {
            //if (i == ArenaTeam::GetSlotByType(ARENA_TEAM_1v1))
            //    continue;
...
void WorldSession::HandleArenaTeamAddMemberOpcode(WorldPacket & recv_data)
{
    ...
    // solo 3v3 and BG
    if (arenateam->GetType() == ARENA_TEAM_3v3 || arenateam->GetType() == ARENA_TEAM_5v5)
    {
    ...
    bool AllowTwoSideArena = sWorld.getConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_ARENA) && !BattleGroundMgr::IsArenaTypeRestrictedPVE(arenateam->GetType());

    if (!AllowTwoSideArena && player->GetTeam() != GetPlayer()->GetTeam())
    {
        SendArenaTeamCommandResult(ERR_ARENA_TEAM_INVITE_SS, "", "", ERR_ARENA_TEAM_NOT_ALLIED);
        return;
    }

    if (!AllowTwoSideArena && (sWorld.getConfig(CONFIG_BATTLEGROUND_ALLOW_DIFFERENT_FACTION) && (GetPlayer()->InBattleGroundOrArena() || player->InBattleGroundOrArena())))
    {
...
void WorldSession::HandleArenaTeamInviteAcceptOpcode(WorldPacket & /*recv_data*/)
{
    ...
    bool AllowTwoSideArena = sWorld.getConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_ARENA) && !BattleGroundMgr::IsArenaTypeRestrictedPVE(at->GetType());

    if (!AllowTwoSideArena && _player->GetTeam() != sObjectMgr.GetPlayerTeamByGUID(at->GetCaptain()))
    {
        SendArenaTeamCommandResult(ERR_ARENA_TEAM_CREATE_S,"","",ERR_ARENA_TEAM_NOT_ALLIED);// not let enemies sign petition
        return;
    }

    if (!AllowTwoSideArena && (sWorld.getConfig(CONFIG_BATTLEGROUND_ALLOW_DIFFERENT_FACTION) && _player->InBattleGroundOrArena()))
    {
        if (_player->TeamForRace(_player->GetRace()) != sObjectMgr.GetPlayerTeamByGUID(at->GetCaptain()))
        {
...
void WorldSession::HandleArenaTeamLeaveOpcode(WorldPacket & recv_data)
{
    ...
    //if (at->GetType() == ARENA_TEAM_3v3)
    //{
    //    SendNotification(_player->GetSession()->GetHellgroundString(LANG_SQ_CANT_DELETE_TEAM));
    //    return;
    //}

	if (_player->InBattleGroundOrArenaQueue() || _player->InBattleGroundOrArena())
	{
		SendNotification(_player->GetSession()->GetHellgroundString(15651));
		return;
	}
    ...
    //if (at->CantLeave())
    //{
    //    sLog.outLog(LOG_SPECIAL, "CMSG_ARENA_TEAM_LEAVE #2 %s", _player->GetName()); //DELETEME
    //    return;
    //}
...
void WorldSession::HandleArenaTeamDisbandOpcode(WorldPacket & recv_data)
{
    ...
    if (at->CantLeave())
    {
        SendNotification(_player->GetSession()->GetHellgroundString(15581));
        return;
    }
...
void WorldSession::HandleArenaTeamRemoveFromTeamOpcode(WorldPacket & recv_data)
{
    ...
    Player *player = ObjectAccessor::GetPlayerInWorld(member->guid);
    if (player)
    {
		if (at->GetType() == ARENA_TEAM_5v5)
		{
			if (player->InBattleGroundOrArenaQueue() || player->InBattleGroundOrArena())
				SendNotification(_player->GetSession()->GetHellgroundString(15652));
			return;
		}
		else
		{
			if (player->InArenaQueue() || player->InArena())
				SendNotification(_player->GetSession()->GetHellgroundString(15581));
			return;
		}
    }
