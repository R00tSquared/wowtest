...
namespace VMAP
{
    ...
    VMapClusterManager::VMapClusterManager(uint32 processNumber) : m_processNumber(processNumber), m_masterPid(0)
    {
        ...
        if(packet.size() != 1+sizeof(pid_t))
            sLog.outLog(LOG_DEFAULT, "ERROR: VMapClusterManager: failed to receive master pid, invalid packet size %llu (%llu)", packet.size(), 1+sizeof(pid_t));
        else
        {
            packet.read_skip(1);
            packet >> m_masterPid;
        ...
        else // no master, just wait for threads to finish
        {
            for(int i = 0; i < n; i++)
                if(ACE_Thread::join(htids[i]) == -1)
                    sLog.outLog(LOG_DEFAULT, "ERROR: VMapClusterManager::Start(): failed to join thread id=%d tid=%llu because of error %d", i, tids[i], ACE_OS::last_error());
        }
    ...
    void VMapClusterManager::Run()
    {
        ...
        while(true)
        {
            ...
            {
                sLog.outLog(LOG_DEFAULT, "ERROR: VMapClusterManager::Run(): received packet with invalid size %llu (%llu)", packet.size(),  1+4+4+sizeof(float)*6);
                return;
            }
            ...
            {
                SendFailCode(tid);
                sLog.outLog(LOG_DEFAULT, "ERROR: VMapClusterManager::Run(): received packet with invalid size %llu (2)", packet.size());
                return;
            }
    ...
    VMapClusterProcess::VMapClusterProcess(uint32 processId) : m_processId(processId), m_masterPid(0)
    {
        ...
        if(packet.size() != 1+sizeof(pid_t))
            sLog.outLog(LOG_DEFAULT, "ERROR: VMapClusterProcess: failed to receive master pid, invalid packet size %llu (%llu)", packet.size(), 1+sizeof(pid_t));
        else
        {
            packet.read_skip(1);
            packet >> m_masterPid;
    ...
    int VMapClusterProcess::Run()
    {
        ...
        while(true)
        {
            ...
            if(packet.size() != 1+4+4+sizeof(float)*6)
            {
                sLog.outLog(LOG_DEFAULT, "ERROR: VMapClusterProcess::Run(): received packet with invalid size %llu (%llu)", packet.size(), 1+4+4+sizeof(float)*6);
                return 0;
            }
    ...
    bool LoSProxy::isInLineOfSight(unsigned int pMapId, float x1, float y1, float z1, float x2, float y2, float z2)
    {
        ...
        if (packet.size() != 2)
        {
            sLog.outLog(LOG_DEFAULT, "ERROR: LoSProxy::isInLineOfSight: received packet with invalid size %llu (2)", packet.size());
            response = 2;
        }
        else
        {
