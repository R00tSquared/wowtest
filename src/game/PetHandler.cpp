...
void WorldSession::HandlePetAction(WorldPacket & recv_data)
{
    ...
    {
        sLog.outLog(LOG_DEFAULT, "ERROR: PetHandler:: charm(%u), player(guid %u) doesn't have such pet/charm.", uint32(GUID_LOPART(charmGUID)), pPlayer->GetGUIDLow());
        return;
    }
    ...
    if (!pCharmInfo)
    {
        sLog.outLog(LOG_DEFAULT, "ERROR: WorldSession::HandlePetAction: object %llu is considered pet-like but doesn't have a charminfo!", charmGUID);
        return;
    }
...
void WorldSession::HandlePetSetAction(WorldPacket & recv_data)
{
    ...
    if (ObjectAccessor::GetPlayerInWorld(petguid))
        return;
    ...
    if (!charmInfo)
    {
        sLog.outLog(LOG_DEFAULT, "ERROR: WorldSession::HandlePetSetAction: object %llu is considered pet-like but doesn't have a charminfo!", pet->GetGUID());
        return;
    }
...
void WorldSession::HandlePetUnlearnOpcode(WorldPacket& recvPacket)
{
    ...
    if (!charmInfo)
    {
        sLog.outLog(LOG_DEFAULT, "ERROR: WorldSession::HandlePetUnlearnOpcode: object %llu is considered pet-like but doesn't have a charminfo!", pet->GetGUID());
        return;
    }
...
void WorldSession::HandlePetSpellAutocastOpcode(WorldPacket& recvPacket)
{
    ...
    if (ObjectAccessor::GetPlayerInWorld(guid))
        return;
    ...
    if (!charmInfo)
    {
        sLog.outLog(LOG_DEFAULT, "ERROR: WorldSession::HandlePetSpellAutocastOpcod: object %llu is considered pet-like but doesn't have a charminfo!", pet->GetGUID());
        return;
    }
