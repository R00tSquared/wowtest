...
class MapUpdateRequest : public ACE_Method_Request
{
    public:
        ...
        virtual int call(void)
        {
            m_updater.register_thread(ACE_OS::thr_self(), m_map.GetId(), m_map.GetAnyInstanceId());
...
int MapUpdater::schedule_update(Map& map, ACE_UINT32 diff)
{
    ...
    if (!rq)
        return 2;
    if (this->m_executor.execute(rq) == -1)
    {
...
void MapUpdater::FreezeDetect()
{
    ...
    if (!m_threads.empty())
    {
        for (ThreadMapMap::const_iterator itr = m_threads.begin(); itr != m_threads.end(); ++itr)
        {
            ...
            {
                sLog.outLog(LOG_CRASH, "MapUpdater::FreezeDetect thread %llu possible freezed (is update map %u instance %u). Killing.",itr->first, itr->second.GetId(), itr->second.GetAnyInstanceId());
                debug_log("MapUpdater::FreezeDetect thread " I64FMT " possible freezed (is update map %u instance %u). Killing.",itr->first, itr->second.GetId(), itr->second.GetAnyInstanceId());

                if (Map *brokenMap = sMapMgr.FindMap(itr->second.GetId(), itr->second.GetAnyInstanceId()))
