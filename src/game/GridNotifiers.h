...
#pragma region Checks
    ...
    {
    public:
        MostHPMissingInRange(Unit const* obj, float range, uint32 hp, bool targetSelf) : i_obj(obj), i_range(range), i_hp(hp), i_targetSelf(targetSelf) {}
        bool operator()(Unit* u)
        {
            if (u->isAlive() && u->IsInCombat() && i_obj->IsFriendlyTo(u) && i_obj->IsWithinDistInMap(u, i_range) && u->GetMaxHealth() - u->GetHealth() > i_hp)
            {
    ...
    class FriendlyHasSomethingToDispell
    {
    public:
        FriendlyHasSomethingToDispell(Unit const* obj, float range, uint32 dispelMask) : i_obj(obj), i_range(range), i_dispelmask(dispelMask) {}
        bool operator()(Unit* u)
        {
            if (u->isAlive() && u->IsInCombat() && i_obj->IsFriendlyTo(u) && i_obj->IsWithinDistInMap(u, i_range) && (u->HasAurasWithDispelMask(i_dispelmask)))
            {
                return true;
            }
            return false;
    ...
    // AllObjectsInRange and type == GAMEOBJECT_TRAP and state GO_READY - 
    // break line of path to a trigger per 2 yds (trap radius is 2), count of triggers depends on distance travelled - WORKS FOR EVERYONE!
    class AllReadyTrapsInRange // all traps within range of distance from FIRST_POINT to END_POINT of movement. 
        //Then the traps check distance to each of the triggers to see if it should be triggered 
    {
    public:
        AllReadyTrapsInRange(Unit const * pUn, float radius) : pUnit(pUn), range(radius) {}
        bool operator() (GameObject* g)
        {
            if (g->getLootState() != GO_READY)
                return false;

            GameObjectInfo const* goInfo = g->GetGOInfo();
            if (!goInfo || goInfo->type != GAMEOBJECT_TYPE_TRAP)
                return false;

            if (g->m_cooldownTime > WorldTimer::getMSTime())
                return false;

            Unit* owner = g->GetOwner();
            if (owner && owner->IsFriendlyTo(pUnit))
                return false;
            ...
            if (!radius)
            {
                if (goInfo->trap.cooldown != 3)            // cast in other case (at some triggering/linked go/etc explicit call)
                {
                    ...
                    if (const SpellEntry* spell = GetSpellStore()->LookupEntry<SpellEntry>(goInfo->trap.spellId))
                        radius = SpellMgr::GetSpellRadius(spell,0,false);
    ...
    struct ContestedGuardCheck
    {
        ...
        bool operator()(Unit* u)
        { 
            return (u->IsContestedGuard() && u->isAlive() && (u->GetTypeId() == TYPEID_UNIT) && u->IsAIEnabled && u->IsWithinLOSInMap(_source));
        }
        WorldObject* _source;
    };
