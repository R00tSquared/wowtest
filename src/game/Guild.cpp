...
bool Guild::create(uint64 lGuid, std::string gname)
{
    ...
    RealmDataDatabase.PExecute("INSERT INTO guild (guildid,name,leaderguid,info,motd,createdate,EmblemStyle,EmblemColor,BorderStyle,BorderColor,BackgroundColor,BankMoney) "
        "VALUES('%u','%s','%u', '%s', '%s', NOW(),'%u','%u','%u','%u','%u','%llu')",
...
{
    Player* pl = sObjectAccessor.GetPlayerInWorldOrNot(plGuid);
    if (pl)
    {
        if (pl->GetGuildId() != 0)
            return false;
    ...
    Player::RemovePetitionsAndSigns(plGuid, PETITION_TYPE_GUILD);
...
bool Guild::LoadRanksFromDB(uint32 GuildId)
{
    ...
    if (broken_ranks)
    {
        ...
        for (size_t i =0; i < m_ranks.size(); ++i)
        {
            ...
            RealmDataDatabase.PExecute("INSERT INTO guild_rank (guildid,rid,rname,rights) VALUES ('%u', '%llu', '%s', '%u')", GuildId, i+1, name.c_str(), rights);
        }
        RealmDataDatabase.CommitTransaction();
    }
...
bool Guild::FillPlayerData(uint64 guid, MemberSlot* memslot)
{
    ...
    Player* pl = sObjectMgr.GetPlayerInWorld(guid);
    if (pl)
    {
        plName  = pl->GetName();
        plLevel = pl->GetLevel();
...
void Guild::LoadPlayerStatsByGuid(uint64 guid)
{
    ...
    Player *pl = ObjectAccessor::GetPlayerInWorld(guid);
    if (!pl)
        return;
...
void Guild::DelMember(uint64 guid, bool isDisbanding)
{
    if (leaderGuid == guid && !isDisbanding)
    {
        ...
        if (Player *newLeader = sObjectMgr.GetPlayerInWorld(newLeaderGUID))
    ...
    if (player)
    {
        player->SetInGuild(0);
        player->SetRank(0);
        player->RemoveAurasDueToSpell(SPELL_GUILD_HOUSE_STATS_BUFF);
    }
...
void Guild::BroadcastToGuild(WorldSession *session, const std::string& msg, uint32 language, bool bad_lexics)
{
    if (session && (session->HasPermissions(PERM_GMT) || (session->GetPlayer() && HasRankRight(session->GetPlayer()->GetRank(), GR_RIGHT_GCHATSPEAK))))
    {
        ...
        if (bad_lexics)
        {
            session->GetPlayer()->SendPacketToSelf(&data);
            return;
        }

        for (MemberList::const_iterator itr = members.begin(); itr != members.end(); ++itr)
        {
            Player *pl = ObjectAccessor::GetPlayerInWorld(MAKE_NEW_GUID(itr->first, 0, HIGHGUID_PLAYER));
...
void Guild::BroadcastToOfficers(WorldSession *session, const std::string& msg, uint32 language, bool bad_lexics)
{
...
void Guild::BroadcastPacket(WorldPacket *packet)
{
    for (MemberList::iterator itr = members.begin(); itr != members.end(); ++itr)
    {
        Player *player = ObjectAccessor::GetPlayerInWorld(MAKE_NEW_GUID(itr->first, 0, HIGHGUID_PLAYER));
        if (player)
...
void Guild::CreateRank(std::string name_,uint32 rights)
{
    ...
    RealmDataDatabase.PExecute("INSERT INTO guild_rank (guildid,rid,rname,rights) VALUES ('%u', '%llu', '%s', '%u')", Id, m_ranks.size(), name_.c_str(), rights);
}
...
void Guild::WriteMemberRosterPacket(Player *sessionPlayer, const MemberSlot &member, Player *pl, WorldPacket &data)
{
    if (pl)
    {
        ...
        if (!sessionPlayer->isGameMaster() && sWorld.getConfig(CONFIG_ENABLE_FAKE_WHO_ON_ARENA) && sWorld.getConfig(CONFIG_ENABLE_FAKE_WHO_IN_GUILD))
        {
            if (pl->InArena())
            {
                WorldLocation const& loc = pl->GetBattleGroundEntryPoint();
    ...
    if (members.size() > membersSize){
        ...
        for (MemberGuidList::const_iterator itr = m_membersOrder.begin(); itr != m_membersOrder.end(); ++itr){
            if (count > MAX_ROSTER_MEMBERS)
                break;
            MemberSlot member = members[*itr];
            Player *pl = ObjectAccessor::GetPlayerInWorld(MAKE_NEW_GUID(member.guid, 0, HIGHGUID_PLAYER));
            if (pl && !pl->IsHidden()){
...
void Guild::LogGuildEvent(uint8 EventType, uint32 PlayerGuid1, uint32 PlayerGuid2, uint8 NewRank)
{
    ...
    // moonwell: added INNORE because SQL errors (addon maybe?)
    //2020-10-27 22:15:41 SQL: INSERT INTO guild_eventlog (guildid, LogGuid, EventType, PlayerGuid1, PlayerGuid2, NewRank, TimeStamp) VALUES ('4751','162','1','764053','691906','0','1603826141')
    //2020-10-27 22:15:41 SQL ERROR: Duplicate entry '4751-162' for key 'PRIMARY'
    //2020-10-27 22:15:41 SQL: INSERT INTO guild_eventlog (guildid, LogGuid, EventType, PlayerGuid1, PlayerGuid2, NewRank, TimeStamp) VALUES ('4751','162','1','764053','691967','0','1603826141')
    //2020-10-27 22:15:41 SQL ERROR: Duplicate entry '4751-162' for key 'PRIMARY'
    //2020-10-27 22:15:41 SQL: INSERT INTO guild_eventlog (guildid, LogGuid, EventType, PlayerGuid1, PlayerGuid2, NewRank, TimeStamp) VALUES ('4751','162','1','764053','692519','0','1603826141')
    //2020-10-27 22:15:41 SQL ERROR: Duplicate entry '4751-162' for key 'PRIMARY
    RealmDataDatabase.PExecute("INSERT IGNORE INTO guild_eventlog (guildid, LogGuid, EventType, PlayerGuid1, PlayerGuid2, NewRank, TimeStamp) VALUES ('%u','%u','%u','%u','%u','%u','%llu')",
...
void Guild::SetBankMoney(int64 money)
{
    ...
    RealmDataDatabase.PExecute("UPDATE guild SET BankMoney='%llu' WHERE guildid='%u'", money, Id);
}
...
void Guild::LogBankEvent(uint8 LogEntry, uint8 TabId, Player* pl, uint32 ItemOrMoney, uint8 ItemStackCount, uint8 DestTabId)
{
    GuildBankEvent *NewEvent = new GuildBankEvent;

    NewEvent->LogGuid = LogMaxGuid++;
    NewEvent->LogEntry = LogEntry;
    NewEvent->PlayerGuid = pl->GetGUIDLow();
    ...
    RealmDataDatabase.PExecute("INSERT INTO guild_bank_eventlog (guildid,LogGuid,LogEntry,TabId,PlayerGuid,ItemOrMoney,ItemStackCount,DestTabId,TimeStamp) VALUES ('%u','%u','%u','%u','%u','%u','%u','%u','%llu')",
    ...
    case GUILD_BANK_LOG_DEPOSIT_ITEM:
        sLog.outLog(LOG_TRADE, "Player %s (Acc: %u) deposit item: %d (Count: %u) to guild bank (ID: %u)",
            pl->GetName(), pl->GetSession()->GetAccountId(),
            ItemOrMoney, ItemStackCount, GetId());
        break;
    case GUILD_BANK_LOG_WITHDRAW_ITEM:
        sLog.outLog(LOG_TRADE, "Player %s (Acc: %u) withdrew item: %d (Count: %u) from guild bank (ID: %u)",
            pl->GetName(), pl->GetSession()->GetAccountId(),
            ItemOrMoney, ItemStackCount, GetId());
        break;
    case GUILD_BANK_LOG_DEPOSIT_MONEY:
        sLog.outLog(LOG_TRADE, "Player %s (Acc: %u) deposit %u money to guild bank (ID: %u)",
            pl->GetName(), pl->GetSession()->GetAccountId(),
            ItemOrMoney, GetId());
        break;
    case GUILD_BANK_LOG_WITHDRAW_MONEY:
        sLog.outLog(LOG_TRADE, "Player %s (Acc: %u) withdrew %u money from guild bank (ID: %u)",
            pl->GetName(), pl->GetSession()->GetAccountId(),
            ItemOrMoney, ItemStackCount, GetId());
        break;
    }
