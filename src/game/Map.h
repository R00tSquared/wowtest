...
struct InstanceTemplate
{
    ...
    uint32 reset_delay_raid;
    uint32 reset_delay_heroic;
    uint32 access_id;
    float startLocX;
    float startLocY;
    float startLocZ;
...
struct AdditionalInfo
{
    ...
    uint8 eot_count;
    uint8 plus_drop_pre;
    uint8 plus_drop_final;
    std::string legendary_weapons;
};
...
#define FALL_UNDER_MAP_HEIGHT -500.0f
...
class HELLGROUND_IMPORT_EXPORT Map : public GridRefManager<NGridType>
{
    ...
    public:
        ...
        uint32 GetAnyInstanceId() const { return i_InstanceId; }
        /*Returns instance id only for instanciated maps*/
        uint32 GetInstanciableInstanceId() const { return i_InstanceId > SINGLEINSTANCE_RESERVED_INSTANCES_LAST ? i_InstanceId : 0; }
        ...
        bool IsHeroicRaid() const { return i_mapEntry && i_mapEntry->IsRaid() && i_spawnMode == DIFFICULTY_HEROIC; }
        ...
        bool isCellMarked(uint32 pCellId) { return marked_cells.testAt(pCellId); }
        void markCell(uint32 pCellId) { marked_cells.setTrue(pCellId); }
        void setStartAndEndForReset(uint32 s, uint32 e) { marked_cells.setStartEndForReset(s, e); }

        bool HavePlayers() const { return !m_mapRefManager.isEmpty(); }
        uint32 GetPlayersCountExceptGMs(bool is_rogue = false) const;
        ...
        bool DebugEnsureGridLoadedAtEnter(float x, float y);
        static ModDamageHP GetMapCreatureMod(uint32 mapId, bool IsHeroic);
        static std::string PrintMapModMinMax(ModDamageHP mod);
        static uint32 GetMaxPlayers(uint32 mapid, bool heroic_mode = false);
    ...
    private:
        ...
        void EnsureGridLoadedAtEnter(Cell const&);
...
class HELLGROUND_IMPORT_EXPORT InstanceMap : public Map
{
    public:
        ...
        virtual void InitVisibilityDistance();
        uint32 Calculate_GBK_Guild(uint32* leaderLowGuid = NULL) const;
...
{
    public:
        BattleGroundMap(uint32 id, time_t, uint32 InstanceId, uint8 mode, BattleGround *bg);
        ~BattleGroundMap();
