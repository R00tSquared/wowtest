                ...
                switch (action.type)
                {
                    ...
                    case ACTION_T_RANDOM_SAY:
                    {
                        if (action.raw.param1)
                            idx_set.erase(action.raw.param1);
                        if (action.raw.param2)
                            idx_set.erase(action.raw.param2);
                        break;
                    }
...
// -------------------
void CreatureEventAIMgr::LoadCreatureEventAI_Positions(bool check_entry_use)
{

    //Drop Existing EventSummon Map
    m_CreatureEventAI_Positions_Map.clear();

    // Gather additional data for EventAI
    QueryResultAutoPtr result = GameDataDatabase.Query("SELECT id, position_x, position_y, position_z, orientation FROM creature_ai_positions");
    if (result)
    {
        //BarGoLink bar(result->GetRowCount());
        uint32 Count = 0;
        ...
        do
        {
            ...
            CreatureEventAI_Positions temp;
            ...
            if (!Hellground::IsValidMapCoord(temp.position_x,temp.position_y,temp.position_z,temp.orientation))
            {
                sLog.outLog(LOG_DB_ERR, "CreatureEventAI:  Positions id %u have wrong coordinates (%f,%f,%f,%f), skipping.", i,temp.position_x,temp.position_y,temp.position_z,temp.orientation);
                continue;
            }

            //Add to map
            m_CreatureEventAI_Positions_Map[i] = temp;
            ++Count;
        }
        while (result->NextRow());

        if (check_entry_use)
            CheckUnusedAIPositions();

        sLog.outString();
        sLog.outString(">> Loaded %u CreatureEventAI Positions definitions", Count);
    }
    else
    {
        //BarGoLink bar(1);
        ////bar.step();
        sLog.outString();
        sLog.outString(">> Loaded 0 CreatureEventAI Positions definitions. DB table `creature_ai_positions` is empty.");
    }
}

void CreatureEventAIMgr::CheckUnusedAIPositions()
{
    std::set<int32> idx_set;
    // check not used strings this is negative range
    for (CreatureEventAI_Positions_Map::const_iterator itr = m_CreatureEventAI_Positions_Map.begin(); itr != m_CreatureEventAI_Positions_Map.end(); ++itr)
        idx_set.insert(itr->first);
        ...
        for (size_t i = 0; i < itr->second.size(); ++i)
        {
            ...
            for (int j = 0; j < MAX_ACTIONS; ++j)
            {
                ...
                switch (action.type)
                {
                    ...
                    {
                        if (action.movepoint.positionId)
                            idx_set.erase(action.movepoint.positionId);
                        break;
                    }
                    default: break;
                }
    ...
    for (std::set<int32>::const_iterator itr = idx_set.begin(); itr != idx_set.end(); ++itr)
        sLog.outLog(LOG_DB_ERR, "CreatureEventAI:  Entry %i in table `creature_ai_positions` but not used in EventAI scripts.",*itr);
}
    ...
    if (result)
    {
        ...
        do
        {
            ...
            switch (temp.event_type)
            {
                ...
                {
                    if(temp.raw.param1 == SPELL_AURA_NONE || temp.raw.param1 >= TOTAL_AURAS)
                    {
                        sLog.outLog(LOG_DB_ERR, "CreatureEventAI:  Creature %u has wrong Aura Type (%u) defined in event %u.", entry, temp.raw.param1, i);
                        continue;
                    }

                    if (temp.raw.param4 < temp.raw.param3)
                        sLog.outLog(LOG_DB_ERR, "CreatureEventAI:  Creature %u are using repeatable event(%u) with param4 < param3 (RepeatMax < RepeatMin). Event will never repeat.", entry, i);
                    break;
                }
                default:
            ...
            for (uint32 j = 0; j < MAX_ACTIONS; j++)
            {
                ...
                switch (action.type)
                {
                    ...
                    case ACTION_T_RANDOM_SAY:
                    {
                        if(action.raw.param1)
                        {
                            if (action.raw.param1 > MIN_CREATURE_AI_TEXT_STRING_ID || action.raw.param1 <= MAX_CREATURE_AI_TEXT_STRING_ID)
                            {
                                sLog.outLog(LOG_DB_ERR, "CreatureEventAI:  Event %u Action %u param1 references out-of-range entry (%i) in texts table.", i, j+1, action.raw.param1);
                                action.raw.param1 = 0;
                            }
                            else if (m_CreatureEventAI_TextMap.find(action.raw.param1) == m_CreatureEventAI_TextMap.end())
                            {
                                sLog.outLog(LOG_DB_ERR, "CreatureEventAI:  Event %u Action %u param1 references non-existing entry (%i) in texts table.", i, j+1, action.raw.param1);
                                action.raw.param1 = 0;
                            }
                        }
                        if(action.raw.param2)
                        {
                            if (action.raw.param2 > MIN_CREATURE_AI_TEXT_STRING_ID || action.raw.param2 <= MAX_CREATURE_AI_TEXT_STRING_ID)
                            {
                                sLog.outLog(LOG_DB_ERR, "CreatureEventAI:  Event %u Action %u param2 references out-of-range entry (%i) in texts table.", i, j+1, action.raw.param2);
                                action.raw.param2 = 0;
                            }
                            else if (m_CreatureEventAI_TextMap.find(action.raw.param2) == m_CreatureEventAI_TextMap.end())
                            {
                                sLog.outLog(LOG_DB_ERR, "CreatureEventAI:  Event %u Action %u param2 references non-existing entry (%i) in texts table.", i, j+1, action.raw.param2);
                                action.raw.param2 = 0;
                            }
                        }
                        break;
                    ...
                    {
                        const SpellEntry *spell = sSpellTemplate.LookupEntry<SpellEntry>(action.castguid.spellId);
                        if (!spell)
                    ...
                    case ACTION_T_MOUNT_TO_ENTRY_OR_MODEL:
                        if (action.mount.creatureId != 0 || action.mount.modelId != 0)
                        {
                            ...
                            {
                                sLog.outLog(LOG_DB_ERR, "Event %u Action %u uses nonexistent Creature entry %u.", i, j + 1, action.mount.creatureId);
                                action.morph.creatureId = 0;
                            }
                            ...
                            if (action.mount.modelId)
                            {
                                ...
                                {
                                    sLog.outLog(LOG_DB_ERR, "Event %u Action %u have unused ModelId %u with also set creature id %u.", i, j + 1, action.mount.modelId, action.mount.creatureId);
                                    action.mount.modelId = 0;
                                }
                                else if (!sCreatureDisplayInfoStore.LookupEntry(action.mount.modelId))
                                {
                                    sLog.outLog(LOG_DB_ERR, "Event %u Action %u uses nonexistent ModelId %u.", i, j + 1, action.mount.modelId);
                                    action.mount.modelId = 0;
                                }
                            }
                        }
                    ...
                    case ACTION_T_DYNAMIC_MOVEMENT:         // CombatMovementEnabled (1 = on; 0 = off)
                        break;
                    ...
                    case ACTION_T_MOVE_POINT:
                    {
                        if (m_CreatureEventAI_Positions_Map.find(action.movepoint.positionId) == m_CreatureEventAI_Positions_Map.end())
                            sLog.outLog(LOG_DB_ERR, "CreatureEventAI:  Event %u Action %u position missing CreatureEventAI_Positions %u", i, j+1, action.movepoint.positionId);
                        break;
                    }
                    case ACTION_T_SET_HOME_POSITION:
                    {
                        if (m_CreatureEventAI_Positions_Map.find(action.sethomepos.positionId) == m_CreatureEventAI_Positions_Map.end())
                            sLog.outLog(LOG_DB_ERR, "CreatureEventAI:  Event %u Action %u position missing CreatureEventAI_Positions %u", i, j+1, action.sethomepos.positionId);
                        break;
                    }
                    case ACTION_T_SET_ORIENTATION:
                    {
                        if (m_CreatureEventAI_Positions_Map.find(action.setorientation.positionId) == m_CreatureEventAI_Positions_Map.end())
                            sLog.outLog(LOG_DB_ERR, "CreatureEventAI:  Event %u Action %u position missing CreatureEventAI_Positions %u", i, j+1, action.setorientation.positionId);
                        break;
                    }
                    case ACTION_T_NEAR_TELEPORT:
                    {
                        if (m_CreatureEventAI_Positions_Map.find(action.setorientation.positionId) == m_CreatureEventAI_Positions_Map.end())
                            sLog.outLog(LOG_DB_ERR, "CreatureEventAI:  Event %u Action %u position missing CreatureEventAI_Positions %u", i, j+1, action.raw.param1);
                        break;
                    }
                    case ACTION_T_THROW_AI_EVENT:
                        if (action.throwEvent.eventType >= MAXIMAL_AI_EVENT_EVENTAI)
                        {
                            sLog.outLog(LOG_DB_ERR, "Event %u Action %u uses invalid event type %u (must be less than %u), skipping", i, j + 1, action.throwEvent.eventType, MAXIMAL_AI_EVENT_EVENTAI);
                            continue;
                        }
                        if (action.throwEvent.radius > SIZE_OF_GRIDS)
                            sLog.outLog(LOG_DB_ERR, "Event %u Action %u uses unexpectedly huge radius %u (expected to be less than %f)", i, j + 1, action.throwEvent.radius, SIZE_OF_GRIDS);

                        if (action.throwEvent.radius == 0)
                        {
                            sLog.outLog(LOG_DB_ERR, "Event %u Action %u uses unexpected radius 0 (set to %f of CONFIG_CREATURE_FAMILY_ASSISTANCE_RADIUS)", i, j + 1, sWorld.getConfig(CONFIG_CREATURE_FAMILY_ASSISTANCE_RADIUS));
                            action.throwEvent.radius = uint32(sWorld.getConfig(CONFIG_CREATURE_FAMILY_ASSISTANCE_RADIUS));
                        }
                        break;
                    default:
                ...
                case EVENT_T_FRIENDLY_HP:
                {
                    ...
                    for (uint32 j = 0; j < MAX_ACTIONS; ++j)
                    {
                        ...
                        if (temp.action[j].type == ACTION_T_CAST && temp.action[j].cast.target == TARGET_T_EVENT_SPECIFIC)
                        {
                            ...
                            if (spellInfo->AttributesEx &(SPELL_ATTR_EX_CANT_TARGET_SELF) || spellInfo->EffectImplicitTargetA[j] == TARGET_UNIT_TARGET_ALLY)
                                data.friendlyHp.targetSelf = false;
                        }
                    }
                    m_creatureEventAI_ComputedDataMap.insert(std::make_pair(temp.event_id, data));
                    break;
                }
                default: break;
            }
