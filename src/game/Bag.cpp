...
void Bag::AddToWorld()
{
    ...
    for (uint32 i = 0;  i < GetBagSize(); ++i)
        if (m_bagslot[i])
        {
            if (m_bagslot[i] == this)
            {
                sLog.outLog(LOG_DEFAULT, "ERROR: Bag has self in slot: %u, bag size: %u, owner: %llu", GetSlotByItemGUID(m_bagslot[i]->GetGUID()), GetBagSize(), GetOwnerGUID());
                continue;
            }
...
void Bag::StoreItem(uint8 slot, Item *pItem, bool /*update*/)
{
    ...
    {
        sLog.outLog(LOG_DEFAULT, "ERROR: Player GUID %llu tried to manipulate packets and crash the server.", GetOwnerGUID());
        return;
    }
