...
struct FakeBotAI : public PlayerAI
{
    FakeBotAI(Player *pPlayer);

    void Reset() {};
    void UpdateAI(const uint32 diff);
    uint32 GenerateLevelupTimer();
    uint32 GenerateChangelocTimer();
    void SendToRandomRaid(uint32 count);

private:
    bool in_raid;

    TimeTrackerSmall m_unloadTimer;
    TimeTrackerSmall m_checkInvited;
    TimeTrackerSmall m_changelocTimer;
    TimeTrackerSmall m_levelupTimer;
};
