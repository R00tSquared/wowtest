...
void WaypointMgr::Load()
{
    ...
    result = GameDataDatabase.PQuery("SELECT `id`,`point`,`position_x`,`position_y`,`position_z`,`orientation`,`move_type`,`delay`,`action`,`action_chance`, `emote` FROM `waypoint_data` ORDER BY `id`, `point`");
    if (!result)
    {
...
void WaypointMgr::UpdatePath(uint32 id)
{
    ...
    QueryResultAutoPtr result = GameDataDatabase.PQuery("SELECT `id`,`point`,`position_x`,`position_y`,`position_z`,`orientation`,`move_type`,`delay`,`action`,`action_chance`, `emote` FROM `waypoint_data` WHERE id = %u ORDER BY `point`", id);
